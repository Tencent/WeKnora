...
### 变更
- 官方 PyPI 包名改为 **`tencent-weknora-mcp`**（由上游 WeKnora 仓库 CI 通过 Trusted Publishing 发布）。
  原社区包 `weknora-mcp` 非官方维护，请迁移安装命令。
- 项目 URL 指向 [NannaOlympicBroadcast/TreeRAGMCP](https://github.com/NannaOlympicBroadcast/TreeRAGMCP)（`mcp-server/` 目录）。
