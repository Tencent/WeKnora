---
name: discover-weknora-tree-from-section-briefs
description: 使用 writing_system.get_document_outline 返回的文档级信息、全文写作指导和目录，并读取全部有效小节的 get_section_brief，但不读取正文，从 WeKnora 素材库发现、核验并归纳与整体写作任务相关的知识，生成独立可读的主题知识树。用于素材发现上下文对照实验的小节要点组；不进行章节映射、写作规划或正文生成。
---

# 依据全文及全部小节写作要点发现 WeKnora 素材知识树
...
调用一次 `writing_system.get_document_outline`，使用其返回的文档标题、产品构想、行业、全文写作指导和目录。

再对目录中全部未忽略的二级小节各调用一次 `writing_system.get_section_brief(node_id)`。所有 `node_id` 必须来自本次目录，只读取 `node_id`、`title`、`key_point`、`status` 和 `updated_at`。

禁止调用 `get_section_content`、`get_section_review`、启发式写作、正文评审或任何写回工具。如果 brief 返回正文或等价字段，不得使用，并在审计中记录接口越界。

合并全部小节写作要点，去除重复要求，将局部措辞提升为对整个研究对象有意义的知识需求。不要按小节拆分检索任务，不建立知识到章节的映射。

把实际使用的 writing_system 字段和值保存到 `context.json`，标记：

```json
{"context_group":"section_briefs","section_briefs_used":true,"section_content_used":false}
```
