# TreeRAG Mini Program

This directory contains a WeChat Mini Program plugin for TreeRAG. It gives mobile users a lightweight entry point to:

- configure a TreeRAG API endpoint and tenant API key;
- switch UI language between Chinese and English;
- list available knowledge bases;
- import a URL into a selected knowledge base;
- ask a selected knowledge base through TreeRAG knowledge chat.
...
3. Open the **Settings** tab and fill in:
   ...
   - API Key from the TreeRAG tenant settings page;
...
- WeChat DevTools may block `localhost` requests when URL validation is enabled. For local testing, either disable domain validation in DevTools or expose TreeRAG through a HTTPS development domain.
- In production Mini Programs, add the TreeRAG API domain to the Mini Program request domain allowlist.
