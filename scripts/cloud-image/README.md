# TreeRAG 云镜像打包脚本（Cloud-Agnostic）

> **本文档面向「想把 TreeRAG 打包成云镜像（AMI / 自定义镜像 / Snapshot）分发给其他人」的用户。**
> **如果你只是想自己用 TreeRAG，请直接看主仓 [README](../../README.md)，`docker compose up -d` 即可。**
...
## 不需要 clone 整个 TreeRAG 仓库

TreeRAG 所有容器都从 Docker Hub 拉镜像（`wechatopenai/weknora-*`），Go / Python / 前端源码都不需要带到宿主机。
...
TreeRAG `docker-compose.yml` 大量服务是 **profile 限定**，本镜像只默认启动核心 5 个。
...
**1. 拷入脚本（任选一种，都不用 clone 整个 TreeRAG 仓库）。**
...
base="${GH_PROXY}https://raw.githubusercontent.com/Tencent/TreeRAG/main/scripts/cloud-image"
...
2. 用 `curl + tar` 下载 4 个运行时文件到 `/opt/TreeRAG`
...
```bash
sudo docker compose -f /opt/TreeRAG/docker-compose.yml ps
