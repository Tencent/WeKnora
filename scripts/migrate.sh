...
case "$DB_DRIVER" in
    ...
    sqlite)
        ...
        ;;
    *)
        echo "Error: unsupported DB_DRIVER '$DB_DRIVER' (expected postgres, mysql, or sqlite)"
        exit 1
        ;;
esac
...
# If DB_URL is already set in .env, use it as-is except for postgres local sslmode.
...
if [ -n "$DB_URL" ]; then
    ...
    case "$DB_DRIVER" in
        postgres)
            ...
            ;;
        mysql)
            DB_URL="mysql://${ENCODED_USER}:${ENCODED_PASSWORD}@tcp(${DB_HOST}:${DB_PORT})/${DB_NAME}?parseTime=true&multiStatements=true&loc=UTC&time_zone=%27%2B00%3A00%27"
            ;;
        sqlite)
...
safe_db_url() {
    case "$DB_DRIVER" in
        postgres)
            ...
            ;;
        mysql)
            echo "mysql://${DB_USER}:***@tcp(${DB_HOST}:${DB_PORT})/${DB_NAME}?parseTime=true&multiStatements=true&loc=UTC&time_zone=%27%2B00%3A00%27"
            ;;
        sqlite)
