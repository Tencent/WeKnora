...
if [ -n "$DB_URL" ]; then
    ...
    case "$DB_DRIVER" in
        postgres)
            DB_URL="postgres://${ENCODED_USER}:${ENCODED_PASSWORD}@${URL_HOST}:${DB_PORT}/${ENCODED_DB_NAME}?sslmode=disable"
            ;;
        mysql)
            DB_URL="mysql://${ENCODED_USER}:${ENCODED_PASSWORD}@tcp(${URL_HOST}:${DB_PORT})/${ENCODED_DB_NAME}?charset=utf8mb4&multiStatements=true&parseTime=true&loc=UTC"
            ;;
        sqlite)
