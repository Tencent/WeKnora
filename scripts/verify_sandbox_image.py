...
def verify(image, profile, backend):
    ...
    # Inspect actual files as well as declarations: office-core must not
    # retain browser binaries or its skill package in a hidden image layer.
    inventory = json.loads(output('docker', 'run', '--rm', '--entrypoint', 'python3', image, '-c',
        ...
        "'browsers': Path('/opt/weknora/browsers').exists()}))"))
    ...
    if inventory['browsers']:
        raise ValueError('office-core must not contain browser binaries')
