...
系统管理员可通过 **设置 -> 系统管理 -> 运维状态**（或
`/platform/system/operations`）查看受保护的运行状态，不会将这些数据暴露到公开路由。
页面打开时每 30 秒刷新一次，也提供手动刷新按钮。页面显示依赖连通性、数据库驱动和
连接池状态、迁移状态、应用文件日志所在磁盘状态，以及定时备份/保留策略状态。

当使用 MySQL、数据库可用且迁移状态干净时，页面会显示 **创建备份**。操作者必须填写
原因并二次确认。该操作调用已有的、仅系统管理员可用的手动备份 API，因此仍会使用同一
个 MySQL advisory lock、manifest/checksum 生成逻辑、错误处理与审计记录。结果只显示
安全的备份标识和相对产物名称。原因会进入审计记录，因此不要填写 password、token 或
业务敏感内容。

此页面不提供恢复、数据库替换、配置回退、镜像回退或删除备份按钮。这些高风险操作仍
只能通过经过审查的 PowerShell CLI 与 break-glass 流程完成。
