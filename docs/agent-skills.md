...
- **网络策略**：每份具名沙箱配置带一块 `network` 策略，作用于该配置下所有沙箱（会话、技能安装、深度检查共用同一份）。默认**出站放行、入站关闭**——入站关闭意味着沙箱公网 URL 必须携带创建时签发的 traffic token，WeKnora 自身的 envd 链路会自动携带（Cube 由 SDK 附加，E2B 由 WeKnora 的数据面 transport 附加）。管理员可配置 allow / deny 列表，Cube 额外支持 CubeEgress L7 规则（scheme / sni / host / method / path + 审计 + header 注入），E2B 额外支持按 host 注入 header。Docker 只能整体开关（`bridge` / `none`）。**改策略只影响之后新建的沙箱**：本期不做运行中热更新。
- **⚠️ 升级行为变更**：入站默认从「公网可达」改为「要求凭证」。如果某个部署依赖浏览器直连沙箱端口（而不是经 WeKnora 代理），需要在该沙箱配置的「网络策略 → 入站访问」里改成「公网可访问」。
