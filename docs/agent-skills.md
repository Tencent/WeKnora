...
启用技能、配置沙箱的智能体，会为有安装权限的用户提供内置 `skill-installer`。
它随服务端分发，无需先安装到沙箱；通过 `read_file(path="skill://skill-installer/SKILL.md")`
按需读取说明，再由 `install_skill` 调用与技能管理相同的安装服务。
例如发送「帮我安装这个技能：<技能来源链接>」，智能体即可提交安装；
工具返回安装 ID，可用 `install_skill(action="status", skill_id="...")` 查询状态。
安装受理不表示安装完成，失败时会返回实际错误。

也可以直接在对话中附加 ZIP，并发送「安装这个技能」；智能体使用附件在沙箱中的路径作为 `archive_path`。
安装进度与结果显示在对话中的工具调用记录里，无需跳转或打开独立安装界面。
已有会话使用其绑定的沙箱配置；尚未绑定的会话使用当前智能体所选配置。
安装结果保存在该配置的技能镜像中，并可在技能管理中查看，供使用同一配置的对话共享。
安装完成后的生效时机遵循沙箱的技能更新策略；仅新会话生效的配置需要新建对话，智能体的技能白名单仍然有效。

对话安装沿用技能管理权限：空间管理员/所有者，或完整访问权限的 API Key。
普通对话不获得安装维护进程的 root Shell；内置技能本身没有可通过 `shell_exec` 执行的脚本。
