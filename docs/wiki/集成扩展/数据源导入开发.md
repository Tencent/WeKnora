...
| 钉钉文档 (DingTalk) | 企业内部应用 Access Token | ✅ | ✅ |
...
1. 在[钉钉开发者后台](https://open-dev.dingtalk.com/fe/app)创建企业内部应用，获取 AppKey /
   AppSecret。
2. 开通并发布 `Wiki.Workspace.Read`、`Wiki.Node.Read`、`Storage.File.Read`。
3. 准备一名有目标知识库访问权的用户 Union ID，作为操作人。
4. 在知识库设置页选择“钉钉文档”，测试连接，按需懒加载并选择空间、文件夹或单篇在线文档。
5. 首版只同步 `FILE / ALIDOC / adoc` 在线文档；普通钉盘文件、附件和其他不支持节点不会作为空文档导入。

钉钉连接器固定访问 `https://api.dingtalk.com`，不接受自定义 Base URL。创建向导通过无状态预览
浏览资源，不创建临时数据源；编辑时，候选配置与凭证会先共同验证，再通过原子重配置接口一次性保存。

连接器在部分失败或节点分类异常时抑制删除。历史非空后的首次完整空快照只告警并保留游标，连续
第二次完整空快照才确认全部删除。错误和 Markdown 输出不会保留凭证、操作人完整标识、Token 或
签名 URL。全量和增量同步都进行删除对账，删除只作用于当前数据源拥有的知识行。

自动化验收使用 mock API；真实企业鉴权与权限验证需要维护者在受控环境提供测试应用，不能用 mock
结果替代。
...
| POST | `/api/v1/datasource/preview-resources` | 使用候选配置无状态验证或懒加载资源 |
| POST | `/api/v1/datasource` | 创建数据源 |
| GET | `/api/v1/datasource?kb_id=xxx` | 列出数据源 |
| PUT | `/api/v1/datasource/:id/reconfigure` | 原子验证并保存配置与凭证 |
| POST | `/api/v1/datasource/:id/resource-ancestors` | 恢复深层资源选择的祖先路径 |
