...
1. 创建钉钉企业内部应用，获取 Client ID / Client Secret
2. 开通知识库和文档读取权限：`Wiki.Workspace.Read`、`Wiki.Node.Read`、`Storage.File.Read`，并发布应用版本
3. 准备一个可读取目标知识库的用户 UnionID（作为 `operatorId`）
4. 在知识库设置页添加“钉钉文档”数据源，测试连接并选择空间、文件夹或单篇文档
5. 触发首次同步并在同步日志中确认结果

当前仅同步钉钉在线文档（`adoc`），不把在线表格、多维表或普通上传文件误当作文档块读取。
