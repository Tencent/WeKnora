...
2. 开通权限：`wiki:wiki:readonly`、`drive:drive:readonly`、`drive:export:readonly`、`docx:document:readonly`；内嵌画板另需 `board:whiteboard:node:read`
...
- **blocks**：图片 / 画板渲染成相对文件名占位符（`![图片](image-{token}.png)`），媒体单独下载成独立知识条目，检索 / Wiki / 智能体无法把图片内容关联回文档；但同步快、保留 docx 内 file block 附件。
