...
# 仅允许白名单出站（默认关闭）：不在白名单里的主机在 DNS 查询之前即被拒绝
# SSRF_DNS_WHITELIST_ONLY=false
```

> 生产环境请谨慎配置。开启 `SSRF_DNS_WHITELIST_ONLY` 前请先把模型、向量库、对象存储、OIDC、MCP 等地址全部列入白名单，详见 [QA 第 8 节](../../QA.md)。搜索引擎的 API 端点硬编码策略参见 [添加网络搜索引擎](../集成扩展/添加网络搜索引擎.md)
