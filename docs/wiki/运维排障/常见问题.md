...
1. 确认 Embedding 与对话模型已在「设置 → 模型」添加（或经初始化接口 / `weknora model create` 注册），且知识库已绑定。本地 Ollama 先运行 `ollama serve`，地址用 `OLLAMA_BASE_URL`（Compose 默认 `http://host.docker.internal:11434`）。`source=local` 的向量与对话模型共用这一个 Ollama。向量模型名是 Embedding 模型的 `name`（如 `nomic-embed-text`）。`INIT_EMBEDDING_MODEL_*` / `INIT_LLM_MODEL_*` 不会被服务端读取；`EMBEDDING_MODEL_NAME` 仅当 `config/builtin_models.yaml` 引用 `${EMBEDDING_MODEL_NAME}` 时生效。详见 [QA.md](../../QA.md) 与 [内置模型管理](../核心功能/内置模型管理.md)。
2. 远程模型在模型记录里填写 `base_url` 与 `api_key`（或在 builtin YAML 中用 `${...}`）。
3. 重排序同样在模型配置里添加，而不是 `INIT_RERANK_MODEL_*`。
