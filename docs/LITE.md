...
| **本地沙盒**  | 提供 `host` 后端：agent 在用户选定的本机目录内读写与执行，越界由操作系统拦截（当前仅 macOS） | 使用远程沙箱（Docker / E2B / Cube） |


若你不需要多团队协作、复杂解析流水线与多服务架构，Lite 更适合个人或小团队在本机零依赖试用；需要共享空间、多空间与完整解析引擎矩阵时，请使用标准版。Lite 的 `host` 后端说明见 [本地沙盒后端](./sandbox-local-backend.md)。