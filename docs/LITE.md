...
直接运行 `WeKnora-lite` 时，程序会自动读取当前目录下的 `.env.lite`，并用 `.env` 补充其中未设置的变量。复制 `.env.lite.example` 为 `.env.lite` 即可开始配置。

如需指定其他文件，可设置 `ENV_FILE` 环境变量。命令行或服务管理器中已经设置的环境变量优先于配置文件。
