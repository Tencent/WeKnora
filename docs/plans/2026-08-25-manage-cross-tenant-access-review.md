...
| Excluded Intermediate Findings | 14（外部 review 问题均已修复） |
| 发布建议 | ✅ 可发 |
...
已从最终态排除 14 个中间问题：授予期间目标并发删除错误映射、审计动作前端注册、中文 workspace 术语、最后双权限管理者事务保护、停用双权限账号残留权限无法撤销、超过 200 个账号无法管理、offset 分页并发漂移、通用 `UpdateUser` 旧快照覆盖平台权限、首个双权限管理者自举缺失、停用账号被计为可用管理者并阻止 bootstrap 恢复、停用账号既有 token 仍可认证、三个新增接口未同步 OpenAPI 生成物、目标查询数据库故障被映射为 404、全量分页重复扫描用户表。它们均已有代码和回归测试修复，因此不计入 findings。
