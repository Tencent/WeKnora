...
| `mysql` | `migrations/mysql` | MySQL 8.0.17+ path. New installs apply baseline `64` and subsequent migration `65`. |
...
schema migration version is `65` with `dirty=0`. Version `64` is the baseline;
new features are added as same-number MySQL migrations instead of editing it.
