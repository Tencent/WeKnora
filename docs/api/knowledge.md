...
- 每次最多 200 个 ID（重复 ID 只下载一次），原始文件实际读取字节合计最多 512 MiB。超限请分批下载。
- 成功返回 `application/zip` 附件；保留文件内容与中文文件名，同名文件自动追加数字，移除不安全路径和 Windows 不支持的文件名字符。
- 手工编写的文档导出为 Markdown。其他条目必须存在原始文件；FAQ、没有原始文件的网页条目不能通过此接口导出。
- 任一条目无权访问、不属于当前知识库、文件缺失或读取失败时，整个请求失败，不返回残缺 ZIP。
- 压缩包在服务器临时目录生成后发送，请求结束或失败后清理临时文件；不包含分块、向量、标签、权限、Wiki 或图谱。

页面操作：进入知识库的文档页 → 点击「批量管理」→ 勾选文件，或「全选已加载」→「批量下载」。全选只包含列表已加载的文档；可先继续加载更多，再选择，单批仍受 200 项限制。ZIP 解压后可将文件批量上传到其他知识库。
