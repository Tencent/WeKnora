...
- 已删除的 `local` 后端曾在 WeKnora 主机上裸跑脚本、没有任何隔离。不要把它和 Lite 桌面版的 `host` 后端当成一回事：`host` 由操作系统强制隔离，路径是真实主机路径，不属于本文的远程沙箱协议。详见 [本地沙盒后端](./sandbox-local-backend.md)。
...
| `local` | 无（本机进程） | 无 | 不支持 | （已移除）本机裸跑，无隔离；不要与 `host` 混淆 |

`docker` 与 E2B 协议后端的分界不在能力而在规模：一个 docker 配置就是一台 daemon，跨主机调度、内核级隔离、内存态快照都不在它的能力范围内，那些正是 E2B 兼容实现提供的东西。已移除的 `local` 不参与会话级能力集。Lite 的 `host` 后端是另一条路径，见下一节。能力矩阵在 `internal/sandbox/capabilities.go` 中显式表达，agent 侧据此决定是否注册 shell/文件类工具。

## 本地沙盒（host）不走 E2B 协议

Lite 桌面版另有一个 `host` 后端，它不属于本文描述的远程沙箱体系：

- **没有协议**。它不是控制面 + envd 的组合，而是在用户本机按策略启动一个被操作系统约束的子进程（macOS Seatbelt，Windows 受限令牌）。
- **路径是真实主机路径**，没有 `/workspace` 虚拟根。用户选定的项目目录就是工作区。
- **与已删除的 `local` 后端无关**。旧的 `local` 是本机裸跑、零隔离；`host` 的约束由内核强制，子进程与孙进程自动继承。
- 不提供交互式终端、图形桌面、快照与 pause/resume —— 这些是远程沙箱的概念。
- 会话 fork **只复制消息**，不回滚、不拷贝本机工作区。项目目录被多个会话共用，reset 会毁掉用户文件。

使用说明见 [本地沙盒后端](./sandbox-local-backend.md)。
