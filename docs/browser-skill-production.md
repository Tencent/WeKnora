...
- 扩展生成 256 位随机设备令牌，仅放在扩展源 IndexedDB；服务器保存 SHA-256 哈希。设备令牌有效期 90 天，30 天后自动轮换。
...
任务使用上游独立 Agent Window。断线清理会关闭任务创建的标签并归还借用标签，保留用户后来加入的页面。当前不承诺恢复之前的页面、DOM 引用、窗口布局或在途命令结果。小预览的「定位到浏览器」仅定位已存在的任务标签，不启动任务、不恢复自动化。这个 UI 操作通过网关传给扩展，复用上游 SessionManager 查找标签；模型不具备该 UI 控制入口。

## 配置

Docker app 镜像已内置匹配目标架构的配套 bsk、扩展和许可证，并预设两个文件路径；单实例默认无需额外环境变量，配对地址从用户当前页面的 origin 生成，保留外部域名和端口。`BROWSERSKILL_PUBLIC_URL` 仅作为独立网关域名/路径等特殊部署的优先覆盖项。原生部署各节点需自行安装产物。配置示例如下：
...
扩展必须更新到 WeKnora 配套 0.3.0 构建。官方发布的 0.3.0 ZIP 早于 `ui.task_preview`/`ui.task_focus` 通道，不能直接替换配套 ZIP。升级前结束任务；在原解压目录覆盖文件后重新加载扩展，可沿用原扩展 ID 并迁移授权凭据。重新安装导致 ID 改变时重新配对。服务端必须同步升级 bsk：从与扩展相同的固定源码提交 `c61eb7e4b1a5785d51b1a316dbf57686fecbf33d` 构建（版本号仍为 0.3.0，需重新构建并更新扩展），然后重启后端以释放旧 daemon。官方发布的 CLI 0.3.0 不包含 daemon 导航响应宽限修复（上游 PR #291 晚于该发布）。
