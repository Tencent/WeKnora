...
- 内置 7 个技能包：xlsx、docx、powerpoint、pdf、exploratory-data-analysis、statistical-analysis、scientific-visualization。
...
当前只构建和发布 `office-core` 办公镜像，预装 Word（docx）、Excel（xlsx）、PDF 和 PowerPoint 四个技能，以及 Python 3.12、Node、LibreOffice、Poppler、中英文字体和锁定依赖。原有基础沙箱镜像继续保留。三个科学分析技能仍在技能发现中提供，可按需安装；不再单独发布对应的办公镜像档位。Cube 使用同一套核心办公内容，另加提供商所需的 envd 和启动入口。
...
国内网络下载 Debian 软件包较慢时，可通过 `APT_MIRROR` 指定镜像站根地址，例如：

```bash
APT_MIRROR=https://mirrors.ustc.edu.cn bash scripts/build_sandbox_office.sh sandbox
```

该参数会同时替换 Debian 和 Debian Security 的服务器地址，保留发行版和签名校验；后续办公镜像构建、技能安装也复用该源。未设置时继续使用上游默认源。它只影响 APT 软件包下载，不改变 Docker 镜像、npm 或 Python 包的下载源。
...
- 成品镜像 Label、运行时清单和实际包目录一致；office-core 不含科学分析技能文件。
- 检查办公镜像提供简繁中文字体。
...
PPT Master 使用 SVG、模板和图标生成演示文稿；Frontend Slides 面向 HTML 演示文稿。两者通过技能发现卡片打开安装抽屉，来源固定到已选定的 commit。
Frontend Slides 的导入地址指向 `plugins/frontend-slides/skills/frontend-slides` 子目录。仓库根目录还有其他 SKILL.md，不能作为单技能包导入。

腾讯 BrowserSkill 依赖用户本机的 CLI、daemon 和浏览器扩展，目录将其标为 `local_browser`。它仅作为外部链接推荐。
