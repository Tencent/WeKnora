...
5. 主对话小预览可定位任务标签、暂停操作、继续操作或结束浏览器任务。暂停中断当前浏览器调用并保留页面，结束会关闭任务新建标签、归还借用标签；这些按钮只控制当前对话的浏览器，不会停止整段对话或撤销浏览器配对。请求人工帮助时先显示提示；不会自动激活标签或窗口。借用已有标签必须经过原生确认，归还时解除控制，保留原位置。

浏览器操作期间，从当前任务页通过新窗口链接（包括 `rel=noopener`）或 `window.open` 打开的标签，会按来源登记到当前任务，加入任务标签组，并在结束时一起回收。若 Chrome 默认激活新标签，扩展会恢复此前查看的标签；浏览器原生激活可能带来短暂切换。任务空闲时手动新建或拖入标签组的页面不会自动获得任务归属。
...
本轮 Agent 执行结束（包括报错、取消）时，通过工具清理钩子释放当前任务的 Chrome 调试连接，保留标签供下一轮继续使用。扩展先阻止新截图，再等待在途截图和操作完成，最后释放连接；结束任务也使用同一截图屏障。空闲时服务端只返回缓存画面，卡片显示「已保留最后画面」，不重复截图，下一次浏览器调用恢复连接。Chrome 提示条的消失可能晚于连接释放；同一扩展有其他活动任务时，提示仍可能保留。
...
BROWSERSKILL_TEST_TASK_WINDOW=1 # 可选：通过扩展界面启用独立窗口并验证其生命周期
```
...
扩展弹窗中的“任务打开方式”提供“后台标签组（默认）”和“独立窗口（保持可见）”。设置保存在本机扩展中，仅对新浏览器任务生效；已有任务需要先结束，或使用新对话。独立窗口开始时会打开到前台，后续创建、选择任务标签只在该窗口内切换；请手动与聊天窗口并排放置。完全遮挡或最小化仍可能导致读取变慢，因此此模式不是不可见后台运行的性能保证。结束任务只清理明确归属于任务的标签，用户后来移入窗口的页面会保留。
