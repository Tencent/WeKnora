...
WeKnora 内置 MySQL 8.0+ 向量检索适配器，驱动名为 `mysql`。该适配器不依赖非社区版向量函数，适合希望复用普通 MySQL 部署的小型到中型知识库：

- **向量检索**：使用 MySQL JSON 数组存储 embedding，并通过标准 JSON 函数计算余弦相似度
- **关键词检索**：使用 MySQL `FULLTEXT INDEX` 进行全文搜索
- **按维度分表**：每个 embedding 维度对应一张物理表 `<prefix>_<dim>`
...
推荐使用 MySQL 8.0+。当前实现采用 JSON-backed brute-force scoring，不需要 `COSINE_DISTANCE`、`DOT_PRODUCT` 或外部插件。该方案便于部署和验证，但不提供 HNSW/ANN 等专用向量数据库索引能力；大规模知识库建议继续使用 Milvus、Qdrant、Weaviate、Doris、Tencent VectorDB 等专用后端。

### 表结构

首次写入时自动建表，表名格式为 `weknora_embeddings_<dim>`（如 `weknora_embeddings_768`）。

```sql
CREATE TABLE IF NOT EXISTS weknora_embeddings_768 (
...
```bash
docker run -d --name weknora-mysql \
  -p 3306:3306 \
  -e MYSQL_ROOT_PASSWORD=weknora \
  -e MYSQL_DATABASE=weknora \
  mysql:8.0
```

首次写入知识库时，WeKnora 会自动创建所需的表。
