...
WeKnora 内置 `mysql` 检索器驱动。可以通过 `RETRIEVE_DRIVER=mysql` 和
`MYSQL_*` 环境变量配置，也可以通过向量库 API 注册，所需字段包括 `addr`、`database`、
可选的 `username`、可选且敏感的 `password`，以及 `collection_prefix`。

MySQL 8 使用 JSON 向量嵌入存储作为兼容回退方案。MySQL 9+ 在服务器支持时可以
使用原生 `VECTOR(N)` 列。只有目标 MySQL 发行版提供兼容的余弦距离函数时，才会启用
数据库侧距离排序，例如 `DISTANCE(..., 'COSINE')` 或
`VECTOR_DISTANCE(..., 'COSINE')`；否则 WeKnora 会读取候选行后回退到 Go 侧余弦排序。

主数据库部署和 PostgreSQL 到 MySQL 迁移步骤见 `docs/mysql.md`。
