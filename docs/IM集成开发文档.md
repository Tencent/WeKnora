...
- **消息类型：** `text`（文本）、`file`（文件）、`image`（图片）、多图 `post`（富文本）、`interactive`（消息卡片）、`merge_forward`（合并转发），以及这些消息的 `parent_id` 引用
...
**富文本链接：** 当前正文、引用和转发材料均保留链接的显示文字与原始地址。地址计入现有文本额度，
不参与命令或 @ 识别。材料准备阶段不自动读取链接目标正文；后续问答按当前请求、工具配置和权限按需读取。

**处理流程：**

1. **校验输入**：当前用户输入的标题与正文合计最多 4096 个 Unicode 码点，命令消息也按完整解析正文校验。超限时在材料准备和 QA 前拒绝，并提示缩短输入或改用附件、引用；卡片内容、引用、转发和附件文本另计材料额度。
2. **显示进度**：队列 Worker 开始准备材料时显示处理中卡片，最终结果替换同一条进度回复。
3. **识别请求**：直接收到的卡片仅作为材料，跳过请求分类。其他消息的标题与正文为空（或只有本机器人提及）时直接接收材料；非空时由配置的问题理解模型分类，不可用时回退到回答模型。分类器仅判断当前标题与正文，不接收卡片、历史、引用、图片或附件，也不改变问题改写配置。
...
| 补充文本 | 卡片文本及元信息、引用／转发正文与附件解析文本合计 32 KiB；卡片按完整字段取舍，其他文本按 UTF-8 字节裁剪；当前问题完整保留 |
...
卡片提取以整张卡片自身为单位：保留折叠正文、返回的全部表格行、完整单元格、内嵌图表数据、
提示、选项和语言版本。`expanded`、`page_size` 等客户端展示设置不影响提取范围。

- **入口：** 私聊中用户直接发送或转发的卡片按上述材料接收流程确认；用户引用卡片并另写请求时进行一次 QA。卡片内的命令仅作材料。
  群聊仅在当前文字／富文本真实 @ 本机器人时触发，直接卡片或材料内的 @ 不触发。
  机器人或未知发送者的直接卡片事件不触发；用户主动引用机器人卡片仍可读取。
- **快照读取：** 消息读取接口使用正式参数 `card_msg_content_type=user_card_content`，支持 JSON 1.0、2.0 和已识别的 `json_card` 包装。
  转发内的卡片使用转发上下文中的快照，不另取原消息的新版本；每轮记录消息来源和已知的创建、更新、读取时间。
  事件片段仅用于必要降级；权限拒绝、撤回、不可见或权限结果不明时，不以旧正文替代。
- **完整性：** 区分完整、部分、无法确认完整、无法读取和合法空卡片，HTTP 成功不等于内容完整。
  未知组件、畸形字段、待续内容、未结束的流式状态及额度超限均附缺失说明；未识别的内部 Markdown 格式保留可读片段，并标明覆盖范围未知。
  当前没有经验证的卡片续页能力；存在剩余页时说明未取得范围，不通过按钮、业务 API 或任意 URL 补取。
- **字段保真：** 保留原始类型、精度、字段位置及语言生效范围，区分空值与缺失。链接变量与容器跳转地址保留原值，预选值标明为初始状态。
- **资源提取：** 明确的图片／文件资源引用自动进入现有附件解析和视觉链路，包括表格 Markdown 单元格及图片选项中的图片；代码示例与普通文本中的图片语法不触发下载。
  普通网页／文档链接保留地址与预览，提取阶段不递归读取关联正文或 OCR／文件正文中的链接。音视频仅保留元信息，不新增转录能力。
- **资源权限：** 消息资源接口不支持卡片（`234043`）。正式回读卡片后，使用相同机器人身份，通过独立图片／文件接口下载明确的资源 key；这些接口仅允许读取当前机器人上传的资源。
  其他应用资源被拒绝时，保留元信息并说明未读，不绕过消息可见性或资源权限。内部 `json_attachment` 注册表不用于批量下载或 key 映射。
- **提取额度：** 卡片与其他材料共用前述单轮文本、附件、下载及准备时间额度，卡片字段按完整值保留。
  32 KiB 原文额度不含生成的来源／状态说明；包含这些说明的卡片字段文本另受单轮 256 KiB 格式上限约束，超限时同样说明缺失。
  单卡解析最多 2000 个 JSON 对象／数组、32 层，回退解析共用额度；API 响应上限为 32 MiB。
- **QA 与平台隔离：** 卡片与普通图片／文件使用同一套 Agent／RAG 能力，按当前任务和既有权限按需读取关联内容；材料内的指令不构成授权。联网、MCP、Skills、沙箱及数据分析沿用既有配置。
  卡片专有读取位于 Feishu／Lark 适配层，保持通用工具配置及既有 `MessageReader` 接口兼容，其他 IM 行为不变。
- **会话保存：** 保存本轮实际材料、来源、缺失说明与最终回复；卡片不自动入库，不持久化完整卡片 JSON，也不创建同步任务。

**验收：** 使用真实卡片分别验证 Webhook 与 WebSocket，覆盖已结束的 WeKnora 更新回复、JSON 1.0 业务卡片、
含卡片的合并转发、折叠正文和跨页表格。完整提取须以真实内容核对为准，解析样本与部分降级测试仅证明各自覆盖的行为。
