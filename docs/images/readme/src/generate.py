...
SANS = "Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC', 'Hiragino Sans GB', 'Noto Sans CJK SC', 'Microsoft YaHei', sans-serif"
SERIF = "'Iowan Old Style', 'Palatino Linotype', Palatino, 'Book Antiqua', Georgia, 'Songti SC', 'Source Han Serif SC', 'Noto Serif CJK SC', STSong, serif"
...
def wrap(s, size, width):
    """Greedy wrap: CJK breaks anywhere, latin breaks at spaces."""
    tokens, buf = [], ""
    for ch in s:
        if ord(ch) > 0x2E80:
            if buf:
...
def hero(lang, theme):
    ...
    o += [f'<text x="72" y="208" font-family="{MONO}" font-size="20" letter-spacing="3" fill="{t["gold"]}">{c["eyebrow"]}</text>',
          f'<text x="68" y="{208+c["h1_size"]+22}" font-family="{SERIF}" font-size="{c["h1_size"]}" fill="{t["ink"]}">{esc(c["h1"][0])}</text>',
          f'<text x="68" y="{208+2*c["h1_size"]+44}" font-family="{SERIF}" font-size="{c["h1_size"]}" fill="{t["gold"]}">{esc(c["h1"][1])}</text>']
    for i, line in enumerate(c["desc"]):
        o.append(f'<text x="72" y="{desc_y+i*42}" font-family="{SANS}" font-size="27" fill="{t["soft"]}">{esc(line)}</text>')
    ...
    for i, (ic, a, b) in enumerate(c["modes"]):
        ...
        o += [f'<rect x="{mx}" y="{y}" width="312" height="76" rx="14" fill="{t["card"]}" stroke="{t["rule"]}" stroke-width="2"/>',
              ...
              f'<text x="{mx+82}" y="{y+48}" font-family="{SANS}" font-size="25" font-weight="600" fill="{t["ink"]}">{esc(a)} <tspan font-weight="400" fill="{t["soft"]}">{esc(b)}</tspan></text>']
    # trust bar
    ...
    for i, (ic, label) in enumerate(c["trust"]):
        ...
        o += [icon(ic, x, H - 70, 26, t["mute"], 1.6),
              f'<text x="{x+40}" y="{H-49}" font-family="{SANS}" font-size="22" fill="{t["soft"]}">{esc(label)}</text>']
    o.append("</svg>")
    return "\n".join(o)
...
def cards(lang, theme):
    ...
    for i, (ic, label, tl, dl, rows) in enumerate(layouts):
        ...
        y = pad + 56 + 56
        for line in tl:
            o.append(f'<text x="{x+pad}" y="{y}" font-family="{SANS}" font-size="{title_size}" font-weight="600" fill="{t["ink"]}">{esc(line)}</text>')
            y += 44
        y += 4
        for line in dl:
            o.append(f'<text x="{x+pad}" y="{y}" font-family="{SANS}" font-size="{desc_size}" fill="{t["soft"]}">{esc(line)}</text>')
            y += 40
        ty = H - pad - tag_h + 6
        for row in rows:
            tx = x + pad
            for tag, w in row:
                o += [f'<rect x="{tx}" y="{ty}" width="{w}" height="38" rx="7" fill="{t["chip_bg"]}" stroke="{t["chip"]}" stroke-width="1.5"/>',
                      f'<text x="{tx+16}" y="{ty+26}" font-family="{SANS}" font-size="20" fill="{t["accent"]}">{esc(tag)}</text>']
                tx += w + 10
            ty += 50
    o.append("</svg>")
    return "\n".join(o)
