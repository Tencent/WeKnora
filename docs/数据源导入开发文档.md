...
- **blocks 模式（默认）**：走 blocks API
  ...
  2. blocksToMarkdown -> 转换为 Markdown 正文
     ...
     ├─ image/画板 block -> weknora-img://<序号> 占位符（图片单独下载为独立知识条目）
     └─ file block -> 附件，作为独立知识条目
  3. 文档处理管线把 weknora-img:// 占位符解析为 WeKnora 持久图片地址写回正文
     （图片条目与父文档通过 metadata image_map / parent_doc_id 双向关联）
...
| 图片与文档关联 | ✅ 图片以 WeKnora 持久图片地址内联回父文档，图片同时作为独立知识条目（metadata 父子双向关联） | ✅ 图片 inline 进父文档，`parent_chunk_id` 关联 |
| 同步速度 | 快 | 慢（异步导出 + docx 解析） |
| docx 内附件（file block） | 保留 | 丢失（.docx 导出不含） |
| 所需权限 | 需 `docx:document:readonly` | 不需要（但建议保留以便切换） |

- **blocks**：快、保留附件；图片以 weknora-img:// 序号占位后由文档处理管线解析为 WeKnora 持久图片地址内联回正文，图片条目与父文档 metadata 双向关联；图片 OCR/caption 依赖多模态配置。
- **export**：图片 inline 进父文档（同普通 docx 上传），通过 `parent_chunk_id` 建立同知识条目父子关联，三个场景都能关联图片内容；代价是同步慢、docx 内附件丢失、图片 OCR/caption 依赖多模态配置。

> 解析模式在数据源编辑页的「解析模式」中选择（默认 blocks）；存量数据源在下次同步时自动按 blocks 补齐配置并全量升级一次。
