...
Cron 触发时:
    ...
    └─ 入队 asynq 任务
          队列: low
          TaskID: dssync:<dsID>:<syncLogID>（每次运行独立身份）
```

### 任务去重（两层保护）

1. **DB 层**：`HasRunningSync` 检查该数据源是否已有运行中的同步任务，防止同步耗时超过调度间隔时产生重叠
2. **触发层**：短租约 token-owned 锁 `weknora:datasource:{<id>}:trigger-lock` 串行化“检查 pending/running → 创建 SyncLog → 入队”；不依赖 Asynq 完成/归档任务的 ID 保留语义
3. **队列层**：每次运行使用独立 `TaskID`（格式 `dssync:<dataSourceID>:<syncLogID>`），归档任务不会阻塞以后正常同步
4. **执行层**：Worker 使用可续租的 token-owned Redis 锁 `weknora:datasource:{<id>}:sync-lock`；锁丢失会取消任务上下文，并让任务返回错误而不是被队列 Ack
