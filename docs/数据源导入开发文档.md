...
- [连接器详解](#连接器详解)
  - [飞书 (Feishu)](#飞书-feishu)
  - [飞书链接列表](#飞书链接列表-feishu_links--lark_links)
- [定时调度](#定时调度)
- [关键参数与阈值](#关键参数与阈值)
- [错误处理](#错误处理)
- [扩展新连接器](#扩展新连接器)
...
不枚举知识库空间、不遍历云盘文件夹，只同步用户粘贴的**单篇文档 URL**。机器人对该篇有可阅读或可编辑即可，不必把应用加成知识库成员或云盘文件夹协作者。

1. 创建飞书自建应用，开通 `wiki:wiki:readonly`、`drive:drive.metadata:readonly`（或 `drive:drive`）、`drive:drive:readonly`、`drive:export:readonly`、`docx:document:readonly`。仅有 `drive:drive:readonly` 时 `/wiki/` 能解析，`/docx/` 等云文档链接会因缺少 metas 权限失败。
2. 打开目标文档 → 分享 → 添加协作者 → 搜索并添加你的应用，权限选「可阅读」。
3. 在 WeKnora 选择「飞书链接列表」，粘贴 `/wiki/{token}`（知识库 copylink）或 `/docx/`、`/docs/`、`/sheets/`、`/base/`、`/file/` 链接，点「解析链接」。
4. 整库请用「飞书」数据源；整夹请用「飞书云盘」。`/wiki/space/` 与 `/drive/folder/` 会被拒绝。

Lark 步骤相同，开放平台换成 <https://open.larksuite.com/>，连接器选「Lark 链接列表」。

详见 [飞书链接列表](#飞书链接列表-feishu_links--lark_links)。

---

### 钉钉文档接入

1. 在钉钉开放平台创建并发布企业内部应用。
2. 为应用开通 `Wiki.Workspace.Read`、`Wiki.Node.Read`、`Storage.File.Read` 权限。
3. 添加“钉钉文档”数据源，填写 Client ID、Client Secret，以及有权访问目标知识库的操作人 Union ID。
4. 测试连接后，可按知识库空间、文件夹或单篇文档选择同步范围。

连接器通过钉钉公开 OpenAPI 读取知识库节点及文档 Block，并转换为 Markdown；当前只同步
`ALIDOC/adoc` 在线文档。增量同步优先用节点的毫秒 `modifiedTimestamp` 作为版本，避免官方示例中
分钟精度 `modifiedTime`（如 `2023-05-15T11:29Z`）导致同一分钟内的编辑被跳过。目录遍历不完整或
单篇文档读取失败时会保留原游标，避免误删或跳过后续重试。实现不依赖灰度权限、异步导出或回调服务。

钉钉 Blocks 查询接口只返回文档根下的一级块。高亮块等容器若响应内联了 `children` 会继续渲染；
若容器没有子块，连接器会在文档元数据中记录 `nested_blocks_unavailable`，而不是把残缺正文当成
完整导入。列表项若是块级 children 也会尝试展开。

测试连接除列出知识库外，还会读取根节点，并在根下存在在线文档时试读 Blocks，以便在保存前发现
缺少 `Wiki.Node.Read` 或 `Storage.File.Read`。全量同步（含手动 ForceFull）会对照上次游标对账删除。

同步以所有已选范围的可见文档集合统一判断删除，文档在已选文件夹之间移动不会触发删除。
某个范围不可访问或已选节点失效时，其他正常范围继续同步，失败范围保留旧游标；在所有范围
恢复可完整读取之前，暂缓删除并保留待核对记录。正文抓取失败的文档也会在后续同步重试。
失效的单篇文档或文件夹选择需要检查权限，必要时重新选择资源，不能直接将不可见等同于源端删除。
开启“同步删除”后，确认源端删除的文档会从 WeKnora 知识库移除；关闭该开关则保留本地内容。
钉钉配置和同步失败提示覆盖中文、英语、日语、韩语、俄语。
...
| `internal/datasource/connector/feishu/links/connector.go` | 飞书链接列表 Connector：单篇 URL 解析与同步 |
| `internal/datasource/connector/feishu/core/url.go` | 飞书文档 URL 解析与去重 |
| `internal/datasource/connector/feishu/{core,wiki,drive,links}/*_test.go` | 单元测试：使用 HTTP Mock 模拟飞书开放平台 |

### 飞书链接列表 (feishu_links / lark_links)

按用户粘贴的**单篇文档 URL**同步，不枚举知识库空间、不遍历云盘文件夹。机器人对该篇有可阅读或可编辑即可。

**支持的 URL：**

- `/wiki/{node_token}` 知识库某一篇（如 `https://xxx.feishu.cn/wiki/TOKEN?from=from_copylink`）。只拉这一篇，不递归子页。
- `/docx/{token}`、`/docs/{token}`、`/sheets/{token}`、`/base/{token}`、`/file/{token}`

**拒绝（其它链接继续解析）：** `/wiki/space/`（整库，请用飞书知识库）、`/drive/folder/`（整夹，请用飞书云盘）、mindnote/slides。

**解析与去重：** 解析后用 `get_node`（wiki）或 `POST /drive/v1/metas/batch_query`（云文档）映射成文档标题。先按规范化 URL 去重，再按 `{obj_type}:{obj_token}` 去重（同一篇的 wiki copylink 与 `/docx/` 会合并）。

**内容获取：** docx 仍走 `FetchDocxWithBlocks`，由 `FEISHU_DOCX_PARSE_MODE`（`export` / `blocks`）控制，与知识库、云盘相同。

**权限：** `wiki:wiki:readonly`（`/wiki/` copylink 走 `get_node`）、`drive:drive.metadata:readonly` 或 `drive:drive`（`/docx/`、`/docs/`、`/sheets/`、`/base/`、`/file/` 走 `metas/batch_query`；`drive:drive:readonly` **不能替代**）、`drive:drive:readonly`、`drive:export:readonly`、`docx:document:readonly`。开通后须创建版本并发布。把应用加为该文档协作者即可，不必进群分享整库/整夹。
