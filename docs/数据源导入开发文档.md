...
钉钉连接器使用企业内部应用读取知识库目录，并同步钉钉在线文档。首版支持知识库空间、文件夹和
`FILE / ALIDOC / adoc` 在线文档；普通钉盘文件、二进制附件、表格及其他无法通过文档 Block API
可靠读取的节点会明确标为不支持，不会生成空文档。

#### 第一步：创建企业内部应用

1. 打开[钉钉开发者后台](https://open-dev.dingtalk.com/fe/app)，创建企业内部应用。
2. 记录应用的 **AppKey** 与 **AppSecret**。
3. 选择一名可访问目标知识库的用户，记录该用户的 **Union ID** 作为操作人。资源可见范围遵循
   该用户权限。
...
| 权限 | 用途 |
|------|------|
| `Wiki.Workspace.Read` | 读取知识库空间 |
| `Wiki.Node.Read` | 读取知识库目录和节点详情 |
| `Storage.File.Read` | 读取在线文档内容 |

权限变更后需要发布应用版本。可对照钉钉官方的
[获取知识库列表](https://open.dingtalk.com/document/development/get-knowledge-base-list)、
[获取节点列表](https://open.dingtalk.com/document/orgapp/get-node-list)和
[获取节点详情](https://open.dingtalk.com/document/orgapp/get-node-detail)文档核对。

#### 第三步：在 WeKnora 中配置

1. 在知识库设置的 **数据源** 页选择 **钉钉文档**。
2. 填写 AppKey、AppSecret 和操作人 Union ID。连接器固定访问
   `https://api.dingtalk.com`；不接受可配置 Base URL，以免凭证被发送到非官方主机。
3. 测试连接后，按需展开知识库和文件夹，选择空间、文件夹或单篇在线文档。
4. 保存并触发首次同步。

凭证由后端加密保存且不会回显。创建向导使用无状态资源预览，不会先创建临时数据源；最终保存只创建
一次。编辑时，未选择“替换凭证”会继续使用后端已保存的密钥；显式替换则由原子重配置接口先共同
校验候选配置和凭证，再一次性持久化，验证失败不会留下半更新状态。错误信息会移除 AppKey、
AppSecret、Union ID、访问 Token、私有路径、查询参数和带签名的 URL。

#### 增量、删除与内容安全

- 连接器以节点修订时间构建快照，重叠选择会去重，目录循环和异常深度会被限制。
- 目录部分失败、文档读取失败或已同步节点的类型发生漂移时，本轮标为部分成功，并抑制删除。
- 历史非空而本轮首次得到完整空快照时，保留旧游标并提示异常；只有连续第二次得到完整空快照才确认
  “全部已删除”。单篇正常删除在完整非空扫描中仍会立即传播。
- 全量和增量模式都使用可恢复游标完成删除对账；物理删除只作用于当前数据源同时匹配
  `external_id` 与 `datasource_id` 的知识行，不会删除其他数据源拥有的同名条目。
- Block 内容转换为 Markdown 时保留标题、列表、表格、引用和内联样式，但不持久化私有链接或图片
  的签名 URL。

自动化测试使用本地 mock API 与合成凭证，不连接真实企业。真实钉钉企业的鉴权、权限和内容同步
验证需要由维护者在受控环境提供测试应用后执行；不得把 mock 结果写成线上验证。
...
| 钉钉文档 (dingtalk) | `app_key`, `app_secret`, `operator_id` | 企业内部应用 Access Token |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/v1/datasource/preview-resources` | 使用候选凭证/设置验证连接或懒加载资源，不创建或修改数据源 |
| PUT | `/api/v1/datasource/:id/reconfigure` | 共同验证候选配置与凭证，并在无运行中同步时原子持久化 |
| POST | `/api/v1/datasource/:id/resource-ancestors` | 恢复编辑态深层资源选择的祖先展开路径 |

预览请求可用 `validate_only=true` 只测试连接；编辑态可传已有数据源 ID 以复用该空间拥有的已保存
凭证。后端会校验空间归属、连接器类型和运行中同步状态，避免跨空间读取或在同步期间切换外部身份。
...
FetchedItem
    │
    ├─ IsDeleted = true?
    │     └─ 按 external_id + datasource_id 查找当前数据源拥有的知识 → 物理删除
    │
    ├─ 有文件内容 (Content)?
    │     └─ CreateKnowledgeFromFile
    │        · 按 external_id + datasource_id 查找已有知识
    │        · 已存在 → 先创建替换项并确认解析任务已入队 → 再删除旧版本
    │        · 不存在 → 新建
...
立即创建、存储或入队失败时会清理失败替换项并保留旧版本；旧版本删除失败时也会尽力回滚新项。
解析 Worker 在成功入队后异步运行，因此其后续失败仍由知识解析状态和重试机制处理，不应宣称整个
替换流程具有跨异步任务的数据库事务原子性。
...
钉钉连接器调用 `https://api.dingtalk.com` 的新版 OpenAPI：

| 能力 | 端点 | 关键约束 |
|------|------|---------|
| 获取 Access Token | `POST /v1.0/oauth2/accessToken` | AppKey / AppSecret；Token 响应缓存 |
| 列知识库 | `GET /v2.0/wiki/workspaces` | 必传 `operatorId`；`maxResults` 最大 30；按 `nextToken` 翻页 |
| 列子节点 | `GET /v2.0/wiki/nodes` | 必传 `parentNodeId`、`operatorId`；`maxResults` 最大 50；按 `nextToken` 翻页 |
| 节点详情 | `GET /v2.0/wiki/nodes/{nodeId}` | 用于核对节点分类和所属知识库 |
| 文档 Block | `GET /v1.0/doc/suites/documents/{docKey}/blocks` | 必传 `operatorId`；用含首尾的 `startIndex/endIndex` 每页读取 100 个；响应为 `success/result/data` |

资源选择器使用懒加载。节点资源 ID 是携带知识库和祖先路径的有界不透明值，因此编辑深层选择时无需
遍历整个空间即可恢复展开路径；编码内容会校验长度、重复节点和跨空间归属。

在线文档 Block 递归转换为 Markdown，兼容 `elementType` 与旧 `inlineType` 形态。链接和图片只保留
可读标签，不写入可能带权限或签名参数的 URL。

源码和测试位于 `internal/datasource/connector/dingtalk/`。Mock 测试覆盖认证缓存、401 重取、
429/5xx 重试、同源重定向约束、官方分页和响应 envelope、直接文档选择、祖先恢复、重叠与循环
去重、全量/增量删除保护、错误脱敏及 Block 转换。真实企业测试仍是受控环境的外部门禁。
...
| 钉钉知识库分页大小 | 30 | `workspaces` 接口的 `maxResults` 上限 |
| 钉钉节点分页大小 | 50 | `nodes` 接口的 `maxResults` 上限 |
| 钉钉目录最大深度 | 32 | 防止异常目录或循环占用同步 Worker |
