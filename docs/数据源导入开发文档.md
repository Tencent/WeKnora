...
> **mention 渲染增强权限（可选）**
>
> 以下权限只影响 docx 正文中提及块（mention）的显示效果，缺失时同步不失败，仅降级展示：
>
> - `contact:user.base:readonly` — @成员 显示真实姓名（经 contacts API 反查；缺失时降级为 `@成员`）
> - `drive:drive.metadata:readonly` — 云文档引用显示文档标题（经 `POST /drive/v1/metas/batch_query` 批量回填；缺失、无权限（970003）或文档已删（970005）时回退为 `[URL](URL)` 形式）
> - `board:whiteboard:node:read` — 画板（含思维导图）导出为图片内嵌正文；缺失时降级为占位标记
...
|------------|------|---------|---------|
| `docx` | 是 | blocks API 转 Markdown（默认）/ 导出 .docx（export 模式） | Markdown / `.docx` |
...
**docx（新版云文档）：** 解析路径由数据源级配置 `parse_mode` 控制（数据源编辑页 → 解析模式；默认 `blocks`，可选 `export`。作用于该数据源，飞书知识库和飞书云盘连接器同源同效）。

- **blocks 模式（默认）**：走 blocks API
  ```
  1. GET /open-apis/docx/v1/documents/{obj_token}/blocks (分页 500)
  2. blocksToMarkdown -> 转换为 Markdown 正文
     ├─ 文本/标题/列表/表格/代码块 -> Markdown
     ├─ mention_user -> @真实姓名（contacts API 反查，无权限降级 @成员）
     ├─ mention_doc（云文档链接）-> 标题回填（drive 元数据批量接口，失败回退 URL）
     ├─ 时间轴等文档小组件块 -> 占位注记（无公开读取 API）
     ├─ image/画板 block -> 同步时下载图片字节，以 base64 data URI 内联进正文
     └─ file block -> 附件，作为独立知识条目
  3. 文档处理管线的图片解析器（与解析 PDF/Word 内嵌图同构）把 data URI 存入文件服务、
     换成 WeKnora 持久图片地址；图片不产生独立知识条目
  4. blocks API 失败或渲染空 -> 回退 export
  ```
- **export 模式**：走异步导出 API
  ```
...
| | blocks（默认） | export |
|---|---|---|
| 解析路径 | blocks API -> Markdown | 导出 .docx -> docreader |
| 图片与文档关联 | ✅ 图片内联在父文档正文中（持久图片地址），不产生独立知识条目 | ✅ 图片 inline 进父文档，`parent_chunk_id` 关联 |
| 同步速度 | 快 | 慢（异步导出 + docx 解析） |
| docx 内附件（file block） | 保留 | 丢失（.docx 导出不含） |
| 所需权限 | 需 `docx:document:readonly` | 不需要（但建议保留以便切换） |

- **blocks**：快、保留附件；图片在同步时内联进正文，由文档处理管线存储为 WeKnora 持久图片地址（不产生独立图片条目）；图片 OCR/caption 依赖多模态配置。
- **export**：图片 inline 进父文档（同普通 docx 上传），通过 `parent_chunk_id` 建立同知识条目父子关联，三个场景都能关联图片内容；代价是同步慢、docx 内附件丢失、图片 OCR/caption 依赖多模态配置。

> 解析模式在数据源编辑页的「解析模式」中选择（默认 blocks）；存量飞书数据源经迁移脚本打一次性标记，下次同步自动按默认 blocks 模式全量升级一次（settings.parse_mode 不回写，可在编辑页手动确认）。
...
| `internal/datasource/connector/feishu/core/board.go` | 画板 (block 43) download_as_image 导出 |
