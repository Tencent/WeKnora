...
从钉钉知识库同步文档到 WeKnora。支持按知识库 / 文件夹选择资源，将文档块内容转为 Markdown 入库。

#### 第一步：创建钉钉应用

1. 登录 [钉钉开放平台](https://open-dev.dingtalk.com/) → **应用开发** → **创建企业内部应用**
2. 在 **凭证与基础信息** 页获取：
   - **AppKey**（Client ID）
...
| 权限 | 说明 |
|------|------|
| `Document.Workspace.Read` | 读取知识库列表 |
| `Document.WorkspaceDocument.Read` | 读取文档内容（blocks） |
| `Wiki.Node.Read` | 读取知识库目录节点 |

#### 第三步：准备操作人 unionId

钉钉 Wiki / 文档接口需要 `operatorId`（用户 **unionId**，不是 userId）做权限校验。可从通讯录、免登、消息回调或个人信息页获取。

#### 第四步：在 WeKnora 中添加数据源

1. 进入知识库设置页 → **数据源** 标签页
2. 点击 **添加数据源** → 选择 **钉钉文档**
3. 填写 **App Key**、**App Secret**、**操作人 unionId**
4. 点击 **测试连接** → 选择要同步的知识库 / 文件夹 → 配置同步策略并保存
...
| 钉钉 (dingtalk) | `app_key`, `app_secret`, `operator_id` | OAuth2 (App Access Token) + 操作人 unionId |
| 语雀 (yuque) | `api_token`, `base_url?` | Personal Token (`X-Auth-Token`) |
| Notion (notion) | `api_key` | Integration Token |
| RSS (rss) | `auth_headers?`（可选） | 自定义请求头 |
...
钉钉连接器支持从钉钉知识库（Wiki / 文档）同步文档到 WeKnora，将块结构转换为 Markdown 后入库。

#### 快速接入

1. 登录 [钉钉开放平台](https://open-dev.dingtalk.com/) → **应用开发** → 创建**企业内部应用**
2. 在 **凭证与基础信息** 获取 **AppKey**、**AppSecret**
3. 在 **权限管理** 中开通并发布至少以下权限：
   - `Document.Workspace.Read`
   - `Document.WorkspaceDocument.Read`
   - `Wiki.Node.Read`
4. 准备一个有权访问目标知识库的用户 **unionId**（不是 userId）作为 `operator_id`
5. 在 WeKnora 知识库设置 → **数据源** → **添加数据源** → 选择 **钉钉文档**，填写上述凭证并测试连接
6. 选择要同步的知识库（可展开选择文件夹/文档），配置同步策略后保存
...
AppKey + AppSecret
  ...
  POST /v1.0/oauth2/accessToken
        │
        ▼
  获取 Access Token（有效期约 2 小时）
        │
        ▼
  请求头 x-acs-dingtalk-access-token + query operatorId=unionId
```

- **认证类型**：企业内部应用 Access Token
- **操作人**：所有 Wiki / 文档接口需要 `operatorId`（unionId）做权限校验
- **API 地址**：默认 `https://api.dingtalk.com`（可通过 `base_url` 覆盖）
...
| parentID | 行为 |
|----------|------|
| 空 | `GET /v2.0/wiki/workspaces` 列出知识库 |
| `workspaceId` | 解析 `rootNodeId` 后 `GET /v2.0/wiki/nodes?parentNodeId=` 列出顶层节点 |
| `workspaceId:nodeId` | 列出该节点直接子节点 |

资源 ID 编码：

- 知识库：`{workspaceId}`
- 节点：`{workspaceId}:{nodeId}`

#### 同步行为

- **全量**：对选中资源递归遍历节点树，对 `FILE` 节点调用 `GET /v1.0/doc/suites/documents/{docKey}/blocks`，将 heading/paragraph/list/table 等块转为 Markdown
- **增量**：对比游标中每个节点的 `modifiedTime`，未变则跳过；上次存在本次消失的节点标记 `IsDeleted`
- **跳过**：`FOLDER` 类型只用于遍历，不作为文档内容入库

#### 源码文件

| 文件 | 职责 |
|------|------|
| `internal/datasource/connector/dingtalk/types.go` | 钉钉 API 类型、配置解析、Markdown 转换 |
| `internal/datasource/connector/dingtalk/client.go` | API 客户端：Token、Workspace/Node/Blocks |
| `internal/datasource/connector/dingtalk/connector.go` | Connector 接口实现 |
| `internal/datasource/connector/dingtalk/connector_test.go` | 单元测试（HTTP Mock） |
