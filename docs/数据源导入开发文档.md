...
从钉钉知识库同步在线文档（`FILE + ALIDOC + adoc`）到 WeKnora。资源选择支持知识库空间、文件夹和单篇文档；表格（`axls`）、多维表（`able`）及普通上传文件不由此连接器处理。

#### 第一步：创建钉钉内部应用

1. 登录 [钉钉开放平台](https://open-dev.dingtalk.com/) 并创建企业内部应用
2. 在应用凭证页获取 **Client ID（AppKey）** 和 **Client Secret（AppSecret）**
3. 创建并发布一个包含所需权限的应用版本

#### 第二步：开通读取权限

在权限管理中开通知识库空间、节点和文件内容的只读权限。当前连接器使用的权限标识为：

- `Wiki.Workspace.Read`
- `Wiki.Node.Read`
- `Storage.File.Read`

> 不同钉钉开放平台版本或应用类型的权限展示名称可能略有差异，请以当前控制台中对应 API 的权限说明为准。

#### 第三步：准备操作人 UnionID

钉钉 Wiki 和文档 API 要求传入 `operatorId`。请使用一个**对目标知识库空间和文档具有读取权限的用户 UnionID**。应用权限与操作人资源权限缺一不可。

#### 第四步：在 WeKnora 中添加数据源

1. 进入知识库设置页 → **数据源** → **添加数据源** → **钉钉文档**
2. 填写 Client ID、Client Secret 和操作人 UnionID
3. 点击 **测试连接**
4. 按需选择整个知识库空间、文件夹或单篇文档
5. 配置同步频率和增量/全量同步策略后保存

增量同步按节点 `modifiedTime` 判断变更。只有当资源树遍历完整时才会产生删除标记；分支读取失败时会保留旧游标，避免把暂时不可见的文档误判为删除。
