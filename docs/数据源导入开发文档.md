...
Xquik 连接器把公开的 X 帖子同步到 WeKnora 知识库。每个保存的搜索查询都是一个可选资源，适合持续收集产品反馈、行业动态和公开讨论。

#### 前置条件

- 一个有效的 [Xquik API 密钥](https://docs.xquik.com/api-reference/authentication)
- 至少一个 [X 搜索查询](https://docs.xquik.com/api-reference/x/tweets/search)
...
2. 点击 **添加数据源**，选择 **Xquik**。
3. 输入 Xquik API 密钥。
4. 每行输入一个 X 搜索查询，最多 20 个。
5. 设置每个查询的结果数，范围为 1 到 1,000。
6. 测试连接，然后选择要同步的查询。
7. 配置同步频率和同步模式，再保存数据源。

连接器按时间顺序读取搜索结果。增量同步会从上次成功时间前 5 分钟开始，避免边界时间内的帖子遗漏。一次同步使用固定的结束时间，分页期间新发布的帖子会留到下次同步。

每条帖子保存为 Markdown 文档，并包含公开帖子链接、作者、发布时间、查询和互动计数。API 密钥作为数据源凭证加密保存；搜索查询和结果数作为普通设置保存。
