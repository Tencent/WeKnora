...
1. 创建飞书自建应用，先按[飞书知识库接入](#飞书知识库接入)开通 `wiki:wiki:readonly`、`drive:drive:readonly`、`drive:export:readonly`、`docx:document:readonly`。若要解析 `/docx/`、`/docs/`、`/sheets/`、`/base/`、`/file/` 等云文档链接，再额外开通 `drive:drive.metadata:readonly`（或 `drive:drive`）；只开 `drive:drive:readonly` 不够。
...
**权限：** 与飞书知识库相同的四项即可解析 `/wiki/` copylink。解析 `/docx/`、`/docs/`、`/sheets/`、`/base/`、`/file/` 时需**额外**开通 `drive:drive.metadata:readonly`（或 `drive:drive`），`drive:drive:readonly` 不能替代。开通后须创建版本并发布。把应用加为该文档协作者即可，不必进群分享整库/整夹。
