...
> - `board:whiteboard:node:read` — 下载内嵌画板缩略图（`block_type=43`）；缺失时该画板降级跳过，不拖垮整篇
...
- **blocks 模式（`FEISHU_DOCX_PARSE_MODE=blocks`）**：走 blocks API
  ...
  2. blocksToMarkdown -> 转换为 Markdown 正文
     ...
     ├─ image block -> ![图片](image-{token}.png)（图片单独下载为独立知识条目）
     ├─ board block (43) -> ![画板](board-{id}.png)（download_as_image + 裁白边）
