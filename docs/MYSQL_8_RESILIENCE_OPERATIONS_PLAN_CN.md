...
| 已有能力 | 现状 | 缺口 |
| --- | --- | --- |
| HTTP 健康检查 | 已有 `GET /health`，Compose 以它检查 app 容器 | 无法区分进程存活和依赖可用，也没有告警出口 |
| 容器重启 | MySQL Compose 的各服务已使用 `restart: unless-stopped` | Docker 健康检查失败不会自动修复业务依赖，也没有启动后验收 |
| 文件日志轮转 | `LOG_PATH` 启用时使用 Lumberjack，固定为 50 MB、保留 3 份、28 天、压缩 | 参数不可配置；Docker stdout/stderr 日志仍可能无限增长 |
| MySQL 迁移 | 全新 MySQL 8 可创建 50 张业务表，迁移版本为 74 | 没有数据库备份、恢复或发布前恢复点 |
| 任务队列观测 | 项目已有运行时队列面板与任务失败信息 | 缺少基础设施级健康、磁盘、备份和告警闭环 |
...
flowchart LR
    ...
    Monitor --> Alert[Email or webhook alert]
    Logger --> DockerLog[Docker local log rotation]
    Backup --> BackupStore[Encrypted backup destination]
...
操作者可以选择是否开启备份，默认关闭定时任务：

1. 手动备份：系统管理员从管理接口或 CLI 创建，需填写原因。
2. 定时备份：显式设置 Cron 表达式后才启用。
3. 发布前备份：执行高风险发布、数据库恢复或破坏性操作前，按策略创建恢复点。
4. 受控自动备份：仅在数据库仍健康、磁盘空间充足、未处于备份锁定时执行；首期仅用于定时任务和高风险操作前的恢复点。

服务异常退出不等于数据即将丢失，因此不建议默认“检测到任意故障就自动备份”。在 MySQL 不可达、磁盘已满、疑似数据损坏或连续备份失败时自动备份甚至可能扩大事故。这些情况应发送高优先级邮件告警并交由操作者处理；`BACKUP_AUTO_ON_INCIDENT` 首期默认保持 `false`。
...
- 备份写入 `/data/backups` 或对象存储，不能和容器临时层放在一起。
- 本地备份目录采用容量配额和保留策略；远端对象存储开启版本控制或不可变保留更可靠。
- 备份文件加密使用操作者提供的密钥或 KMS，不把密钥写进数据库、日志或清单。
- 同时只允许一个备份或恢复任务运行，使用数据库锁或专用锁文件防止并发覆盖。
...
BACKUP_AUTO_BEFORE_ROLLBACK=true
BACKUP_AUTO_ON_INCIDENT=false
...
| 磁盘空间不足 | 不启动自动备份，触发高优先级告警与预设降级行为 |
| 手动备份 | 生成可校验、可解密、含清单的备份，审计记录完整 |
...
| --- | --- | --- | --- | --- |
| 手动 | 仅操作者手动创建，重要操作前可自动创建恢复点 | 无固定保证 | 视最近备份而定 | 本地试用、开发 |
| 标准（推荐） | 每天 03:30 备份；保留最近 7 份；高风险操作前创建恢复点 | 24 小时 | 2 小时内 | 个人部署、小型知识库 |
| 加强 | 每 6 小时备份；本地保留 14 天，远端保留 30 天 | 6 小时 | 2 小时内 | 数据更新频繁、多人共用 |

首期建议在安装向导或管理员设置中让使用者选择档位，而不是隐式开启。默认展示“标准”作为推荐值，但必须由管理员确认备份目录、可用磁盘空间和收件人邮箱后才启用。
...
3. **Qdrant 快照做成可选的第二阶段能力。** 丢失 Qdrant 索引通常可由 MySQL 元数据和原文件重新构建，但耗时并可能再次消耗模型调用成本。频繁使用大知识库时，再开启 Qdrant 快照。
...
| 入口 | 建议功能 | 原因 |
| --- | --- | --- |
| 管理前端（仅系统管理员） | 健康状态、最近告警、磁盘状态、备份列表、创建手动备份、设置保留策略 | 日常使用直观；创建备份风险可控，仍需二次确认与审计 |
...
| 1 | `feature/mysql-8-ops-health` | `/livez`、`/readyz`、依赖短超时检查；保持 `/health` 兼容 | 单元测试、MySQL 依赖断开/恢复测试、PostgreSQL/SQLite 回归 |
| 2 | `feature/mysql-8-docker-log-rotation` | Compose 中所有服务的 Docker 日志上限 | Compose 配置检查、连续输出验证轮转、磁盘不无限增长 |
| 3 | `feature/mysql-8-app-log-limits` | `LOG_PATH` 轮转参数配置化、磁盘阈值信号 | 日志轮转、错误配置、低磁盘阈值测试 |
| 4 | `feature/mysql-8-ops-metrics` | 低基数指标与运行状态接口 | 指标格式测试、无敏感字段、高基数防护检查 |
| 5 | `feature/mysql-8-email-alerts` | SMTP 邮件、告警去重和恢复通知 | SMTP 测试服务器、冷却期、通知失败不形成日志风暴 |
