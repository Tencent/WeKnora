...
本实现对应腾讯犀牛鸟开源实战课题三“质量评测基线与成本可观测”。
...
- 提供中、英、日、俄、韩五种界面文案，未知调用用途安全回退为可读文本。
...
完整指标、原始导出校验和、脱敏策略和结果边界见
[RAG 双模型受控对比报告](./evidence/rag-model-comparison-2026-09-08.md)。

## 真实实验二：百炼 Wiki 缓存

2026-09-09 在本地 WeKnora 使用阿里云百炼 `qwen3.7-plus` 执行严格 A/B。
三组隔离冷请求分别与逐字节相同的暖请求一一配对，完整请求 HMAC、公共前缀 HMAC、
模型、工作负载和价格配置均通过自动校验。
...
| Token 缓存命中率 | 0% | 99.62% | +99.62 个百分点 |
| 耗时中位数 | 2.675 s | 2.364 s | -11.63% |
| 耗时 P95 | 3.219 s | 2.397 s | -25.54% |
| 估算成本 | 0.014798 CNY | 0.004822 CNY | -67.41% |

两组 Prompt Token 完全一致，Token 总量相差 4 仅来自模型输出长度波动。缓存命中由
Provider usage 字段证明，费用采用同一价格快照：普通输入 2、输出 8、缓存读取 0.4、
缓存创建 2 CNY/M。免费额度可能使实际扣款为零，但不影响原价口径下的严格 A/B 对比。

完整证据：

- [严格 A/B 中文实验报告](./evidence/wiki-cache-bailian-strict-2026-09-09.md)
- [严格 A/B 原始结构化结果](./evidence/wiki-cache-bailian-strict-2026-09-09.json)

2026-08-30 的 `wiki-cache-bailian-2026-08-30.*` 文件继续保留，以保证探索性线上 Wiki
实验可追溯。该轮缓存 usage 仍可复核，但当时把隐式缓存创建价格错误配置为 0，费用和
成本改善结论已经作废；Markdown 报告顶部已添加明确勘误，不能作为最终成本验收证据。

## 真实实验三：Ollama Embedding 持久化缓存
...
- 严格 Wiki A/B 只证明缓存对相同请求的命中率、费用和延迟影响，不用于证明回答质量。
- 百炼实验按执行时配置的公开价格估算；账户使用免费额度时，实际账单可能为零。
- 8 月 30 日探索性 Wiki 实验的费用因价格配置错误而作废，仅保留缓存 usage 和过程记录。
