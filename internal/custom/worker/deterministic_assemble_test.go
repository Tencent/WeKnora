...
func TestComposeTranscriptPageKeepsFoundationOrderAndMetadata(t *testing.T) {
	content := composeTranscriptPage("视频一", "video-1", "generation-1", "概览", "大纲", "总结", "知识")
	...
	if strings.Index(content, "概览") > strings.Index(content, "大纲") || strings.Index(content, "大纲") > strings.Index(content, "总结") {
...
func TestComposeTranscriptPageStripsNestedFrontmatter(t *testing.T) {
	...
	if strings.Contains(content, "type: overview") {
