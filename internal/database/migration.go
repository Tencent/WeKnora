...
func RunMigrationsWithOptions(dsn string, opts MigrationOptions) error {
	...
	migrationsPath, err := migrationSourceForDSN(dsn)
	if err != nil {
...
func GetMigrationVersion() (uint, bool, error) {
	...
	migrationsPath, err := migrationSourceForDSN(dbURL)
	if err != nil {
		return 0, false, err
	}
...
func migrationSourceForDSN(dsn string) (string, error) {
	switch {
	case strings.HasPrefix(dsn, "sqlite3://"):
		return "file://migrations/sqlite", nil
	case strings.HasPrefix(dsn, "mysql://"):
		return "file://migrations/mysql", nil
	case strings.HasPrefix(dsn, "postgres://"), strings.HasPrefix(dsn, "postgresql://"):
		return "file://migrations/versioned", nil
	default:
		return "", fmt.Errorf("unsupported migration database DSN: expected postgres, mysql, or sqlite3 scheme")
	}
}
...
func migrationDSNFromEnv() (string, error) {
	...
	case "mysql":
		...
		return fmt.Sprintf(
			...
			net.JoinHostPort(host, os.Getenv("DB_PORT")),
