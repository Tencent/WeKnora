...
func TestLearningSQLiteMigrationConstraintsAndRollback(t *testing.T) {
	...
	for range 2 {
		...
		_, err = db.Exec(`INSERT INTO learning_quizzes(id,tenant_id,subject_id,page_id,knowledge_base_id,epoch,source_stamp,status,created_at,updated_at)
		...
		require.NoError(t, err)
		_, err = db.Exec(`INSERT INTO learning_questions(id,quiz_id,position,prompt,options,correct_option,explanation,evidence,fingerprint)
			VALUES ('question','q1',0,'prompt','[]','0','explanation','[]','fp')`)
		require.NoError(t, err)
		_, err = db.Exec(`INSERT INTO learning_attempts(tenant_id,subject_id,attempt_id,question_id,quiz_id,page_id,knowledge_base_id,fingerprint,source_stamp,before,after,credited,algorithm_version,result,created_at)
			VALUES (1,'web_user:a','a1','question','q1','page','kb','fp','stamp',0.2,0.55,TRUE,'bkt-v1','{}',CURRENT_TIMESTAMP)`)
		require.NoError(t, err)
		_, err = db.Exec(`INSERT INTO learning_attempts SELECT tenant_id,subject_id,'a2',question_id,quiz_id,page_id,knowledge_base_id,fingerprint,source_stamp,before,after,credited,algorithm_version,result,created_at FROM learning_attempts`)
		require.Error(t, err)
		...
		require.NoError(t, err)
		for _, table := range []string{"learning_profiles", "learning_mastery", "learning_quizzes", "learning_questions", "learning_attempts"} {
