...
func TestJSONScalarLookup(t *testing.T) {
	pg, pgKey := JSONScalarLookup("postgres", "metadata", "external_id")
	assert.Equal(t, "metadata ->> ?", pg)
	assert.Equal(t, "external_id", pgKey)

	my, myKey := JSONScalarLookup("mysql", "metadata", `source."resource-id"`)
	assert.Equal(t, "JSON_UNQUOTE(JSON_EXTRACT(metadata, ?))", my)
	assert.Equal(t, `$."source.\"resource-id\""`, myKey)

	sq, sqKey := JSONScalarLookup("sqlite", "metadata", "external.id")
	assert.Equal(t, "json_extract(metadata, ?)", sq)
	assert.Equal(t, `$."external.id"`, sqKey)
}
