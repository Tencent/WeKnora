...
const WikiPageModifyContinuationPrompt = `Your previous message was cut off because it reached the output length limit, so the page is incomplete.

Continue the SAME message from exactly where it stopped:
1. Do NOT repeat anything you already wrote — not the SUMMARY line, not the headings, not the rows, not the opening of the row you were in the middle of.
2. Your reply is appended verbatim to what you already wrote. Its first characters must be exactly the characters that follow the last character of your previous message. If you stopped in the middle of a Markdown table row, finish that row first.
