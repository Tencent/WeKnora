...
`)
	...
	if slices.Contains(names, tools.ToolDiscoverMCPTools) {
		b.WriteString("- Connected MCP services may provide relevant evidence or actions. Use the selected service when applicable; a service description or a discovered tool is not itself evidence that an action was performed.\n")
	}
