...
const (
	...
	// install accepted, taking read down for a working install.
...
// A source lives for a single agent run. read is typically called
...
type TenantSkillSource struct {
	...
	// read inflate one entry at a time so a 13k-file skill cannot pin
...
// would need a sandbox, and read must work whether or not this turn has
// already booted one.
