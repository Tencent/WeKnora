...
func TestOfficeCoreSourceCannotAdvertiseOrRunBrowser(t *testing.T) {
	manifest, err := builtin.PublishedManifestForProfile("office-core")
	require.NoError(t, err)
	source := NewBuiltinSkillSource(manifest)
	metadata, err := source.DiscoverSkills()
	require.NoError(t, err)
	require.Len(t, metadata, 4)
	_, err = source.LoadSkillInstructions("browser")
	require.Error(t, err)
	manager := NewManager(&ManagerConfig{Enabled: true}, nil).WithTenantSource(source)
	require.NoError(t, manager.Initialize(context.Background()))
	_, _, err = manager.PrepareShellEnvironment(context.Background(), "session", "browser", "true", nil)
	require.Error(t, err)
	command, _, err := manager.PrepareShellEnvironment(context.Background(), "session", "pdf", "python3 report.py", nil)
	require.NoError(t, err)
	require.Contains(t, command, "builtin skill runtime missing")
}
