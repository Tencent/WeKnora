...
const cubeMetaDescription = `List the semantic models (cube/view) visible to the current identity in the semantic modeling module, with their fields.

## Usage
- You MUST call this tool before querying business data: the model and field names it returns (of the form "cube_name.field") are the valid inputs to cube_query
- Results are filtered by the current user's data permissions; models you cannot see are ones you have no access to, so do not guess field names
- Pass the cube parameter to filter by keyword and inspect a single model

## Output
Each model lists its title/description along with its measures, dimensions, and segments; the title and description are the primary hints for understanding what each field means.`
...
func (t *CubeMetaTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	if len(meta.Cubes) == 0 {
		b.WriteString("The current identity has no visible semantic models. Either modeling has not started, or the current user has not been granted any data group.\n")
	}
...
2. Assemble the query: measures (required, e.g. sales.amount), dimensions (optional, grouping), time_dimensions (time range/granularity), filters, order (sort), limit (default 200)
3. Field names must be fully qualified as "model_name.field_name"; do not invent field names

## Examples
{"measures": ["orders.count"], "dimensions": ["orders.status"], "time_dimensions": [{"dimension": "orders.created_at", "date_range": ["2025-01-01", "2025-12-31"], "granularity": "month"}]}
{"measures": ["stock.quantity"], "filters": [{"member": "stock.warehouse", "operator": "equals", "values": ["guangzhou_warehouse"]}]}

## Notes
- Results are filtered by the user's data permissions; if you get a "permission denied" hint, switch to a visible model instead of retrying
- results are in YAML row format; row count is capped — for finer detail narrow the time range or add more filters`

const cubeSQLDescription = `Show the SQL a cube_query would generate (syntax expansion only, not executed).

## Usage
The inputs are identical to cube_query; use this to explain the data semantics to the user, verify query intent, or debug unexpected results.
...
type CubeQueryFilterInput struct {
	...
	Operator string   `json:"operator" jsonschema:"equals/notEquals/contains/startsWith/gt/gte/lt/lte/set/notSet/inRange/beforeDate/afterDate"`
...
func validateBoundModels(bound []string, input *CubeQueryInput) error {
	...
	check := func(member string) error {
		if i := strings.Index(member, "."); i > 0 {
			if !allowed[member[:i]] {
				return fmt.Errorf("model %s is not bound to the current agent; available models: %s", member[:i], strings.Join(bound, ", "))
			}
		}
		return nil
	}
