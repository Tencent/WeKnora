...
type AgentCompleteData struct {
	...
	// AgentSteps contains []types.AgentStep values with detailed execution data.
...
type AgentToolResultData struct {
	...
	// FeedbackReferences is server-only attribution metadata.
	FeedbackReferences []types.AgentFeedbackReference `json:"-"`
}
