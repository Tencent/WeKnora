...
// KnowledgeFolderSearchResult is one row of the cross-knowledge-base folder
// search backing the chat "@" picker. It carries the owning KB's name so the
// picker can label a folder without a second lookup, and the live document
// count because the search only ever returns non-empty folders — a folder with
// no documents is not a usable retrieval scope.
type KnowledgeFolderSearchResult struct {
	KnowledgeFolder
	KnowledgeCount    int64  `json:"knowledge_count"`
