...
type Event struct {
	ID        string                 // Event ID
	Type      Type                   // Event type (uses Type from chat_manage.go)
...
// BusInterface defines the interface for event bus operations
// This interface allows types package to use Bus without importing the concrete type
// and avoids circular dependencies
type BusInterface interface {
	// On registers an event handler for a specific event type
	On(eventType Type, handler Handler)
