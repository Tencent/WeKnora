...
type OrganizationRepository interface {
	...
	ListTenantMembers(ctx context.Context, orgID string) ([]*types.OrganizationTenantMember, error)
	GetTenantMember(ctx context.Context, orgID string, tenantID uint64) (*types.OrganizationTenantMember, error)
	ListTenantMembersByTenantForOrgs(
		ctx context.Context,
		tenantID uint64,
		orgIDs []string,
	...
	GetPendingJoinRequestByTenant(
		ctx context.Context,
		orgID string,
		tenantID uint64,
	) (*types.OrganizationJoinRequest, error)
	GetPendingRequestByTenantAndType(
		ctx context.Context,
		orgID string,
		tenantID uint64,
...
type KBShareService interface {
	...
	ListSharedKnowledgeBasesInOrganization(
		ctx context.Context,
		orgID string,
		tenantID uint64,
		callerTenantRole types.TenantRole,
	...
	ListSharedKnowledgeBaseIDsByOrganizations(
		ctx context.Context,
		orgIDs []string,
		tenantID uint64,
...
type AgentShareService interface {
	...
	ListSharedAgentsInOrganizations(
		ctx context.Context,
		orgIDs []string,
		tenantID uint64,
		callerTenantRole types.TenantRole,
	) (map[string][]*types.OrganizationSharedAgentItem, error)
	// SetSharedAgentDisabledByMe sets whether the current tenant has "disabled" this shared agent for their
	// conversation dropdown (per-tenant preference; will be revisited in a follow-up PR).
	...
	// GetSharedAgentForTenant returns the shared agent by agentID if the caller's tenant has access; used to resolve KB
	// scope for @ mention.
	...
	// TenantCanAccessKBViaSomeSharedAgent returns true if the caller's tenant has at least one shared agent that can
	// access the given KB (for opening KB detail from "通过智能体可见" list without passing agent_id).
	...
	// GetShareByAgentIDForTenant returns one share for the given agentID that the tenant can access, excluding
	// source_tenant_id == excludeTenantID (e.g. caller's own tenant to get shared-from-other only).
	GetShareByAgentIDForTenant(
		ctx context.Context,
		tenantID uint64,
		agentID string,
...
type AgentShareRepository interface {
	...
	CountByOrganizations(ctx context.Context, orgIDs []string) (map[string]int64, error)
	// GetShareByAgentIDForTenant returns one share for the given agentID that the tenant can access (tenant in org),
	// excluding source_tenant_id == excludeTenantID.
	GetShareByAgentIDForTenant(
		ctx context.Context,
		tenantID uint64,
		agentID string,
...
// TenantDisabledSharedAgentRepository stores per-tenant "disabled" agents (hidden from conversation dropdown; own and
// shared)
type TenantDisabledSharedAgentRepository interface {
	...
	// ListDisabledOwnAgentIDs returns agent IDs that this tenant has disabled for their own agents (source_tenant_id =
	// tenant_id)
