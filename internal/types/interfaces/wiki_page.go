...
type WikiPageService interface {
	...
	// FindSimilarPages performs a dialect-appropriate title similarity search.
	// Used by the dedup pre-filter to surface candidate merge targets server-side.
...
type WikiPageRepository interface {
	...
	// is most similar under the active database dialect. `pageTypes`
	// empty defaults to entity+concept. Used by the dedup pre-filter
	// to surface candidate merge targets server-side.
	FindSimilarPages(ctx context.Context, kbID string, query string, pageTypes []string, limit int) ([]*types.WikiPageLite, error)
