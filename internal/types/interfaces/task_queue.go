...
type TaskPendingOpsRepository interface {
	...
	// Scope, ScopeID, Op, DedupKey, Payload; ID and FailCount use database
	// defaults, while a zero EnqueuedAt is normalized by the repository.
	...
	// abandoned by a crashed worker. On PostgreSQL and MySQL the per-key anchor
	// row is locked with FOR UPDATE SKIP LOCKED so concurrent claimers take
