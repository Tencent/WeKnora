...
// StorageBackendSourceEnv and related constants.
const (
	// StorageBackendSourceUser marks a user-managed storage backend row.
...
// TableName returns the storage_backends table name.
func (StorageBackend) TableName() string { return "storage_backends" }

// BeforeCreate assigns defaults before inserting a storage backend row.
...
// Validate checks required storage backend fields and provider config.
...
// Value encrypts secrets and serializes StorageBackendConfig for persistence.
...
// Scan decrypts secrets and loads StorageBackendConfig from the database.
...
// MaskSensitiveFields returns a copy with credential fields redacted.
...
// MergeSecrets preserves existing secrets when incoming values are redacted.
...
// ValidateForProvider implements the required interface method.
func (c StorageBackendConfig) ValidateForProvider(provider string) error {
	...
	case "cos":
		for name, value := range map[string]string{"region": c.Region, "access_key_id": c.AccessKeyID, "secret_access_key": c.SecretAccessKey, "bucket_name": c.BucketName} { //nolint:lll // long struct tag or string
			if err := required(name, value); err != nil {
				return err
			}
		}
		return nil
	default:
		for name, value := range map[string]string{"endpoint": c.Endpoint, "region": c.Region, "access_key_id": c.AccessKeyID, "secret_access_key": c.SecretAccessKey, "bucket_name": c.BucketName} { //nolint:lll // long struct tag or string
			if err := required(name, value); err != nil {
				return err
			}
		}
...
func (b StorageBackend) ToStorageEngineConfig() *StorageEngineConfig {
	...
	case "minio":
		...
		result.MinIO = &MinIOEngineConfig{
			Mode:            mode,
			Endpoint:        c.Endpoint,
...
// NewStorageBackendResponse is an exported function.
...
func StorageBackendFromLegacy(tenantID uint64, provider string, legacy *StorageEngineConfig) *StorageBackend {
	...
	b := &StorageBackend{
		TenantID:    tenantID,
		Provider:    provider,
		Source:      StorageBackendSourceUser,
		Status:      StorageBackendStatusActive,
		LegacyAlias: true,
	}
	switch provider {
	case "local":
...
func StorageBackendFromEnvironment(tenantID uint64) *StorageBackend {
	...
	case "minio":
		b.Config = StorageBackendConfig{
			...
			PathPrefix:      os.Getenv("MINIO_PATH_PREFIX"),
	...
	case "cos":
		b.Config = StorageBackendConfig{
			...
			TempBucketName:  os.Getenv("COS_TEMP_BUCKET_NAME"),
			TempRegion:      os.Getenv("COS_TEMP_REGION"),
		}
	case "tos":
		b.Config = StorageBackendConfig{
			...
			PathPrefix:      os.Getenv("TOS_PATH_PREFIX"),
			TempBucketName:  os.Getenv("TOS_TEMP_BUCKET_NAME"),
			TempRegion:      os.Getenv("TOS_TEMP_REGION"),
		}
	case "s3":
		b.Config = StorageBackendConfig{
	...
	case "obs":
		b.Config = StorageBackendConfig{
			...
			AccessKeyID:     os.Getenv("OBS_ACCESS_KEY"),
			SecretAccessKey: os.Getenv("OBS_SECRET_KEY"),
			BucketName:      os.Getenv("OBS_BUCKET_NAME"),
			PathPrefix:      os.Getenv("OBS_PATH_PREFIX"),
			UseSSL:          !strings.EqualFold(os.Getenv("OBS_USE_SSL"), "false"),
		}
	default:
		return nil
	}
