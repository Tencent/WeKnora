...
// FolderScope represents a folder-constrained retrieval scope inside one
// knowledge base. Folder subtrees are resolved to concrete knowledge IDs at
// request time (mirroring how document-KB tag scopes resolve), so the
// retrieval backends only ever see a KnowledgeIDs filter.
...
type SearchTarget struct {
	...
	// kept for tracing after the subtree has been resolved to KnowledgeIDs.
...
func HasKnowledgeRetrievalScope(
	...
	for _, target := range searchTargets {
		...
		if target.Type == SearchTargetTypeKnowledgeBase || len(target.KnowledgeIDs) > 0 ||
			len(target.TagIDs) > 0 || len(target.ScopeTagIDs) > 0 || len(target.ScopeFolderIDs) > 0 {
			return true
		}
	}
	return false
