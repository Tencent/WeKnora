...
var contextCloneAcrossDetach = map[ContextKey]bool{
	...
	// OrgUnitID is the caller's active unit in the tenant hierarchy
	// (X-Org-Unit-ID / primary membership). Background work that lists
	// agents, KBs, or MCP services must keep this restriction; dropping it
	// would let a cloned goroutine see sibling units the request could not.
	OrgUnitIDContextKey: true,
