...
func renderBlock(builder *strings.Builder, block map[string]interface{}, depth int, unknown map[string]struct{}) {
	...
	case "heading", "heading1", "heading2", "heading3", "heading4", "heading5", "heading6", "heading_1", "heading_2", "heading_3", "heading_4", "heading_5", "heading_6", "h1", "h2", "h3", "h4", "h5", "h6", "title":
		text := textBlockContent(block, payload, depth, unknown)
		if text != "" {
	...
	case "paragraph", "text", "normal":
		if text := textBlockContent(block, payload, depth, unknown); text != "" {
			builder.WriteString(text)
			builder.WriteString("\n\n")
			written = true
		}
	...
	case "unorderedlist", "unordered_list", "bullet", "bulletedlist", "bulleted_list":
		...
		if !written {
			written = renderOfficialList(builder, block, payload, "- ", depth, unknown)
		}

	case "orderedlist", "ordered_list", "number", "numberedlist", "numbered_list":
		written = renderList(builder, payload, "1. ", depth)
		if !written {
			written = renderOfficialList(builder, block, payload, "1. ", depth, unknown)
		}
	...
	case "callout", "columns":
		// These are containers whose children are block elements. Their visual
		// decoration cannot be represented faithfully in Markdown, but their
		// child content is rendered below.
		written = true
	...
	if !hasInlineChildren(blockType) {
...
// textBlockContent prefers rich inline children over the summary text carried
// in paragraph/heading payloads. DingTalk commonly returns both; concatenating
// them would duplicate every paragraph.
func textBlockContent(
	block, payload map[string]interface{},
	depth int,
	unknown map[string]struct{},
) string {
	if inline := renderInlineElements(firstArray(block, "children"), depth+1, unknown); inline != "" {
		return inline
	}
	if text := extractInlineText(payload); text != "" {
		return text
...
func hasInlineChildren(blockType string) bool {
	switch blockType {
	case "heading", "heading1", "heading2", "heading3", "heading4", "heading5", "heading6",
		...
		"blockquote", "quote",
		"unorderedlist", "unordered_list", "bullet", "bulletedlist", "bulleted_list",
		"orderedlist", "ordered_list", "number", "numberedlist", "numbered_list":
		return true
	default:
		return false
	}
...
func listLevel(payload map[string]interface{}) int {
	list := firstMap(payload, "list")
	if value, ok := list["level"].(float64); ok && value > 0 {
		level := int(value)
...
func renderInlineElements(values []interface{}, depth int, unknown map[string]struct{}) string {
	if depth > maxResourceDepth {
	...
	for _, value := range values {
		...
		case map[string]interface{}:
			elementType := normalizedBlockType(firstString(typed, "elementType", "type"))
			properties := firstMap(typed, "properties")
			switch elementType {
			case "", "text":
				text := firstRawString(typed, "text", "value")
				if text == "" {
					text = firstRawString(properties, "text", "value")
				}
				builder.WriteString(styleInlineText(text, typed))
			case "sticker":
				code := firstNonEmptyString(firstString(properties, "code"), firstString(typed, "code"))
				if code != "" {
					builder.WriteString(code)
				}
			case "image", "picture":
				src := firstNonEmptyString(
					firstString(properties, "src", "url"),
					firstString(typed, "src", "url"),
				)
				if src != "" {
					alt := firstNonEmptyString(
						firstString(properties, "alt", "title", "name"),
						firstString(typed, "alt", "title", "name"),
						"image",
					)
			...
			case "link", "hyperlink":
				href := firstNonEmptyString(
					firstString(properties, "href", "url"),
					firstString(typed, "href", "url"),
				)
				label := renderInlineElements(firstArray(typed, "children"), depth+1, unknown)
				if label == "" {
					label = firstRawString(typed, "text", "value")
				}
				if label == "" {
					label = firstRawString(properties, "text", "title", "name")
				}
				if label == "" {
					label = href
				}
			...
			default:
				if text := extractInlineText(typed); text != "" {
					builder.WriteString(text)
				}
				if elementType != "" {
...
func styleInlineText(text string, element map[string]interface{}) string {
	if text == "" {
		return ""
	}
	if fonts := strings.ToLower(firstString(element, "fonts", "font")); fonts == "monospace" {
		text = "`" + strings.ReplaceAll(text, "`", "\\`") + "`"
	}
	if enabledBool(element, "bold") {
		text = "**" + text + "**"
	}
	if enabledBool(element, "italic") {
		text = "*" + text + "*"
	}
	if enabledBool(element, "strike", "stike", "strikethrough") {
		text = "~~" + text + "~~"
	}
	return text
}

func enabledBool(values map[string]interface{}, keys ...string) bool {
	for _, key := range keys {
