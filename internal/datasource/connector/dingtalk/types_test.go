...
func TestDingTalkFailureClassification(t *testing.T) {
	tests := []struct {
		name      string
		err       error
		wantCode  string
		...
		{
			name:     "permission",
			err:      &dingTalkAPIError{StatusCode: 403, Code: "permissionDenied"},
			wantCode: "dingtalk_auth_or_permission",
		},
		{
			name:     "rate limit",
			err:      &dingTalkAPIError{StatusCode: 429, Code: "rateLimited"},
			wantCode: "dingtalk_rate_limited",
		},
		{
			name:     "timeout",
			err:      context.DeadlineExceeded,
			wantCode: "dingtalk_timeout",
		},
		{
			name:     "server unavailable",
			err:      &dingTalkAPIError{StatusCode: 503, Code: "operationTimeout"},
			wantCode: "dingtalk_server_unavailable",
		},
		{
			name:      "api error",
			err:       &dingTalkAPIError{StatusCode: 400, Code: "paramError"},
			wantCode:  "dingtalk_api_error",
			wantValue: "paramError",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			code, value, fallback := dingtalkFailure(tt.err)
			if code != tt.wantCode || value != tt.wantValue || fallback == "" {
				t.Fatalf("dingtalkFailure() = (%q, %q, %q)", code, value, fallback)
			}
		})
	}
}
