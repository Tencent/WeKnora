...
func (c *Connector) sync(
	...
	for _, resourceID := range resourceIDs {
		...
		if resolveErr != nil {
			...
			warnings = append(warnings, fmt.Sprintf("resource %s: %v", redact(resourceID), resolveErr))
		...
		for _, failure := range result.Failures {
			...
			warnings = append(warnings, fmt.Sprintf("folder %s: %v", redact(failure.ParentNodeID), failure.Err))
		}
		...
		for _, node := range result.Nodes {
			...
			if err != nil {
				...
				warnings = append(warnings, fmt.Sprintf("document %s: %v", redact(node.NodeID), err))
