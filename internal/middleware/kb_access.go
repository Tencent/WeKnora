...
func resolveKBAccess(
	...
	// Org-unit hierarchy rules apply only to own-tenant KBs; shared paths skip.
	if kb.TenantID == request.Caller.TenantID {
		if err := enforceOrgUnitKBAccess(c, kb, requiredPermission); err != nil {
			if stderrors.Is(err, errKBAccessForbidden) {
	...
	grant, err := access.ResolveKB(ctx, request, kb, requiredPermission, kbShareService, agentShareService)
	if err != nil {
		return nil, err
	}
	...
	if kb.TenantID == request.Caller.TenantID {
		...
		if kbUnitID != "" && activeID != "" && kbUnitID != activeID {
			grant.Permission = types.OrgRoleViewer
		}
	}
	return grant, nil
}
