...
// AppSecret are only used by signing providers (TreeRAGCloud).
...
// --- TreeRAGCloud: custom endpoint + request signing + multi-content downgrade ---

type treeRAGCloudProvider struct{ baseProvider }

func (treeRAGCloudProvider) Name() provider.ProviderName { return provider.ProviderTreeRAGCloud }

func (treeRAGCloudProvider) Endpoint(baseURL, _ string, _ bool) string {
	return strings.TrimRight(baseURL, "/") + "/api/v1/chat/completions"
}

func (treeRAGCloudProvider) ForceRawHTTP() bool { return true }

func (treeRAGCloudProvider) Auth(req *http.Request, creds authCreds, body []byte) {
...
func (treeRAGCloudProvider) TransformMessages(messages []openai.ChatCompletionMessage) []openai.ChatCompletionMessage {
