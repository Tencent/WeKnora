...
func registerTestVendors(t *testing.T) {
	...
	t.Cleanup(reset())
	Register(&Vendor{
		ID: GenericID, Name: "Custom", API: api.APIOpenAICompletions,
	...
	Register(&Vendor{
		...
		Compat: VendorCompat{OpenAICompletions: OpenAICompletionsCompat{
			ThinkingFormat:          Ptr(ThinkingFormatThinkingType),
...
func TestDetectByURL_LongestPatternWins(t *testing.T) {
	registerTestVendors(t)
	assert.Equal(t, "tencent-lkeap", DetectByURL("https://api.lkeap.cloud.tencent.com/v1"))
	assert.Equal(t, "acme", DetectByURL("https://api.acme.ai/v1"))
	assert.Equal(t, GenericID, DetectByURL("http://localhost:8000/v1"))
	assert.Equal(t, GenericID, DetectByURL(""))
}
...
func TestResolve_MergesVendorModelAndOverrideLayers(t *testing.T) {
	...
	assert.Equal(t, ThinkingFormatThinkingType, r.OpenAICompletions.ThinkingFormat)
...
func TestResolve_UnknownModelAndVendorDegradeToGeneric(t *testing.T) {
	...
	r, err := Resolve(Ref{Provider: "acme", Model: "brand-new"})
	require.NoError(t, err)
	assert.False(t, r.Cataloged)
	assert.Equal(t, "brand-new", r.Spec.ID)
	assert.Equal(t, ThinkingFormatThinkingType, r.OpenAICompletions.ThinkingFormat, "vendor defaults still apply")

	r, err = Resolve(Ref{Provider: "nope", Model: "x", BaseURL: "http://localhost:8000/v1"})
	require.NoError(t, err)
	assert.Equal(t, GenericID, r.Vendor.ID)
	assert.Equal(t, ThinkingFormatChatTemplateKwargs, r.OpenAICompletions.ThinkingFormat)

	r, err = Resolve(Ref{Model: "x", BaseURL: "https://api.acme.ai/v1"})
	require.NoError(t, err)
...
func TestResolve_ProtocolInference(t *testing.T) {
	...
	r, err := Resolve(Ref{Provider: "acme", Model: "acme-claude"})
	require.NoError(t, err)
	assert.Equal(t, api.APIAnthropicMessages, r.API)
	assert.Equal(t, AnthropicThinkingAdaptive, r.AnthropicMessages.ThinkingMode)
	assert.True(t, r.AnthropicMessages.SupportsEffort)
	assert.Equal(t, "thinking.adaptive", r.Capabilities().ThinkingFormat)

	r, err = Resolve(Ref{Provider: "acme", Model: "acme-pro", BaseURL: "https://api.acme.ai/anthropic"})
	require.NoError(t, err)
	assert.Equal(t, api.APIAnthropicMessages, r.API, "/anthropic suffix switches protocol")

	r, err = Resolve(Ref{Provider: "acme", Model: "acme-pro", Extra: map[string]string{ExtraAPI: "openai-responses"}})
	require.NoError(t, err)
	assert.Equal(t, api.APIOpenAIResponses, r.API, "extra_config.api forces the protocol")

	_, err = Resolve(Ref{Provider: "acme", Model: "acme-pro", Extra: map[string]string{ExtraAPI: "bogus"}})
	require.Error(t, err)
}

func TestResolve_LegacyThinkingControlAndRemoteModelName(t *testing.T) {
	registerTestVendors(t)
	r, err := Resolve(Ref{Provider: "acme", Model: "acme-pro", Extra: map[string]string{
		ExtraThinkingControl: "enable_thinking",
		ExtraRemoteModelName: "acme-pro-2026",
	}})
	require.NoError(t, err)
	assert.Equal(t, ThinkingFormatEnableThinking, r.OpenAICompletions.ThinkingFormat)
	assert.Equal(t, "acme-pro-2026", r.RemoteModel)

	r, err = Resolve(Ref{Provider: "acme", Model: "acme-pro", Extra: map[string]string{ExtraThinkingControl: "none"}})
	require.NoError(t, err)
...
func TestResolve_RejectsUnknownCompatKeys(t *testing.T) {
	registerTestVendors(t)
	_, err := Resolve(Ref{Provider: "acme", Model: "acme-pro", Override: &types.ModelSpecOverride{
...
func TestApplyOverlay(t *testing.T) {
	...
	r, err = Resolve(Ref{Provider: "my-vllm", Model: "Qwen/Qwen3-32B"})
	require.NoError(t, err)
	assert.Equal(t, ThinkingFormatChatTemplateKwargs, r.OpenAICompletions.ThinkingFormat)
	assert.Equal(t, "max_tokens", r.OpenAICompletions.MaxTokensField)
	...
	assert.Error(t, ApplyOverlay([]byte(`{"providers": {"x": {"bogus": 1}}}`), t.TempDir()), "unknown keys rejected")
	assert.Error(t, ApplyOverlay([]byte(`{"providers": {"x": {"api": "nope"}}}`), t.TempDir()))
}
