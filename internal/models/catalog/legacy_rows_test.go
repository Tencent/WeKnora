...
// model in the UI and saving it (PUT /models/{id} runs catalog.ValidateRow)
// must not 400, and the chat path must still resolve the row.
func TestLegacyRowsValidateAndResolve(t *testing.T) {
	for _, row := range legacyRows() {
		t.Run(row.name, func(t *testing.T) {
			params := row.params
			if err := catalog.ValidateRow(row.model, row.typ, &params); err != nil {
...
func TestEveryCataloguedModelResolves(t *testing.T) {
	for _, v := range catalog.List() {
		for _, m := range v.Models {
			...
			if err := catalog.ValidateRow(m.ID, modelType, &types.ModelParameters{Provider: v.ID}); err != nil {
...
func TestUncataloguedModelKeepsThinkingSwitch(t *testing.T) {
	unknown, err := catalog.Resolve(catalog.Ref{
...
func TestLegacyFreeFormAPIExtraKeyIsRejected(t *testing.T) {
	err := catalog.ValidateRow("gpt-4o", types.ModelTypeKnowledgeQA, &types.ModelParameters{
