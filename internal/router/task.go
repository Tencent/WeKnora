...
func RunAsynqServer(params AsynqTaskParams) *asynq.ServeMux {
	...
	mux.HandleFunc(types.TypeWebhookDeliver, func(ctx context.Context, t *asynq.Task) error {
		if params.WebhookDeliverer == nil {
			// Must not ack: a silent nil (e.g. dig type mismatch) previously
			// left deliveries pending forever while outbox was marked processed.
			logger.Errorf(ctx, "webhook deliverer not configured; refusing to ack")
			return errors.New("webhook deliverer not configured")
		}
