...
func RegisterKnowledgeRoutes(r *gin.RouterGroup, handler *handler.KnowledgeHandler, g *rbacGuards) {
	...
	// same Contributor + ingest/full policy as existing batch writes. The
	// folder-scoped move endpoint shares this plural /knowledges prefix with
	// the single-document MoveKnowledgeToFolder route registered inline in
	// NewRouter; both attach to the same path under different methods.
