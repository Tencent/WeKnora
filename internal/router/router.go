...
func RegisterChunkRoutes(r *gin.RouterGroup, handler *handler.ChunkHandler, g *rbacGuards) {
	...
	chunks := r.Group("/chunks")
	{
		...
		chunks.GET("/:knowledge_id", g.Viewer(), g.KBAccessReadFromKnowledgeIDParam("knowledge_id"), g.ChunkFeedbackGovernanceFromKnowledgeIDParam(), handler.ListKnowledgeChunks)
		...
		chunks.GET("/by-id/:id/feedback/weight-logs", g.Viewer(), g.KBAccessReadFromChunkIDParam("id"), g.OwnedChunkKBOrAdminFromChunkID(), handler.ListChunkFeedbackWeightLogs)
