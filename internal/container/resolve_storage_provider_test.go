...
// TestResolveStorageProviderPending_DefaultsToLocal verifies that an
// unset STORAGE_TYPE falls back to "local" rather than producing an
// empty provider string.
func TestResolveStorageProviderPending_DefaultsToLocal(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.Exec(`CREATE TABLE knowledge_bases (
		id TEXT PRIMARY KEY,
	...
	var got string
	require.NoError(t, db.Raw(`SELECT storage_provider_config FROM knowledge_bases WHERE id = 'kb-pending'`).Scan(&got).Error)
	assert.Equal(t, `{"provider":"local"}`, got)
}
