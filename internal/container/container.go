...
func initDatabase(cfg *config.Config) (*gorm.DB, error) {
	...
	// Built-in storage engine fallback from config/builtin_storage_engine.yaml (optional, no DB write).
	// Loaded regardless of AUTO_MIGRATE — runtime fallback singleton has no DB dependency.
	if err := types.LoadBuiltinStorageEngineConfig(config.ConfigDir()); err != nil {
		logger.Warnf(context.Background(), "Load builtin storage engine config failed: %v", err)
	}
