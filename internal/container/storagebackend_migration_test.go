...
func TestMigrateLegacyStorageBackends(t *testing.T) {
	...
	tenantConfig, err := (&types.StorageEngineConfig{DefaultProvider: "local", Local: &types.LocalEngineConfig{PathPrefix: "workspace-a"}}).Value() //nolint:lll
	require.NoError(t, err)
	...
	require.NoError(
		t,
		db.Exec(
			"INSERT INTO tenants(id, name, storage_engine_config) VALUES (?, ?, ?)",
			7,
			"workspace",
			tenantConfig,
		).Error,
	...
	require.NoError(
		t,
		db.Exec(
			"INSERT INTO knowledge_bases(id, tenant_id, storage_provider_config, cos_config) VALUES (?, ?, ?, ?)",
			"kb-a",
			7,
			providerConfig,
			[]byte("{}"),
	...
	require.NoError(
		t,
		db.Where("tenant_id = ? AND provider = ? AND legacy_alias = ?", 7, "local", true).First(&backend).Error,
	)
	...
	require.NoError(
		t,
		db.Raw("SELECT storage_backend_id FROM knowledge_bases WHERE id = 'kb-a'").Scan(&kbBackend).Error,
	)
