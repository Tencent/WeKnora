...
// embedding write.
//
// The values must match the keys of retrieverEngineMapping in
// internal/types/tenant.go.
var localOnlyRetrieveEngines = map[string]string{
	"postgres": "postgres",
	"sqlite":   "sqlite",
}
...
func ValidateDriverCombination(dbDriver, retrieveDriver string) error {
	...
	for _, d := range drivers {
		if name, isLocal := localOnlyRetrieveEngines[d]; isLocal {
			return fmt.Errorf(
				"DB_DRIVER=mysql is incompatible with RETRIEVE_DRIVER=%s: "+
					...
					"(%s).",
				name, name, validExternalRetrieveEnginesHint(),
			)
		}
	}
