...
type SessionTerminalManager interface {
	...
	// must not create microVMs as a side effect. A paused sandbox returns
	// ErrSandboxPaused unless opts.AllowResume is set, so a panel open
	// cannot silently resume (and re-bill) a paused instance. A backend
	// that cannot stream PTYs returns ErrTerminalUnsupported, not "no
	// sandbox".
