...
// UpdateMessageFeedback stores the latest feedback for an assistant message
// identified by the platform feedback ID. The ID is generated by WeCom and is
// persisted separately from RequestID, which is the internal Q&A pair ID.
func (r *messageRepository) UpdateMessageFeedback(ctx context.Context, feedbackID string, feedback *types.MessageFeedback) error {
	feedbackID = strings.TrimSpace(feedbackID)
	if feedbackID == "" || feedback == nil {
		return gorm.ErrRecordNotFound
	}
	result := r.db.WithContext(ctx).
		Model(&types.Message{}).
		Where("feedback_id = ? AND role = ? AND channel = ? AND deleted_at IS NULL", feedbackID, "assistant", "im").
