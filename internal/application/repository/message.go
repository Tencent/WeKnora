...
// SearchMessagesByKeyword searches messages by keyword (case-insensitive,
// dialect-aware via database.CaseInsensitiveLike: ILIKE on postgres,
// LOWER(col) LIKE LOWER(?) on mysql/sqlite) across sessions for a tenant
func (r *messageRepository) SearchMessagesByKeyword(
	...
	query := r.db.WithContext(ctx).
		...
		Where(database.CaseInsensitiveLike(r.db.Dialector.Name(), "messages.content", "?"), "%"+escapeLikeKeyword(keyword)+"%")
