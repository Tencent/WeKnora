...
func TestMetadataKeyPrefixConditionUsesDialectSpecificJSON(t *testing.T) {
	condition, args := metadataKeyPrefixCondition(dbWithDialect("postgres"), "external_id", `node\_%`)
	if condition != "metadata->>'external_id' LIKE ? ESCAPE ?" {
		t.Fatalf("postgres metadata prefix condition = %q", condition)
	}
	if len(args) != 2 || args[0] != `node\_%` || args[1] != `\` {
		t.Fatalf("postgres metadata prefix args = %#v", args)
	}

	condition, args = metadataKeyPrefixCondition(dbWithDialect("mysql"), "external_id", `node\_%`)
	if condition != "metadata_external_id LIKE ? ESCAPE ?" {
		t.Fatalf("mysql external_id metadata prefix condition = %q", condition)
	}
	if len(args) != 2 || args[0] != `node\_%` || args[1] != `\` {
		t.Fatalf("mysql external_id metadata prefix args = %#v", args)
	}

	condition, args = metadataKeyPrefixCondition(dbWithDialect("mysql"), "custom_key", `node\_%`)
	if condition != "JSON_UNQUOTE(JSON_EXTRACT(metadata, ?)) LIKE ? ESCAPE ?" {
		t.Fatalf("mysql generic metadata prefix condition = %q", condition)
	}
	if len(args) != 3 || args[0] != `$."custom_key"` || args[1] != `node\_%` || args[2] != `\` {
		t.Fatalf("mysql generic metadata prefix args = %#v", args)
	}
}
