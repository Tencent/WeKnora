...
var _ interfaces.WikiPageRenameChunkUpdater = (*chunkRepository)(nil)

func (r *chunkRepository) UpdateRenamedWikiChunk(
	ctx context.Context, page *types.WikiPage, chunk *types.Chunk, expectedUpdatedAt time.Time,
) error {
	if page == nil || chunk == nil || chunk.ID != "wp-"+page.ID ||
		chunk.TenantID != page.TenantID || chunk.KnowledgeBaseID != page.KnowledgeBaseID ||
		chunk.ChunkType != types.ChunkTypeWikiPage {
		return errors.New("invalid wiki rename chunk binding")
	}
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var current types.WikiPage
		if err := tx.Where("id = ? AND tenant_id = ? AND knowledge_base_id = ?",
			page.ID, page.TenantID, page.KnowledgeBaseID).
			Clauses(clause.Locking{Strength: "UPDATE"}).First(&current).Error; err != nil {
			return err
		}
		if current.Slug != page.Slug || current.Version != page.Version || !current.UpdatedAt.Equal(page.UpdatedAt) {
			return ErrWikiPageConflict
		}
