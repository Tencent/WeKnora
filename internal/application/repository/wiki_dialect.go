...
// similarity, full-text search), the fallback uses LIKE (higher recall).
...
// Returns (fragment, argCount).
func wikiFullTextSearch(dialectName string) (fragment string, argCount int) {
	switch dialectName {
	case "postgres":
	...
	default:
		return "(" + database.CaseInsensitiveLike(dialectName, "title", "?") +
			...
			" OR " + database.CaseInsensitiveLike(dialectName, "aliases", "?") + ")", 3
	}
}
