...
// NewStorageBackendRepository is an exported function.
...
func (r *storageBackendRepository) GetByID(
	ctx context.Context,
	tenantID uint64,
	id string,
...
func (r *storageBackendRepository) FindLegacyAlias(
	ctx context.Context,
	tenantID uint64,
	provider string,
) (*types.StorageBackend, error) {
	var backend types.StorageBackend
	if err := r.db.WithContext(ctx).Where("tenant_id = ? AND provider = ? AND legacy_alias = ?", tenantID, provider, true).First(&backend).Error; err != nil { //nolint:lll
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
