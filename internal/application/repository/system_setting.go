...
func (r *systemSettingRepository) Get(ctx context.Context, key string) (*types.SystemSetting, error) {
	...
	err := r.db.WithContext(ctx).Where(systemSettingKeyEquals(key)).First(&s).Error
	if err != nil {
...
func (r *systemSettingRepository) List(ctx context.Context) ([]*types.SystemSetting, error) {
	...
	err := r.db.WithContext(ctx).Order(systemSettingListOrder()).Find(&rows).Error
	if err != nil {
		return nil, err
	}
	return rows, nil
...
func (r *systemSettingRepository) Delete(ctx context.Context, key string) (bool, error) {
	res := r.db.WithContext(ctx).Where(systemSettingKeyEquals(key)).Delete(&types.SystemSetting{})
	if res.Error != nil {
...
func systemSettingListOrder() clause.OrderBy {
