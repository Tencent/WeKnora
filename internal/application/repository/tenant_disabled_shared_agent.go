...
func (r *tenantDisabledSharedAgentRepository) ListByTenantID(
	ctx context.Context,
	tenantID uint64,
) ([]*types.TenantDisabledSharedAgent, error) {
	var list []*types.TenantDisabledSharedAgent
	err := r.db.WithContext(ctx).Where("tenant_id = ?", tenantID).Find(&list).Error
	return list, err
}

func (r *tenantDisabledSharedAgentRepository) ListDisabledOwnAgentIDs(
	ctx context.Context,
	tenantID uint64,
) ([]string, error) {
	var ids []string
...
func (r *tenantDisabledSharedAgentRepository) Add(
	ctx context.Context,
	tenantID uint64,
	agentID string,
...
func (r *tenantDisabledSharedAgentRepository) Remove(
	ctx context.Context,
	tenantID uint64,
	agentID string,
	sourceTenantID uint64,
