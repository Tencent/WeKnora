...
// TestListPagedAndFolderCounts_HideEmbeddedImages documents the document-list
// contract for embedded image/board sub-items: the rows stay in the KB (VLM
// chunks, retrieval and image-map marker resolution read them) but must not
// appear as standalone documents in the paged list or the folder tree counts.
// Attachment rows are ordinary list members.
func TestListPagedAndFolderCounts_HideEmbeddedImages(t *testing.T) {
	db := setupKnowledgeTestDB(t)
	repo := NewKnowledgeRepository(db).(*knowledgeRepository)
	ctx := context.Background()
	...
	insertWithMetadata := func(t *testing.T, folderPath, fileName, metadata string) string {
		t.Helper()
		id := uuid.New().String()
		require.NoError(t, db.Exec(`
			INSERT INTO knowledges (id, tenant_id, knowledge_base_id, type, title, source, parse_status, file_name, folder_path, metadata)
			VALUES (?, ?, ?, 'file', ?, 'manual', 'completed', ?, ?, ?)
		`, id, tenantID, kbID, fileName, fileName, folderPath, metadata).Error)
		return id
	}

	docID := insertWithMetadata(t, "", "报告.md", `{"datasource_id":"ds1"}`)
	imgID := insertWithMetadata(t, "", "image-abc.png", `{"embedded_image":"true","parent_doc_id":"doc1"}`)
	boardID := insertWithMetadata(t, "画板", "board-xyz.png", `{"embedded_image":"true"}`)
	attID := insertWithMetadata(t, "", "手册.pdf", `{"attachment":"true"}`)
	...
	assert.ElementsMatch(t, []string{docID, attID}, ids,
		"embedded image/board rows must be hidden from the document list")
	assert.NotContains(t, ids, imgID)
	assert.NotContains(t, ids, boardID)
	assert.Equal(t, int64(2), total)
	...
	// The 画板 folder only held an embedded board — it drops out entirely.
	assert.Equal(t, map[string]int64{"": 2}, byPath,
		"folder counts must exclude embedded image/board rows")
}
