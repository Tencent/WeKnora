...
func (e *elasticsearchRepository) Save(ctx context.Context,
	...
	// Index the document
	resp, err := e.client.Index(e.index).Id(embedding.SourceID).Request(embeddingDB).Do(ctx)
	if err != nil {
...
func (e *elasticsearchRepository) BatchSave(ctx context.Context,
	...
	for _, embedding := range embeddingList {
		embeddingDB := elasticsearchRetriever.ToDBVectorEmbedding(embedding, additionalParams)
		// IndexOp with a stable ID provides replace/upsert semantics for a
		// reconciled chunk. CreateOp with an autogenerated ID accumulated stale
		// copies whenever the same chunk identity was re-embedded.
		err := indexRequest.IndexOp(types.IndexOperation{Index_: &e.index, Id_: &embedding.SourceID}, embeddingDB)
		if err != nil {
