...
func learningDelete(tx *gorm.DB, scope interfaces.LearningScope, kbID, pageID string) (*types.LearningClearResult, error) {
	filter := func() *gorm.DB {
...
func (r *learningRepository) Clear(ctx context.Context, scope interfaces.LearningScope, kbID string) (*types.LearningClearResult, error) {
	...
	err := r.profileTx(ctx, scope, true, false, func(tx *gorm.DB, p *types.LearningProfile) error {
		...
		if err := learningScope(tx, scope).Model(&types.LearningProfile{}).Updates(map[string]any{"epoch": p.Epoch + 1, "enabled": enabled}).Error; err != nil {
			return err
		}
...
func (r *learningRepository) Export(ctx context.Context, scope interfaces.LearningScope, kbID string) (*types.LearningExport, error) {
	var result *types.LearningExport
	err := r.profileTx(ctx, scope, false, false, func(tx *gorm.DB, p *types.LearningProfile) error {
		result = &types.LearningExport{Settings: types.LearningSettings{Enabled: p.Enabled, AlgorithmVersion: types.LearningAlgorithmVersion},
			Nodes: []*types.LearningNodeView{}, Quizzes: []*types.LearningQuizView{}, Attempts: []types.LearningAttemptExport{}, ExportedAt: time.Now().UTC()}
		var quizzes []types.LearningQuiz
		if err := learningKBFilter(learningScope(tx, scope), kbID).Order("created_at, id").Find(&quizzes).Error; err != nil {
			return err
		}
		retained := quizzes[:0]
		...
		if err := learningKBFilter(learningScope(tx, scope), kbID).Order("created_at, attempt_id").Find(&attempts).Error; err != nil {
			return err
		}
		for _, a := range attempts {
			result.Attempts = append(result.Attempts, types.LearningAttemptExport{PageID: a.PageID, KnowledgeBaseID: a.KnowledgeBaseID,
				Fingerprint: a.Fingerprint, SourceStamp: a.SourceStamp, Before: a.Before, After: a.After, Credited: a.Credited,
				AlgorithmVersion: a.AlgorithmVersion, CreatedAt: a.CreatedAt, Result: a.Result})
		}
		return nil
	})
	return result, err
...
func (r *learningRepository) Recover(ctx context.Context, limit int) ([]types.LearningGeneratePayload, error) {
	...
	var orphans []orphan
	const missing = `NOT EXISTS (SELECT 1 FROM wiki_pages p WHERE p.id = x.page_id AND p.tenant_id = x.tenant_id AND p.knowledge_base_id = x.knowledge_base_id AND p.deleted_at IS NULL)
		OR NOT EXISTS (SELECT 1 FROM knowledge_bases k WHERE k.id = x.knowledge_base_id AND k.tenant_id = x.tenant_id AND k.deleted_at IS NULL)`
	err := r.db.WithContext(ctx).Raw(`SELECT tenant_id, subject_id, page_id, knowledge_base_id FROM learning_quizzes x WHERE `+missing+
		` UNION SELECT tenant_id, subject_id, page_id, knowledge_base_id FROM learning_mastery x WHERE `+missing+
		` ORDER BY tenant_id, subject_id, page_id LIMIT ?`, limit).Scan(&orphans).Error
	if err != nil {
	...
	err = learningRemovedSourceQuizzes(r.db.WithContext(ctx)).Order("x.updated_at, x.id").Limit(limit).Find(&removedSources).Error
	if err != nil {
		return nil, learningDBError(err)
	}
	...
	err = r.db.WithContext(ctx).Table("learning_quizzes AS q").Select("q.*").
		Joins("JOIN learning_profiles p ON p.tenant_id = q.tenant_id AND p.subject_id = q.subject_id AND p.epoch = q.epoch AND p.enabled = ?", true).
		Where("q.status = ? OR (q.status = ? AND (q.lease_until IS NULL OR q.lease_until <= ?))", "pending", "running", time.Now().UTC()).
	...
	for _, q := range pending {
		...
		err := r.profileTx(ctx, scope, false, false, func(tx *gorm.DB, p *types.LearningProfile) error {
			...
			if current.Status != "pending" && (current.Status != "running" || (current.LeaseUntil != nil && current.LeaseUntil.After(time.Now()))) {
				return nil
			}
