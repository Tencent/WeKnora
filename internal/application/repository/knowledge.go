...
func (r *knowledgeRepository) MarkFinalizingKnowledgeFailed(ctx context.Context, id, reason string) (bool, error) {
	if id == "" {
		return false, nil
	}
	res := r.db.WithContext(ctx).
		Model(&types.Knowledge{}).
		Where("id = ? AND parse_status = ?", id, types.ParseStatusFinalizing).
		Updates(map[string]interface{}{
			"parse_status":           types.ParseStatusFailed,
			"error_message":          reason,
			"pending_subtasks_count": 0,
