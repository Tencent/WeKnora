...
func (d *TaskJobDispatcher) DispatchUserRoot(ctx context.Context, req UserRootDispatchRequest) (*asynq.TaskInfo, error) {
	...
	if err != nil {
		if d.repo != nil {
			_, _ = d.repo.MarkDispatchFailed(context.Background(), req.JobID, req.ExecutionID, interfaces.TaskLedgerFailure{
	...
	if d.repo != nil {
		_, _ = d.repo.MarkDispatched(context.Background(), req.ExecutionID, time.Now())
	}
	return info, nil
}
