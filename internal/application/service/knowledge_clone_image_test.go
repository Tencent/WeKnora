...
func TestCopyOwnedObjectDeletesFileWhenBindFails(t *testing.T) {
	ref := contentRef("y")
	files := &handleCopyFileService{saveRef: ref}
	catalog := &fakeCatalog{bindErr: fmt.Errorf("db down")}

	_, err := copyOwnedObject(
		context.Background(), files, files, "local://7/src.png", 7, "kn-dst",
	...
	if len(files.deleted) != 1 || files.deleted[0] != ref {
		t.Fatalf("deleted %v, want %q rolled back", files.deleted, ref)
	}
}
