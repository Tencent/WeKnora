...
func (s *KnowledgeAutoTagService) Handle(ctx context.Context, task *asynq.Task) error {
	...
	if knowledge.ParseStatus == types.ParseStatusCancelled || knowledge.ParseStatus == types.ParseStatusDeleting || knowledge.ParseStatus == types.ParseStatusFailed {
	...
	tags, _, err := s.tagRepo.ListByKB(ctx, payload.TenantID, payload.KnowledgeBaseID, &types.Pagination{Page: 1, PageSize: 1000}, "")
	if err != nil {
	...
	if err := adder.AddKnowledgeTagRelations(ctx, payload.TenantID, payload.KnowledgeBaseID, payload.KnowledgeID, added); err != nil {
...
func classifyExistingTags(ctx context.Context, model chat.Chat, tags []*types.KnowledgeTag, content string, maxTags int) (*autoTagModelResponse, error) {
	if maxTags <= 0 {
...
Rules: never invent a tag; return an empty matches array when uncertain; confidence must be between 0 and 1; choose at most %d tags.`, maxTags)
