...
func (s *sessionService) DeleteSession(ctx context.Context, id string) error {
	...
	knowledgeIDs, err := s.messageRepo.GetKnowledgeIDsBySessionID(ctx, id)
	if err != nil {
		return err
	}
...
func (s *sessionService) BatchDeleteSessions(ctx context.Context, ids []string) error {
	...
	for _, id := range visibleIDs {
		idsForSession, err := s.messageRepo.GetKnowledgeIDsBySessionID(ctx, id)
		if err != nil {
			return err
		}
		knowledgeIDs = append(knowledgeIDs, idsForSession...)
...
func (s *sessionService) DeleteAllSessions(ctx context.Context) error {
	...
	for _, session := range sessions {
		...
		idsForSession, err := s.messageRepo.GetKnowledgeIDsBySessionID(ctx, session.ID)
		if err != nil {
			return err
		}
		knowledgeIDs = append(knowledgeIDs, idsForSession...)
