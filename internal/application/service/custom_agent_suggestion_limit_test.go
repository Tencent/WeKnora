...
func TestGetSuggestedQuestions_ExplicitLimitOverridesStarterCount(t *testing.T) {
	...
	got, err := svc.GetSuggestedQuestions(ctx, "agent-1", nil, nil, nil, nil, false, 10)
...
func TestGetSuggestedQuestions_SmallerLimitStillHonored(t *testing.T) {
	...
	got, err := svc.GetSuggestedQuestions(ctx, "agent-1", nil, nil, nil, nil, false, 2)
	require.NoError(t, err)
	assert.Len(t, got, 2)
}
...
func TestGetSuggestedQuestions_UnspecifiedLimitFallsBackToStarterCount(t *testing.T) {
	...
	got, err := svc.GetSuggestedQuestions(ctx, "agent-1", nil, nil, nil, nil, false, 0)
	require.NoError(t, err)
	assert.Len(t, got, 4)
}
...
func TestGetSuggestedQuestions_LimitBoundedByMax(t *testing.T) {
	...
	got, err := svc.GetSuggestedQuestions(ctx, "agent-1", nil, nil, nil, nil, false, 999)
	require.NoError(t, err)
