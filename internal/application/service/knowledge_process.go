...
func (s *knowledgeService) processDocumentFromPassage(ctx context.Context,
	...
	if err := s.processChunks(ctx, kb, knowledge, chunks, opts); err != nil {
		logger.Errorf(ctx, "processDocumentFromPassage: process chunks failed for %s: %v", knowledge.ID, err)
	}
}
...
func (s *knowledgeService) processChunks(ctx context.Context,
	...
	if attemptSuperseded(ctx, s.tracker(), knowledge.ID, attemptFromCtx(ctx)) {
		logger.Infof(ctx, "processChunks: attempt %d superseded for %s, skipping stale chunk processing",
			attemptFromCtx(ctx), knowledge.ID)
		return nil
	}
	...
	// Reparse keeps content-addressed chunks and existing external indexes in
	// place until replacement chunks have been written successfully.
	logger.Infof(ctx, "Preparing idempotent reparse for knowledge: %s", knowledge.ID)
	...
	if err != nil && kb.NeedsEmbeddingModel() {
		...
		logger.Errorf(ctx, "Failed to resolve retrieve engine for knowledge %s: %v", knowledge.ID, err)
		return fmt.Errorf("resolve retrieve engine: %w", err)
	}

	logger.Infof(ctx, "Starting chunk diff for knowledge: %s", knowledge.ID)
	...
	if hasParentChild {
		...
		for i, pc := range options.ParentChunks {
			parentDBChunks[i] = &types.Chunk{
				...
				Content:         pc.Content,
				ContentHash:     contentcache.ChunkContentHash(pc.Content, ""),
				ChunkIndex:      pc.Seq,
				IsEnabled:       true,
				CreatedAt:       time.Now(),
				UpdatedAt:       time.Now(),
	...
	for idx, chunkData := range chunks {
		...
		textChunk := &types.Chunk{
			...
			Content:         chunkData.Content,
			ContextHeader:   chunkData.ContextHeader,
			ContentHash:     contentcache.ChunkContentHash(chunkData.Content, chunkData.ContextHeader),
	...
	if attemptSuperseded(ctx, s.tracker(), knowledge.ID, attemptFromCtx(ctx)) {
		logger.Infof(ctx, "processChunks: attempt %d superseded for %s before chunk write",
			attemptFromCtx(ctx), knowledge.ID)
		return nil
	}
	...
	chunkSync, err := s.syncReparseBaseChunks(ctx, tenantInfo.ID, knowledge.ID, insertChunks)
	if err != nil {
		knowledge.ParseStatus = types.ParseStatusFailed
		knowledge.ErrorMessage = err.Error()
		knowledge.UpdatedAt = time.Now()
		...
		s.failStage(ctx, knowledge.ID, types.StageChunking,
			werrors.ErrCodeChunkingFailed, "sync chunks failed", err)
		return fmt.Errorf("sync reparse chunks: %w", err)
	}
	baseChunksChanged := chunkSync.Created > 0 || chunkSync.Updated > 0 || len(chunkSync.StaleIDs) > 0
	refreshImageDerived := options.EnableMultimodel && len(options.StoredImages) > 0
	derivedStaleIDs, err := s.collectReparseStaleDerivedChunkIDs(ctx, tenantInfo.ID, knowledge.ID, baseChunksChanged, refreshImageDerived)
	if err != nil {
		knowledge.ParseStatus = types.ParseStatusFailed
		knowledge.ErrorMessage = err.Error()
		knowledge.UpdatedAt = time.Now()
		s.repo.UpdateKnowledge(ctx, knowledge)
		s.failStage(ctx, knowledge.ID, types.StageChunking,
			werrors.ErrCodeChunkingFailed, "collect stale derived chunks failed", err)
		return fmt.Errorf("collect stale derived chunks: %w", err)
	}
	allStaleChunkIDs := mergeUniqueIDs(chunkSync.StaleIDs, derivedStaleIDs)
	totalChunkChars := 0
	for _, c := range insertChunks {
		totalChunkChars += len(c.Content)
	}
	s.endStage(ctx, knowledge.ID, types.StageChunking, types.JSONMap{
		"chunks_planned":        chunkSync.Planned,
		"chunks_created":        chunkSync.Created,
		"chunks_updated":        chunkSync.Updated,
		"chunks_reused":         chunkSync.Reused,
		"chunks_deleted":        chunkSync.Deleted,
		"extra_chunks_retained": chunkSync.ExtraKept,
	...
	if kb.NeedsEmbeddingModel() && embeddingModel != nil {
		...
		if tenantInfo.StorageQuota > 0 {
			...
			if err != nil {
				...
				knowledge.UpdatedAt = time.Now()
				s.repo.UpdateKnowledge(ctx, knowledge)
				return fmt.Errorf("get tenant for quota check: %w", err)
			}
		...
		if attemptSuperseded(ctx, s.tracker(), knowledge.ID, attemptFromCtx(ctx)) {
			logger.Infof(ctx, "processChunks: attempt %d superseded for %s before indexing",
				attemptFromCtx(ctx), knowledge.ID)
			return nil
		}
		...
		if err != nil {
			...
			return fmt.Errorf("batch index chunks: %w", err)
		}
		...
		if err := deleteReparseStaleVectors(ctx, retrieveEngine, embeddingModel, allStaleChunkIDs, kb.Type); err != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = err.Error()
			knowledge.UpdatedAt = time.Now()
			s.repo.UpdateKnowledge(ctx, knowledge)
			s.failStage(ctx, knowledge.ID, types.StageEmbedding,
				werrors.ErrCodeVectorStoreWriteFailed, "delete stale vectors failed", err)
		...
		if attemptSuperseded(ctx, s.tracker(), knowledge.ID, attemptFromCtx(ctx)) {
			logger.Infof(ctx, "processChunks: attempt %d superseded for %s after indexing",
				attemptFromCtx(ctx), knowledge.ID)
			return nil
		}
	} else {
	...
	if len(allStaleChunkIDs) > 0 {
		if err := s.deleteReparseStaleChunks(ctx, tenantInfo.ID, allStaleChunkIDs); err != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = err.Error()
			knowledge.UpdatedAt = time.Now()
			s.repo.UpdateKnowledge(ctx, knowledge)
	...
	if (len(chunkSync.StaleIDs) > 0 || chunkSync.Updated > 0) && s.graphEngine != nil {
		...
		if err := s.graphEngine.DelGraph(ctx, []types.NameSpace{namespace}); err != nil {
			...
			knowledge.UpdatedAt = time.Now()
			s.repo.UpdateKnowledge(ctx, knowledge)
			s.failStage(ctx, knowledge.ID, types.StagePostProcess,
				werrors.ErrCodeUnknown, "delete stale graph failed", err)
			return fmt.Errorf("delete stale graph: %w", err)
		}
	}

	if attemptSuperseded(ctx, s.tracker(), knowledge.ID, attemptFromCtx(ctx)) {
		logger.Infof(ctx, "processChunks: attempt %d superseded for %s before finalizing",
			attemptFromCtx(ctx), knowledge.ID)
		return nil
	}
	...
	if err := s.repo.UpdateKnowledge(ctx, knowledge); err != nil {
		...
		return fmt.Errorf("update knowledge after indexing: %w", err)
	}
	...
	if options.EnableMultimodel && len(options.StoredImages) > 0 {
		...
		if err := s.enqueueImageMultimodalTasks(ctx, knowledge, kb, options.StoredImages, chunks, options.Metadata); err != nil {
			knowledge.ParseStatus = types.ParseStatusFailed
			knowledge.ErrorMessage = err.Error()
			knowledge.UpdatedAt = time.Now()
			s.repo.UpdateKnowledge(ctx, knowledge)
			s.failStage(ctx, knowledge.ID, types.StageMultimodal,
				werrors.ErrCodeUnknown, "enqueue multimodal tasks failed", err)
			return fmt.Errorf("enqueue multimodal tasks: %w", err)
		}
	} else {
	...
	// Update tenant storage usage by delta. Reparse replaces the old index
	// footprint; unchanged content should not charge the tenant a second time.
	storageDelta := totalStorageSize - previousStorageSize
	if storageDelta != 0 {
		...
		if err := s.tenantRepo.AdjustStorageUsed(ctx, tenantInfo.ID, storageDelta); err != nil {
			...
			return fmt.Errorf("adjust tenant storage: %w", err)
		}
	}
...
func (s *knowledgeService) getSummary(ctx context.Context,
	...
	summary, err := summaryModel.Chat(modelCtx, messages, opts)
	if err != nil {
...
func (s *knowledgeService) ProcessSummaryGeneration(ctx context.Context, t *asynq.Task) (retErr error) {
	...
	if attemptSuperseded(ctx, s.tracker(), payload.KnowledgeID, payload.Attempt) {
		superseded = true
		summaryOut["skipped"] = "superseded"
		logger.Infof(ctx, "summary: attempt %d superseded for %s after LLM, discarding stale output",
			payload.Attempt, payload.KnowledgeID)
		return nil
	}
	...
	if strings.TrimSpace(summary) != "" && kb.NeedsEmbeddingModel() {
		...
		summaryChunk := &types.Chunk{
			ID: contentcache.StableChunkID(contentcache.ChunkIDInput{
				...
				Occurrence:  1,
				Content:     summary,
				ParentID:    textChunks[0].ID,
			}),
			TenantID:        knowledge.TenantID,
			KnowledgeID:     knowledge.ID,
			KnowledgeBaseID: knowledge.KnowledgeBaseID,
			Content:         fmt.Sprintf("# Summary\n%s", summary),
			ContentHash:     contentcache.ChunkContentHash(summary, "# Summary"),
		...
		// Save summary chunk
		summaryCreated, summaryUpdated, err := s.upsertStableChunk(ctx, payload.TenantID, summaryChunk)
		if err != nil {
			logger.Errorf(ctx, "Failed to create summary chunk: %v", err)
			summaryErr = err
			return fmt.Errorf("failed to create summary chunk: %w", err)
		}
		logger.Infof(ctx, "Summary chunk saved: created=%t updated=%t reused=%t",
			summaryCreated, summaryUpdated, !summaryCreated && !summaryUpdated)
...
func (s *knowledgeService) processQuestionGenerationForKnowledge(ctx context.Context, t *asynq.Task, payload types.QuestionGenerationPayload) (retErr error) {
	...
	for i, chunk := range textChunks {
		...
		if attemptSuperseded(ctx, s.tracker(), payload.KnowledgeID, payload.Attempt) {
			superseded = true
			exitStatus = "superseded"
			logger.Infof(ctx, "question: attempt %d superseded for %s after LLM, discarding stale questions",
				payload.Attempt, payload.KnowledgeID)
			return nil
		}
		...
		for j, question := range questions {
			questionID := stableGeneratedQuestionID(chunk.ID, question)
	...
	if len(indexInfoList) > 0 {
		if attemptSuperseded(ctx, s.tracker(), payload.KnowledgeID, payload.Attempt) {
			superseded = true
			exitStatus = "superseded"
			logger.Infof(ctx, "question: attempt %d superseded for %s before indexing, discarding stale questions",
				payload.Attempt, payload.KnowledgeID)
			return nil
		}
...
func (s *knowledgeService) processQuestionGenerationForChunks(ctx context.Context, t *asynq.Task, payload types.QuestionGenerationPayload) (retErr error) {
	...
	for i, chunk := range batchChunks {
		...
		if attemptSuperseded(ctx, s.tracker(), payload.KnowledgeID, payload.Attempt) {
			superseded = true
			exitStatus = "superseded"
			logger.Infof(ctx, "question: attempt %d superseded for %s after LLM, discarding stale batch %d",
				payload.Attempt, payload.KnowledgeID, payload.BatchIndex)
			return nil
		}
		latestChunk, latestErr := s.chunkRepo.GetChunkByID(ctx, payload.TenantID, chunk.ID)
		if latestErr != nil || latestChunk.ContentRevision != generationRevision {
		...
		for j, question := range questions {
			generatedQuestions[j] = types.GeneratedQuestion{
				ID:              stableGeneratedQuestionID(chunk.ID, question),
				Question:        question,
				ContentRevision: &questionRevision,
			}
		}
	...
	if len(indexInfoList) > 0 {
		if attemptSuperseded(ctx, s.tracker(), payload.KnowledgeID, payload.Attempt) {
			superseded = true
			exitStatus = "superseded"
			logger.Infof(ctx, "question: attempt %d superseded for %s before indexing batch %d",
				payload.Attempt, payload.KnowledgeID, payload.BatchIndex)
			return nil
		}
		if err := retrieveEngine.BatchIndex(ctx, embeddingModel, indexInfoList); err != nil {
...
func (s *knowledgeService) ReparseKnowledge(
	...
	markReparseEnqueueFailed := func(err error) {
		existing.ParseStatus = types.ParseStatusFailed
		existing.ErrorMessage = "Failed to enqueue processing task"
		existing.UpdatedAt = time.Now()
		if updateErr := s.repo.UpdateKnowledge(ctx, existing); updateErr != nil {
			logger.Warnf(ctx, "Failed to persist reparse enqueue failure for %s: %v (original error: %v)",
				existing.ID, updateErr, err)
		}
	}
	...
	if existing.IsManual() {
		...
		if _, err := s.enqueueManualProcessing(ctx, existing, meta.Content, false); err != nil {
			logger.Errorf(ctx, "Failed to enqueue manual reparse task: %v", err)
			markReparseEnqueueFailed(err)
...
func (s *knowledgeService) deleteChunkVectors(ctx context.Context, kbID string, ids []string) error {
	if len(ids) == 0 {
		return nil
	}
	sourceKB, err := s.kbService.GetKnowledgeBaseByID(ctx, kbID)
	...
	if sourceKB == nil || !sourceKB.NeedsEmbeddingModel() {
		return nil
	}
	embeddingModel, err := s.modelService.GetEmbeddingModel(ctx, sourceKB.EmbeddingModelID)
	if err != nil {
	...
	return retrieveEngine.DeleteByChunkIDList(ctx, ids, embeddingModel.GetDimensions(), sourceKB.Type)
}
...
func (s *knowledgeService) UpdateImageInfo(
	...
	newImageChildChunk := func(chunkType string, content string) *types.Chunk {
		return &types.Chunk{
			ID: contentcache.StableChunkID(contentcache.ChunkIDInput{
				KnowledgeID: chunk.KnowledgeID,
				ChunkType:   chunkType,
	...
	for _, child := range chunkChildren {
		...
		case types.ChunkTypeImageCaption:
			...
			if image.Caption != cImageInfo[0].Caption {
				deleteChunkIDs = append(deleteChunkIDs, child.ID)
				if image.Caption != "" {
					addChunk = append(addChunk, newImageChildChunk(types.ChunkTypeImageCaption, image.Caption))
				}
			}
		case types.ChunkTypeImageOCR:
			...
			if image.OCRText != cImageInfo[0].OCRText {
				deleteChunkIDs = append(deleteChunkIDs, child.ID)
				if image.OCRText != "" {
					addChunk = append(addChunk, newImageChildChunk(types.ChunkTypeImageOCR, image.OCRText))
				}
			}
		}
	}
	...
	if !hasCaptionChunk && image.Caption != "" {
		captionChunk := newImageChildChunk(types.ChunkTypeImageCaption, image.Caption)
	...
	if !hasOCRChunk && image.OCRText != "" {
		ocrChunk := newImageChildChunk(types.ChunkTypeImageOCR, image.OCRText)
	...
	if len(deleteChunkIDs) > 0 {
		if err := s.deleteChunkVectors(ctx, chunk.KnowledgeBaseID, deleteChunkIDs); err != nil {
			return err
		}
		if err := s.chunkRepo.HardDeleteChunks(ctx, tenantID, deleteChunkIDs); err != nil {
			return err
		}
	}

	if len(addChunk) > 0 {
		_, err := upsertStableChunks(ctx, s.chunkRepo, tenantID, addChunk)
		if err != nil {
			logger.ErrorWithFields(ctx, err, map[string]interface{}{
...
func (s *knowledgeService) ProcessDocument(ctx context.Context, t *asynq.Task) error {
	...
	ctx = context.WithValue(ctx, types.TenantIDContextKey, payload.TenantID)
	if attemptSuperseded(ctx, s.tracker(), payload.KnowledgeID, payload.Attempt) {
		logger.Infof(ctx, "ProcessDocument: attempt %d superseded for %s, skipping stale task",
			payload.Attempt, payload.KnowledgeID)
		return nil
	}
	if payload.Language != "" {
	...
	if err := s.repo.UpdateKnowledge(ctx, knowledge); err != nil {
		...
		return fmt.Errorf("update knowledge %s to processing: %w", knowledge.ID, err)
	}
...
func (s *knowledgeService) enqueueImageMultimodalTasks(
	...
	if s.redisClient != nil {
		if err := s.redisClient.Set(ctx, redisKey, len(images), 24*time.Hour).Err(); err != nil {
			return fmt.Errorf("set multimodal pending count: %w", err)
		}
	}
	...
	for idx, img := range images {
		...
		if err != nil {
			enqueueErrs = append(enqueueErrs, fmt.Errorf("marshal image multimodal payload %d: %w", idx, err))
			continue
		}
		...
		if _, err := s.task.Enqueue(task); err != nil {
			enqueueErrs = append(enqueueErrs, fmt.Errorf("enqueue image multimodal task %d: %w", idx, err))
		} else {
