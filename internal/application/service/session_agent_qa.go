...
func (s *sessionService) buildAgentConfig(
	...
	searchTargets, err := s.buildSearchTargets(
		ctx, agentTenantID, agentConfig.KnowledgeBases, agentConfig.KnowledgeIDs, req.TagScopes, req.FolderScopes,
	)
	if err != nil {
	...
	// Folder scopes are resolved the same way and need the same treatment.
...
// mergeResolvedScopeKnowledgeIDs lifts the documents that a tag or folder scope
// resolved to into the agent's pinned knowledge IDs, so the model sees the same
// selection the retrieval layer was constrained to.
