...
// CitationTaskHandler consumes the off-path citation snapshot. It uses the
// explicit payload scope because background workers do not inherit request
// principals.
type CitationTaskHandler struct {
	repo interfaces.MasteryRepository
}

func NewCitationTaskHandler(repo interfaces.MasteryRepository) interfaces.TaskHandler {
	return &CitationTaskHandler{repo: repo}
}

func (h *CitationTaskHandler) Handle(ctx context.Context, task *asynq.Task) error {
	var payload types.MasteryCitationPayload
	if err := json.Unmarshal(task.Payload(), &payload); err != nil {
		return fmt.Errorf("unmarshal mastery citation payload: %w", err)
	}
...
func NewMasteryService(repo interfaces.MasteryRepository, enqueuers ...interfaces.TaskEnqueuer) interfaces.MasteryService {
...
func (s *Service) RecordCitations(ctx context.Context, refs []types.MemoryDocAffinity) {
	...
	if _, err := s.enqueuer.Enqueue(asynq.NewTask(types.TypeMasteryCitation, body), asynq.Queue(types.QueueMemory), asynq.MaxRetry(3)); err != nil {
