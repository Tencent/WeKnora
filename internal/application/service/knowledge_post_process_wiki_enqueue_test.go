...
type fixedLatestAttemptTracker struct {
	noopSpanTracker
	latest int
}

func (t fixedLatestAttemptTracker) LatestAttempt(context.Context, string) int {
	return t.latest
}
...
func TestKnowledgePostProcessSkipsSupersededAttempt(t *testing.T) {
	payload, err := json.Marshal(types.KnowledgePostProcessPayload{
		TenantID:        7,
		KnowledgeID:     "knowledge-superseded",
	...
	service := &KnowledgePostProcessService{
		spanTracker: fixedLatestAttemptTracker{latest: 2},
	}
	require.NoError(t, service.Handle(context.Background(), asynq.NewTask(types.TypeKnowledgePostProcess, payload)))
}
