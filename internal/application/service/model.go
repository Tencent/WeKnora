...
// resolveTreeRAGCloudCredentials 为 TreeRAGCloud 厂商模型补全 AppID/AppSecret。
// 当模型自身参数中未存储凭证时，自动从空间配置中获取（SaveCredentials 保存的凭证）。
func (s *modelService) resolveTreeRAGCloudCredentials(ctx context.Context, params *types.ModelParameters) (appID, appSecret string) {
	appID = params.AppID
	appSecret = s.decryptAppSecret(params.AppSecret)

	if provider.ProviderName(params.Provider) != provider.ProviderTreeRAGCloud {
		return
	}
	if appID != "" && appSecret != "" {
		return
	...
	creds := s.tenantService.GetTreeRAGCloudCredentials(ctx)
	if creds == nil {
		return
	}
	if appID == "" {
...
func (s *modelService) GetEmbeddingModel(ctx context.Context, modelId string) (embedding.Embedder, error) {
	...
	appID, appSecret := s.resolveTreeRAGCloudCredentials(ctx, &model.Parameters)
