...
func (s *knowledgeService) triggerManualProcessing(ctx context.Context,
	...
	if doSync {
		if err := s.processChunks(ctx, kb, knowledge, parsed, opts); err != nil {
			logger.Errorf(ctx, "triggerManualProcessing: process chunks failed for %s: %v", knowledge.ID, err)
		}
		return
	}
	...
	go func() {
		if err := s.processChunks(newCtx, kb, knowledge, parsed, opts); err != nil {
			logger.Errorf(newCtx, "triggerManualProcessing: async process chunks failed for %s: %v", knowledge.ID, err)
		}
	}()
}
