...
// organizeBatchSize bounds each organize-by-path work-queue fetch.
...
// NewKnowledgeFolderService creates the folder tree service for document
// knowledge bases. Mirrors the wiki folder service design (issue #1311).
...
// validateKnowledgeFolderName trims and rejects blank names or names carrying
...
// cleanFolderPathSegments normalizes a relative-path chain: trims each
// segment, drops empties (so "/a//b/" folds to ["a","b"]), and rejects
// nothing — invalid separators cannot occur inside a segment by construction.
...
// findOrCreateFolderPath is the counting variant backing both the public
// method and OrganizeByPath. Concurrency-safe against the unique
// (kb, parent, name) constraint via a re-fetch on create conflict.
func (s *knowledgeFolderService) findOrCreateFolderPath(
	ctx context.Context, kbID string, tenantID uint64, baseFolderID string, path []string,
) (string, int64, error) {
...
// (moving a folder into itself or one of its descendants), sibling name
// collisions, and the depth cap. Documents only reference folder ids, so no
// document rows need touching.
...
// DeleteFolder removes a folder. Without promote it fails on any live content
// (the repository re-checks emptiness atomically inside the delete). With
// promote, direct documents and child folders are relocated to the parent
// first — documents are never deleted by this path.
...
// OrganizeByPath files root-level documents whose file_name carries a relative
// path (the folder-upload convention, e.g. "reports/2026/q1.pdf") into folders
// derived from that path. file_name itself is left untouched. Idempotent:
// filed documents drop out of the work queue, and a second run is a no-op.
...
// ExpandFolderSubtrees returns the given live folder ids plus all their
// descendants, walking the parent adjacency (not the materialized path) so
// the expansion cannot be confused by look-alike path prefixes.
...
// ListKnowledgeIDsByFolderIDs resolves folder scopes to document IDs. An empty
// result means an empty scope — callers must never widen it to "no filter".
...
// DeleteFoldersByKnowledgeBase drops the whole folder tree of a KB and parks
// every document back at the root, so a document whose async delete later
// fails can never point at a dead folder.
