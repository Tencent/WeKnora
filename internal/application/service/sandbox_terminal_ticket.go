...
// IssueSandboxBrowserTicket mints a browser-only session handshake ticket.
func IssueSandboxBrowserTicket(
	userID string, tenantID uint64, sessionID, tokenID string, ttl time.Duration,
) (string, error) {
	return issueSandboxTicket(userID, tenantID, sessionID, tokenID, ttl, "sandbox_browser")
}
...
// ParseSandboxBrowserTicket rejects non-browser handshake tickets.
func ParseSandboxBrowserTicket(raw string) (*SandboxTerminalTicketClaims, error) {
	return parseSandboxTicket(raw, "sandbox_browser")
}
