...
func (s *ImageMultimodalService) Handle(ctx context.Context, task *asynq.Task) error {
	...
	// Wrap with VLM cache — same image bytes + prompt → cached OCR/Caption result
	vlmModel = vlm.NewCachedVLM(vlmModel, s.vlmCache)
	...
	if imageInfo.OCRText != "" {
		newChunks = append(newChunks, &types.Chunk{
			ID:              StableChunkID(payload.KnowledgeID, 0, imageInfo.OCRText),
			TenantID:        payload.TenantID,
			KnowledgeID:     payload.KnowledgeID,
			KnowledgeBaseID: payload.KnowledgeBaseID,
			Content:         imageInfo.OCRText,
			ContentHash:     ComputeContentHash(payload.KnowledgeID, 0, imageInfo.OCRText),
	...
	if imageInfo.Caption != "" {
		newChunks = append(newChunks, &types.Chunk{
			ID:              StableChunkID(payload.KnowledgeID, 1, imageInfo.Caption),
			TenantID:        payload.TenantID,
			KnowledgeID:     payload.KnowledgeID,
			KnowledgeBaseID: payload.KnowledgeBaseID,
			Content:         imageInfo.Caption,
			ContentHash:     ComputeContentHash(payload.KnowledgeID, 1, imageInfo.Caption),
