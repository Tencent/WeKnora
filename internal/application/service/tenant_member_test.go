...
func newServiceWithRepo() (interfaces.TenantMemberService, *fakeTenantMemberRepo) {
	r := newFakeRepo()
	// Audit / user / token / org-unit dependencies are intentionally nil —
	// these tests exercise membership invariants only. The service's audit,
	// org-scope, and RemoveMember-cleanup hooks are nil-safe.
...
func (r *cleanupUserRepo) CountUsers(context.Context) (int64, error) { return 0, nil }
...
func TestTenantMemberService_RemoveMember_ClearsStaleHomeAndRevokesTokens(t *testing.T) {
	...
	svc := NewTenantMemberService(memberRepo, nil, nil, userRepo, tokenRepo)
