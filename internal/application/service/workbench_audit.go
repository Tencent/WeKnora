...
func (s *WorkbenchService) beginAudit(ctx context.Context, session *types.Session, operation string, details map[string]any) (*workbenchAudit, error) {
	if s.deps.Audit == nil {
		return nil, ErrWorkbenchAudit
	}
	a := &workbenchAudit{service: s, identity: WorkbenchIdentity{TenantID: session.TenantID, UserID: session.UserID, SessionID: session.ID}, id: uuid.NewString(), operation: operation, started: time.Now()}
...
func (a *workbenchAudit) write(ctx context.Context, outcome types.AuditOutcome, details map[string]any) error {
	...
	entry := &types.AuditLog{TenantID: a.identity.TenantID, ActorUserID: a.identity.UserID,
		Action: types.AuditAction(workbenchAuditScope + "." + a.operation), ScopeType: workbenchAuditScope, ScopeID: a.identity.SessionID,
		TargetType: "execution", TargetID: a.id, Outcome: outcome, Details: types.JSON(data), CreatedAt: time.Now().UTC()}
...
func (a *workbenchAudit) finish(exitCode int, reason string, operationErr error) error {
	a.once.Do(func() {
		exit := NormalizeWorkbenchTerminalExit(sandbox.TerminalExit{ExitCode: exitCode, Reason: reason}, operationErr)
		...
		defer cancel()
		a.err = a.write(ctx, outcome, map[string]any{"execution_id": a.id, "session_id": a.identity.SessionID,
...
func workbenchExitReason(reason string) string {
	switch reason {
	case "exited", "completed", "timeout", "interrupted", "canceled", "killed", "output_limit", "cpu_limit", "memory_limit", "closed", "lease_lost", "signal", "unknown", "error":
		return reason
	default:
		return "unknown"
	}
...
func NormalizeWorkbenchTerminalExit(exit sandbox.TerminalExit, operationErr error) sandbox.TerminalExit {
...
func (s *WorkbenchService) Audit(ctx context.Context, sessionID string, limit int) ([]*types.AuditLog, error) {
	...
	rows, err := s.deps.Audit.List(ctx, session.TenantID, &interfaces.AuditLogQuery{Limit: limit, ScopeType: workbenchAuditScope, ScopeID: sessionID, ActorUserID: session.UserID})
	if err != nil {
		return nil, ErrWorkbenchUnavailable
	}
	result := make([]*types.AuditLog, 0, len(rows))
	for _, row := range rows {
		if row != nil && row.TenantID == session.TenantID && row.ScopeType == workbenchAuditScope && row.ScopeID == sessionID && row.ActorUserID == session.UserID {
...
func (e *WorkbenchExecution) Finish(exit sandbox.TerminalExit, err error) error {
	return e.audit.finish(exit.ExitCode, exit.Reason, err)
}

func (s *WorkbenchService) OpenTerminal(ctx context.Context, sessionID string, request sandbox.TerminalRequest) (*WorkbenchExecution, error) {
	if strings.TrimSpace(request.Command) == "" || len(request.Command) > WorkbenchMaxCommandBytes || strings.ContainsRune(request.Command, 0) || !utf8.ValidString(request.Command) || request.Cols == 0 || request.Rows == 0 || request.Cols > 500 || request.Rows > 500 {
	...
	provider, ok := sandbox.TerminalProviderFrom(mgr)
	if !ok {
	...
	terminal, err := provider.OpenSessionTerminal(types.WithSandboxTenantID(ctx, session.TenantID), sessionID, request)
...
func (s *WorkbenchService) Files(ctx context.Context, sessionID string, request sandbox.WorkbenchFileRequest) (*sandbox.WorkbenchFileResult, error) {
	...
	if request.Operation != "list" && request.Operation != "read" {
		audit, err = s.beginAudit(ctx, session, request.Operation, map[string]any{"path": request.Path, "new_path": request.NewPath})
		if err != nil {
			return nil, err
		}
	}
