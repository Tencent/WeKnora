...
func (c *Client) do(ctx context.Context, method, path string, query url.Values, body any, out any) error {
	if c.endpoint == "" {
		return fmt.Errorf("Cube 未配置 (CUBE_API_URL / CUBEJS_API_SECRET)")
	}
	var rdr io.Reader
	if body != nil {
	...
	if err != nil {
		return fmt.Errorf("Cube 请求失败: %w", err)
	}
	...
	if resp.StatusCode >= 400 {
		...
		if json.Unmarshal(data, &errResp) == nil && errResp.Error != "" {
			return fmt.Errorf("Cube 错误: %s", errResp.Error)
		}
		return fmt.Errorf("Cube 请求失败: HTTP %d: %s", resp.StatusCode, truncate(string(data), 512))
	}
	if out != nil {
		if err := json.Unmarshal(data, out); err != nil {
			return fmt.Errorf("Cube 响应解析失败: %w", err)
		}
	}
	return nil
}
...
func (c *Client) Load(ctx context.Context, q Query) (*LoadResponse, error) {
	...
	if out.Error != "" {
		return nil, fmt.Errorf("Cube 错误: %s", out.Error)
	}
	return &out, nil
}
...
func (c *Client) WaitUntilCompiled(ctx context.Context, modelName string, timeout time.Duration) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			if err != nil {
				return fmt.Errorf("等待模型编译超时 (最后错误: %v); 请检查 Cube 日志", err)
			}
			return fmt.Errorf("等待模型 %s 编译超时 (%s); 模型可能有语法错误, 请检查 Cube 日志", modelName, timeout)
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
