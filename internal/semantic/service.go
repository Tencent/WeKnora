...
func (e *Engine) audit(ctx context.Context, tenantID uint64, userID, action, target string, detail map[string]interface{}) {
	var raw types.JSON
	if detail != nil {
...
func (e *Engine) CreateConnection(ctx context.Context, userID string, tenant uint64, in *ConnectionInput) (*CubeConnection, error) {
...
func (e *Engine) UpdateConnection(ctx context.Context, userID string, tenant uint64, id string, in *ConnectionInput) (*CubeConnection, error) {
		...
		if models, _ := e.repo.ListModels(ctx, tenant); len(models) > 0 {
			for _, m := range models {
				if m.ConnectionID == conn.ID && m.Status == ModelStatusPublished {
					return nil, fmt.Errorf("connection is used by a published model; unpublish the relevant models before changing its type")
				}
			}
		}
		conn.Type = in.Type
...
func (e *Engine) TestConnectionRaw(ctx context.Context, in *ConnectionInput) (map[string]interface{}, error) {
	...
	if _, ok := dbinspector.Get(in.Type); !ok {
		return nil, fmt.Errorf("type %s does not support guided connection testing; for passthrough types, save it and verify through the publish flow", in.Type)
	}
...
func (e *Engine) TestConnectionDraft(ctx context.Context, tenant uint64, id string, in *ConnectionInput) (map[string]interface{}, error) {
	conn, err := e.repo.FindConnection(ctx, tenant, id)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) ListColumns(ctx context.Context, tenant uint64, id, schema, table string) ([]dbinspector.ColumnSchema, error) {
	conn, err := e.repo.FindConnection(ctx, tenant, id)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) CreateModel(ctx context.Context, userID string, tenant uint64, in *SaveModelInput) (*SemanticModel, error) {
...
func (e *Engine) UpdateModel(ctx context.Context, userID string, tenant uint64, id string, in *SaveModelInput) (*SemanticModel, error) {
...
func (e *Engine) Publish(ctx context.Context, userID string, tenant uint64, id string, note string) (*PublishResult, error) {
	m, err := e.repo.FindModel(ctx, tenant, id)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) Rollback(ctx context.Context, userID string, tenant uint64, modelID, versionID string) (*PublishResult, error) {
...
func (e *Engine) filterMetaForUser(ctx context.Context, tenantID uint64, userID string, meta *cubeclient.MetaResponse) *cubeclient.MetaResponse {
...
func (e *Engine) Preview(ctx context.Context, userID string, tenant uint64, modelID string, q *PreviewQuery) (*cubeclient.LoadResponse, error) {
	m, err := e.repo.FindModel(ctx, tenant, modelID)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) QueryForUser(ctx context.Context, tenantID uint64, userID string, q *PreviewQuery) (*cubeclient.LoadResponse, error) {
	uctx, err := e.WithUserContext(ctx, tenantID, userID)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) SQLForUser(ctx context.Context, tenantID uint64, userID string, q *PreviewQuery) (*cubeclient.SQLResponse, error) {
	uctx, err := e.WithUserContext(ctx, tenantID, userID)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) UpdateGroup(ctx context.Context, userID string, tenant uint64, id string, in *DataGroup) (*DataGroup, error) {
...
func (e *Engine) SetGroupMembers(ctx context.Context, userID string, tenant uint64, groupID string, userIDs []string) error {
	g, err := e.repo.FindGroup(ctx, tenant, groupID)
	if err != nil {
		return err
	}
