...
// KnowledgeFolderHandler exposes the multi-level folder tree of a document
// knowledge base (issue #1311).
//
// All KB-access checks (own / org-shared / via shared agent) are performed by
// the route-level g.KBAccessRead / g.KBAccessWrite guards in router.go — the
// guard rewrites c.Request.Context() to carry the effective tenant ID, so the
// handlers below just use c.Request.Context() directly.
