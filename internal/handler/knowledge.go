...
func (h *KnowledgeHandler) CreateKnowledgeFromFile(c *gin.Context) {
	...
	// Place the document into its folder: the explicit folder_id is the base,
	// and a relative path in the custom filename (folder uploads) extends it.
...
// fileKnowledgeIntoFolder places a freshly created document into a folder.
// baseFolderID is the user-selected upload target ("" = root); when fileName
// carries a relative path (the folder-upload convention, preserved verbatim
// in file_name), the missing chain is created beneath that base. Best-effort:
// a stale folder id must not fail an upload whose file already parsed — the
// document stays at the root and organize-by-path can file it later.
...
func (h *KnowledgeHandler) ListKnowledge(c *gin.Context) {
	...
	if folderID := strings.TrimSpace(c.Query("folder_id")); folderID != "" {
		...
		// Recursive listing expands the folder subtree here so the repository
		// stays tree-agnostic. The root sentinel never expands: "root and all
		// descendants" is simply "no folder filter".
...
func (h *KnowledgeHandler) ClearKnowledgeBaseContents(c *gin.Context) {
	...
	// Folders are KB content too: drop the tree and park every document back
	// at the root, so a document whose async delete fails cannot keep pointing
	// at a dead folder. Best-effort — the doc deletion task is already queued.
