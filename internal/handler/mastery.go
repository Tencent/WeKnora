...
func (h *MasteryHandler) RecordPageView(c *gin.Context) {
	...
	if h.wikiService != nil {
		page, err := h.wikiService.GetPageBySlug(c.Request.Context(), kbID, req.Slug)
		if err != nil || page == nil {
	...
	h.masteryService.RecordPageView(c.Request.Context(), kbID, req.Slug, req.Duration, neighbors)
	resp := gin.H{"success": true}
	// 回传该节点的最新状态，前端就地更新水位球，无需重拉整张图。
	if st, err := h.masteryService.NodeStateForSlug(c.Request.Context(), kbID, req.Slug, sourceKnowledgeIDs); err == nil {
...
func (h *MasteryHandler) ExposureClick(c *gin.Context) {
	...
	kbID := c.Param("kb_id")
	if kbID == "" || req.CandidateSlug == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "kb_id and candidate_slug are required"})
		return
	}
	h.masteryService.MarkExposureClicked(c.Request.Context(), kbID, req.CandidateSlug)
	c.JSON(http.StatusOK, gin.H{"success": true})
}
...
func (h *MasteryHandler) RecordExposure(c *gin.Context) {
	...
	kbID := c.Param("kb_id")
	if kbID == "" || req.TriggerSlug == "" || len(req.CandidateSlugs) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "kb_id, trigger_slug and candidate_slugs are required"})
		return
	}
	h.masteryService.RecordExposures(c.Request.Context(), kbID, req.TriggerSlug, req.CandidateSlugs)
	c.JSON(http.StatusOK, gin.H{"success": true})
}
