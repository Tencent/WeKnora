-- Mirrors versioned migration 000091_evaluation_task_cancellation.

ALTER TABLE evaluation_tasks
    ADD COLUMN cancel_requested_at DATETIME;

CREATE TABLE IF NOT EXISTS evaluation_tasks_cancel_guard (
    id                     TEXT PRIMARY KEY,
...
INSERT INTO evaluation_tasks_cancel_guard (
    ...
    lease_expires_at, heartbeat_at, version, created_at, updated_at, deleted_at, cancel_requested_at
)
SELECT
    id, tenant_id, dataset_id, status, start_time, end_time, total, finished, err_msg,
    cleanup_errors, params, metric, temporary_kb_id, temporary_knowledge_id, owner_id,
...
ALTER TABLE evaluation_tasks_cancel_guard RENAME TO evaluation_tasks;
