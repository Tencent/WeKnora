...
DO $$ BEGIN RAISE NOTICE '[Migration 000066] Rolling back chunk feedback setup...'; END $$;

-- ============================================
-- 1. 回滚 messages 表的 feedback_count 字段
-- ============================================
DO $$ BEGIN RAISE NOTICE '[Migration 000066] Rolling back messages table...'; END $$;

ALTER TABLE messages DROP COLUMN IF EXISTS like_count;
ALTER TABLE messages DROP COLUMN IF EXISTS dislike_count;

DO $$ BEGIN RAISE NOTICE '[Migration 000066] messages table rolled back'; END $$;

-- ============================================
-- 2. 删除 chunk_weight_logs 表
-- ============================================
DO $$ BEGIN RAISE NOTICE '[Migration 000066] Dropping chunk_weight_logs table...'; END $$;

DROP TABLE IF EXISTS chunk_weight_logs;

DO $$ BEGIN RAISE NOTICE '[Migration 000066] chunk_weight_logs table dropped'; END $$;

-- ============================================
-- 3. 删除 chunk_feedbacks 表
-- ============================================
DO $$ BEGIN RAISE NOTICE '[Migration 000066] Dropping chunk_feedbacks table...'; END $$;

DROP TABLE IF EXISTS chunk_feedbacks;

DO $$ BEGIN RAISE NOTICE '[Migration 000066] chunk_feedbacks table dropped'; END $$;

-- ============================================
-- 4. 删除 qa_reply_chunk_refs 表
-- ============================================
DO $$ BEGIN RAISE NOTICE '[Migration 000066] Dropping qa_reply_chunk_refs table...'; END $$;

DROP TABLE IF EXISTS qa_reply_chunk_refs;

DO $$ BEGIN RAISE NOTICE '[Migration 000066] qa_reply_chunk_refs table dropped'; END $$;

-- ============================================
-- 5. 回滚 chunks 表
-- ============================================
DO $$ BEGIN RAISE NOTICE '[Migration 000066] Rolling back chunks table...'; END $$;
...
DO $$ BEGIN RAISE NOTICE '[Migration 000066] chunks table rolled back'; END $$;

DO $$ BEGIN RAISE NOTICE '[Migration 000066] Rollback completed successfully'; END $$;
