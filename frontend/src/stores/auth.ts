...
export const useAuthStore = defineStore('auth', () => {
  ...
  //
  // SECURITY: This value is read from localStorage (`gridora_memberships`)
  ...
  const setUser = (userData: UserInfo) => {
    ...
    user.value = userData
    // 保存到localStorage
    localStorage.setItem('gridora_user', JSON.stringify(userData))
  ...
  const setTenant = (tenantData: TenantInfo | null) => {
    ...
    if (tenantData) {
      localStorage.setItem('gridora_tenant', JSON.stringify(tenantData))
    } else {
      localStorage.removeItem('gridora_tenant')
  ...
  const setToken = (tokenValue: string) => {
    ...
    localStorage.setItem('gridora_token', tokenValue)
  }

  const setRefreshToken = (refreshTokenValue: string) => {
    refreshToken.value = refreshTokenValue
    localStorage.setItem('gridora_refresh_token', refreshTokenValue)
  }

  const setKnowledgeBases = (kbList: KnowledgeBaseInfo[]) => {
    // 确保输入是数组
    knowledgeBases.value = Array.isArray(kbList) ? kbList : []
    localStorage.setItem('gridora_knowledge_bases', JSON.stringify(knowledgeBases.value))
  }

  const setCurrentKnowledgeBase = (kb: KnowledgeBaseInfo | null) => {
    currentKnowledgeBase.value = kb
    if (kb) {
      localStorage.setItem('gridora_current_kb', JSON.stringify(kb))
    } else {
      localStorage.removeItem('gridora_current_kb')
    }
  }
  ...
  const clearTenantScopedClientState = () => {
    try {
      localStorage.removeItem('gridora_last_chat_model_id')
      localStorage.removeItem('gridora_current_kb')
      const raw = localStorage.getItem('GRIDORA_settings')
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          localStorage.setItem('GRIDORA_settings', JSON.stringify(parsed))
        }
      }
    } catch (e) {
  ...
  const setSelectedTenant = (tenantId: number | null, tenantName: string | null = null) => {
    ...
    if (tenantId !== null) {
      localStorage.setItem('gridora_selected_tenant_id', String(tenantId))
      if (tenantName) {
        localStorage.setItem('gridora_selected_tenant_name', tenantName)
      }
    } else {
      localStorage.removeItem('gridora_selected_tenant_id')
      localStorage.removeItem('gridora_selected_tenant_name')
    }
    if (tenantChanged) {
  ...
  const setMemberships = (
    ...
    localStorage.setItem('gridora_memberships', JSON.stringify(memberships.value))
  }
  ...
  const setLiteMode = (value: boolean) => {
    ...
    if (value) {
      localStorage.setItem('gridora_lite_mode', 'true')
    } else {
      localStorage.removeItem('gridora_lite_mode')
    }
  }
  ...
  const logout = () => {
    ...
    // 清空localStorage
    localStorage.removeItem('gridora_user')
    localStorage.removeItem('gridora_tenant')
    localStorage.removeItem('gridora_token')
    localStorage.removeItem('gridora_refresh_token')
    localStorage.removeItem('gridora_knowledge_bases')
    localStorage.removeItem('gridora_current_kb')
    localStorage.removeItem('gridora_selected_tenant_id')
    localStorage.removeItem('gridora_selected_tenant_name')
    localStorage.removeItem('gridora_memberships')
    localStorage.removeItem('gridora_lite_mode')
    isLiteMode.value = false
    try {
      sessionStorage.removeItem('gridora_lite_last_path')
    } catch {
      /* ignore */
    }
    reloadUserPreferences()
  ...
  const initFromStorage = () => {
    // 从localStorage恢复状态
    const storedUser = localStorage.getItem('gridora_user')
    const storedTenant = localStorage.getItem('gridora_tenant')
    const storedToken = localStorage.getItem('gridora_token')
    const storedRefreshToken = localStorage.getItem('gridora_refresh_token')
    const storedKnowledgeBases = localStorage.getItem('gridora_knowledge_bases')
    const storedCurrentKb = localStorage.getItem('gridora_current_kb')
    const storedSelectedTenantId = localStorage.getItem('gridora_selected_tenant_id')
    const storedSelectedTenantName = localStorage.getItem('gridora_selected_tenant_name')
    ...
    const storedMemberships = localStorage.getItem('gridora_memberships')
    ...
    isLiteMode.value = localStorage.getItem('gridora_lite_mode') === 'true'
  }
