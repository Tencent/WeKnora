...
instance.interceptors.request.use(
  (config) => {
    ...
    // 但这条优化会被任何把 gridora_tenant 写成激活空间的代码（OIDC
    ...
    if (!isEmbedAuth && !isEmbedPath) {
      const selectedTenantId = localStorage.getItem('gridora_selected_tenant_id');
      if (selectedTenantId) {
...
instance.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (error.response.status === 401 && !originalRequest._retry && !originalRequest.url?.includes('/auth/refresh')) {
      ...
      const refreshToken = localStorage.getItem('gridora_refresh_token');
      ...
      if (refreshToken) {
        try {
          ...
          if (response.success && response.data) {
            ...
            localStorage.setItem('gridora_token', token);
            localStorage.setItem('gridora_refresh_token', newRefreshToken);
