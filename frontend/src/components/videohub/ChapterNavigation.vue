<template>
  <section class="chapters" aria-label="视频章节">
    ...
    <t-empty v-if="chapters.length === 0" description="暂无章节" />
...
const props = defineProps<{ chapters: Chapter[]; currentSeconds: number }>()
defineEmits<{ seek: [seconds: number] }>()
const chapterRefs = new Map<string, HTMLElement>()
const activeChapterId = computed(() => props.chapters.find(chapter => props.currentSeconds >= chapter.start_seconds && props.currentSeconds < chapter.end_seconds)?.id)
