<template>
  <SettingDrawer :visible="dialogVisible" :title="isEdit ? $t('model.editor.editTitle') : $t('model.editor.addTitle')"
    ...
    :confirm-disabled="formData.provider === 'gridoracloud' && wkcCredentialState !== 'configured'"
    ...
    <template v-if="formData.source === 'remote'" #footer-left>
      <t-button variant="outline" @click="checkRemoteAPI" :loading="checking"
        :disabled="!formData.modelName || (!formData.baseUrl && formData.provider !== 'gridoracloud') || (formData.provider === 'gridoracloud' && wkcCredentialState !== 'configured')">
        <template #icon>
    ...
    <t-form ref="formRef" :data="formData" :rules="rules" layout="vertical">
      ...
      <template v-if="formData.source === 'remote'">
        <section class="setting-drawer__section">
          ...
          <!-- GRIDORACloud 提示信息 -->
          <template v-if="formData.provider === 'gridoracloud'">
            <!-- 凭证已配置 -->
            <div v-if="wkcCredentialState === 'configured'" class="gridoracloud-hint gridoracloud-hint--ok">
              <t-icon name="check-circle-filled" class="hint-icon hint-icon--ok" />
              <div>
                {{ $t('settings.gridoraCloud.modelHintConfigured') }}
                <a href="https://developers.weixin.qq.com/doc/aispeech/knowledge/atomic_capability/atomic_interface.html"
                  ...
                  {{ $t('settings.gridoraCloud.modelHintDocsLink') }}
            ...
            <!-- 未配置 / 失效 -->
            <div v-else-if="wkcCredentialState !== 'loading'" class="gridoracloud-hint gridoracloud-hint--warn">
              <t-icon name="error-circle-filled" class="hint-icon hint-icon--warn" />
              <div style="flex: 1;">
                <template v-if="wkcCredentialState === 'expired'">
                  {{ $t('settings.gridoraCloud.credentialExpired') }}
                </template>
                <template v-else>
                  {{ $t('settings.gridoraCloud.credentialUnconfigured') }}
                </template>
                <div style="margin-top: 8px;">
                  <t-button variant="text" size="small" @click="goToGRIDORACloudSettings"
                    style="padding: 0; height: auto;">
                    <template #icon><t-icon name="jump" /></template>
                    {{ $t('settings.gridoraCloud.goToSettings') }}
                  </t-button>
                </div>
              </div>
            </div>
            ...
            <!-- 加载中 -->
            <div v-else class="gridoracloud-hint">
              <t-icon name="loading" class="spinning hint-icon hint-icon--loading" />
              <span>{{ $t('settings.gridoraCloud.checkingStatus') }}</span>
            </div>
          </template>
          ...
          <div class="form-item">
            ...
            <t-input v-model="formData.modelName" :placeholder="getModelNamePlaceholder()"
              :disabled="formData.provider === 'gridoracloud' && wkcCredentialState !== 'configured'" />
          </div>
          ...
          <div v-if="formData.provider !== 'gridoracloud'" class="form-item">
...
const credentialFields = computed<CredentialFieldDef<ModelCredentialField>[]>(() => {
  ...
  if (formData.value.provider === 'gridoracloud') {
...
const goToGRIDORACloudSettings = async () => {
  emit('update:visible', false)
  if (uiStore.showSettingsModal) {
    uiStore.closeSettings()
    await nextTick()
  }
  uiStore.openSettings('gridoracloud')
}
...
watch(() => props.visible, (val) => {
  if (val) {
    ...
    try {
      ...
      // 如果当前 provider 是 GRIDORACloud，检查凭证状态
      if (formData.value.provider === 'gridoracloud') {
...
const checkRemoteAPI = async () => {
  if (!formData.value.modelName || (!formData.value.baseUrl && formData.value.provider !== 'gridoracloud')) {
...
const handleConfirm = async () => {
  try {
    ...
    // 如果是 remote 类型且非 GRIDORACloud，必须填写 baseUrl
    if (formData.value.source === 'remote' && formData.value.provider !== 'gridoracloud') {
