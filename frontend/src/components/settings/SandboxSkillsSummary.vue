<template>
  <div class="sandbox-skills-summary" @click.stop @keydown.enter.stop @keydown.space.stop @keydown.esc.stop="close">
    <t-popup :visible="expanded" trigger="click" placement="bottom-right" attach="body" destroy-on-close
      ...
      <button ref="trigger" type="button" class="skills-entry" :aria-expanded="expanded" :aria-controls="panelId"
        :aria-label="hasFailure ? `${$t('skillDiscovery.failedSkills', { count: failureCount })}，${$t('skillDiscovery.viewSkills')}` : $t('skillDiscovery.viewSkills')">
        <span v-if="hasFailure" class="skills-entry__error">{{ $t('skillDiscovery.failedSkills', { count: failureCount }) }}</span>
        <span>{{ $t('skillDiscovery.viewSkills') }}</span>
...
const failureCount = computed(() => liveInstalled.value.filter(skill => skill.status === 'failed').length)
const hasFailure = computed(() => failureCount.value > 0)
...
.skills-entry {
  display: inline-flex; align-items: center; gap: 6px; max-width: 100%; padding: 5px 8px;
  border: 0; border-radius: 4px; background: transparent; text-align: left;
  color: var(--td-text-color-secondary); font: inherit; font-size: 12px; line-height: 22px; cursor: pointer;
  ...
  &__arrow { color: var(--td-text-color-placeholder); transition: transform 0.16s ease; }
  &[aria-expanded='true'] &__arrow { transform: rotate(180deg); }
  &__error { margin-right: 6px; color: var(--td-error-color); }
}
.skill-panel {
