    ...
    <template v-else>
      <section v-for="group in groups" :key="group.kind" class="discovery-section">
        <div class="discovery-section__heading">
          ...
          <t-tooltip v-if="group.kind === 'builtin'" v-model:visible="runtimeHintVisible" :content="t('skillDiscovery.runtimeHint')">
            <button type="button" class="discovery-help" :aria-label="t('skillDiscovery.runtimeHint')"
              @focus="runtimeHintVisible = true" @blur="runtimeHintVisible = false" @keydown.esc="runtimeHintVisible = false">
...
const runtimeHintVisible = ref(false)
