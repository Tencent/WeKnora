...
test('artifacts is the first sandbox panel tab', () => {
  ...
  const files = panel.indexOf("id: 'files' as SandboxPanelTab")
  assert.notEqual(artifacts, -1)
  assert.ok(artifacts < terminal)
  assert.ok(terminal < files)
	assert.match(panel, /<ChatArtifactsPanel/)
	assert.match(panel, /<SandboxFilesPanel/)
	assert.doesNotMatch(panel, /desktopPlaceholder/)
