...
const feedbackStatsEnabled = ref(true);
...
const chunkFeedbackQuery = computed(() => ({
  ...
  max_positive_rate: lowQualityOnly.value ? lowQualityRate.value : undefined,
  min_feedback_count: lowQualityOnly.value ? minFeedbackCount.value : undefined,
}));
...
const findDocDrawerScrollEl = (): HTMLElement | null =>
  document.querySelector(
    ".doc-main-drawer .t-drawer__body",
  ) as HTMLElement | null;
...
const bindDrawerScroll = () => {
  ...
  if (docScrollEl) {
    docScrollEl.addEventListener("scroll", handleDetailsScroll, {
      passive: true,
    });
  }
};
...
const renderMermaidDiagrams = async () => {
  try {
    const mermaidElements =
      getMarkdownRenderRoot()?.querySelectorAll(".mermaid");
...
const getFeedbackStats = (item: any) => {
  const stats = item?.feedback_stats || {};
  const likeCount =
    Number(stats.like_count ?? item?.feedback_like_count ?? 0) || 0;
  const dislikeCount =
    Number(stats.dislike_count ?? item?.feedback_dislike_count ?? 0) || 0;
  const total = likeCount + dislikeCount;
  const positiveRate =
    stats.positive_rate !== null && stats.positive_rate !== undefined
      ? Number(stats.positive_rate)
      : total > 0
        ? likeCount / total
        : undefined;
  ...
  return {
    ...
    session_count: Number(stats.session_count ?? 0) || 0,
    dislike_reasons: Array.isArray(stats.dislike_reasons)
      ? stats.dislike_reasons
      : [],
  };
};

const formatPositiveRate = (item: any) => {
  const rate = getFeedbackStats(item).positive_rate;
  if (rate === null || rate === undefined) return "--";
  return `${Math.round(Number(rate) * 100)}%`;
};

const getPositiveRateTheme = (item: any) => {
  const rate = getFeedbackStats(item).positive_rate;
  if (rate === null || rate === undefined) return "default";
  if (rate < 0.6) return "danger";
  if (rate < 0.8) return "warning";
  return "success";
};

const getFeedbackReasonSummary = (item: any) => {
  const reasons = getFeedbackStats(item).dislike_reasons || [];
  if (!Array.isArray(reasons) || reasons.length === 0)
    return "\u6682\u65e0\u70b9\u8e29\u539f\u56e0";
  return reasons
    .map(
      (reason: any) =>
        `${reason.reason || "\u672a\u586b\u5199"}\uff1a${reason.count || 0}`,
    )
    .join("\n");
};
...
const reloadChunksWithFeedbackFilter = () => {
  ...
  pendingChunksBeforeLoad = 0;
  emit("getDoc", 1, chunkFeedbackQuery.value);
};
...
const requestNextChunkPage = () => {
  ...
  emit("getDoc", page, chunkFeedbackQuery.value);
};
      ...
      <template #header>
        <div class="doc-drawer-header">
          ...
          <div class="header-actions">
            <t-button
              v-if="details.type === 'file' || details.type === 'manual'"
      ...
      >
        <section v-if="details.id" class="setting-drawer__section">
          <h4 class="setting-drawer__section-title">
            {{ $t("knowledgeBase.detailSectionMeta") }}
          </h4>
          <div class="doc-detail-rows">
            ...
            <div v-if="details.type" class="doc-detail-row">
              <span class="doc-detail-label">{{
                $t("knowledgeBase.infoCard.type")
              }}</span>
              <span class="doc-detail-value">
                <t-tag size="small" :theme="getTypeTheme()" variant="light">{{
                  getTypeLabel()
                }}</t-tag>
              </span>
            </div>
        ...
        <section v-if="details.type === 'url'" class="setting-drawer__section">
          <h4 class="setting-drawer__section-title">
            {{ $t("knowledgeBase.urlSource") }}
          </h4>
          <div class="url_link_box">
            <a
              :href="
                isValidURL(details.source)
                  ? details.source
                  : 'javascript:void(0)'
              "
        ...
        <section v-if="showSummarySection" class="setting-drawer__section">
          <h4 class="setting-drawer__section-title">
            {{ $t("knowledgeBase.documentSummary") }}
          </h4>
          <div
            ...
            <div class="view-mode-actions">
              ...
              <div v-if="viewMode === 'chunks'" class="chunk-quality-filter">
                ...
                <t-select
                  ...
                  size="small"
                  class="quality-rate-select"
                  :disabled="!lowQualityOnly"
                  @change="reloadChunksWithFeedbackFilter"
                >
                  <t-option :value="0.5" label="50%" />
                  <t-option :value="0.6" label="60%" />
                  <t-option :value="0.7" label="70%" />
                  <t-option :value="0.8" label="80%" />
                </t-select>
              </div>
            </div>
          </div>
              ...
              >
                <div class="chunk-header">
                  ...
                  <div class="chunk-header-right">
                    ...
                    <t-tag
                      size="small"
                      variant="light"
                      :theme="getPositiveRateTheme(chunk.original)"
                    >
                ...
                >
                  <div class="questions-toggle" @click="toggleQuestions(index)">
                    <t-icon
                      :name="
                        isExpanded(index) ? 'chevron-down' : 'chevron-right'
                      "
                      size="14px"
                    />
                    <span
