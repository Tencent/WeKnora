<template>
  <div class="bot_msg" :class="{ 'is-embedded': embeddedMode }">
    <div style="display: flex; flex-direction: column; gap: 8px">
      ...
      <div
        v-if="
          !session.isAgentMode && mentionedItems && mentionedItems.length > 0
        "
      ...
      <template v-else>
        <docInfo
          v-if="session.knowledge_references?.length"
          :session="session"
        ></docInfo>
        <AgentStreamDisplay
          :session="session"
      ...
      <deepThink
        :deepSession="session"
        v-if="session.showThink && !session.isAgentMode"
      ></deepThink>
    </div>
    ...
    <div ref="parentMd" v-if="!session.hideContent && !session.isAgentMode">
      ...
      <div class="content-wrapper" v-if="hasActualContent">
        <div
          class="ai-markdown-template markdown-content"
          v-stable-html="renderedHTML"
        ></div>
      </div>
      <!-- Streaming indicator (non-Agent mode) -->
      <div
        v-if="hasActualContent && !session.is_completed"
      ...
      <div
        v-if="session.is_completed && (content || session.content)"
      ...
      >
        ...
        <t-button
          ...
          :class="[
            'answer-feedback-btn',
            { 'is-active': answerFeedback.type === 'like' },
          ]"
          title="点赞"
        ...
        <t-button
          ...
          :class="[
            'answer-feedback-btn',
            'is-negative',
            { 'is-active': answerFeedback.type === 'dislike' },
          ]"
          title="点踩"
          @click.stop="handleDislikeAnswer"
        >
...
const markdownRenderer = createChatMarkdownRenderer({
  ...
  imageRenderer: ({ href, title, text }) =>
    createSafeImage(href, text || "", title || ""),
...
const handleDislikeAnswer = async () => {
  if (answerFeedback.type === "dislike") {
    const ok = await clearFeedback();
    if (ok) {
      answerFeedback.type = "";
      answerFeedback.reason = "";
