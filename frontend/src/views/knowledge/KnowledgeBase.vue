...
import { createFolder, moveFolder } from "@/api/knowledge-folder";
...
import FolderActionMenu from '@/views/knowledge/components/FolderActionMenu.vue';
...
// Track which folder card's action popup is open (for closing after action)
...
const formatDocTime = (time?: string) => {
  if (!time) return '--';
  try {
    return formatStringDate(new Date(time));
  } catch {
    return '--';
  }
};
...
const onCardClick = (item: (KnowledgeCard & { isFolder?: boolean }) | any) => {
  if (batchMode.value) {
...
const handleListAction = (
  action: 'edit' | 'reparse' | 'cancel-parse' | 'move' | 'delete' | 'view-trace' | 'batch-manage' | 'rename' | 'folder-move',
  ...
  if ((item as any).isFolder) {
    if (action === 'delete') {
      ...
      handleFolderRename(item as any);
    } else if (action === 'folder-move') {
      handleFolderMoveIndividual(item as any);
    }
    return;
  }
  const idx = (cardList.value || []).findIndex((i: KnowledgeCard) => i.id === item.id);
                        ...
                        <div class="card-content-nav">
                          <div v-if="canEdit && batchMode" class="card-nav-check" @click.stop>
                            <t-checkbox
                              ...
                              :title="f.file_name"
                              @change="(checked: boolean, ctx?: { e?: Event }) => onCardGridCheckboxChange(f.id, checked, ctx)"
                            />
                          </div>
                          ...
                          <t-popup
                            ...
                            :visible="openFolderMenuId === f.id"
                            trigger="click"
                            destroy-on-close
                            placement="bottom-right"
                            overlay-class-name="card-more"
                            @visible-change="(v: boolean) => openFolderMenuId = v ? f.id : null"
                          >
                            ...
                            <template #content>
                              <div class="card-menu">
                                <FolderActionMenu
                                  :folder="f"
                                  :can-edit="true"
                                  @rename="() => { openFolderMenuId = null; handleFolderRename(f) }"
                                  @move="() => { openFolderMenuId = null; handleFolderMoveIndividual(f) }"
                                  @delete="() => { openFolderMenuId = null; confirmDeleteFolder(f as any) }"
                                />
                              </div>
                            </template>
                          </t-popup>
