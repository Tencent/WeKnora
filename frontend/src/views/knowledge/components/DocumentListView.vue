...
const emit = defineEmits<{
  ...
  (e: 'action', action: 'edit' | 'reparse' | 'cancel-parse' | 'move' | 'delete' | 'view-trace' | 'batch-manage' | 'rename' | 'folder-move', item: KnowledgeItem): void;
...
const handleAction = (action: 'edit' | 'reparse' | 'cancel-parse' | 'move' | 'delete' | 'view-trace' | 'batch-manage' | 'rename' | 'folder-move', item: KnowledgeItem) => {
  // Don't close popup for move / folder-move — they trigger sub-flows
  if (action !== 'move' && action !== 'folder-move') {
    moreOpen.value = null;
  }
  item.isMore = false;
        ...
        <div class="cell cell-actions" v-if="canEdit" @click.stop>
          <template v-if="item.isFolder">
            <t-popup
              ...
              @visible-change="(v: boolean) => onMoreVisible(item.id, v)"
            >
              <button class="row-more-btn" :class="{ active: moreOpen === item.id }" type="button"
                :aria-label="t('knowledgeBase.columnActions')" @click.stop>
              ...
              <template #content>
                <div class="card-menu">
                  <FolderActionMenu
                    :folder="item"
                    :can-edit="true"
                    @rename="() => { moreOpen = null; handleAction('rename', item) }"
                    @move="() => { moreOpen = null; handleAction('folder-move', item) }"
                    @delete="() => { moreOpen = null; emit('action', 'delete', item) }"
                  />
                </div>
              </template>
            </t-popup>
