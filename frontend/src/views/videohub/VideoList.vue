  ...
  <main class="video-list-page">
    <header class="video-list-page__header">
      ...
      <div class="video-list-page__actions">
        ...
        <t-button @click="showComingSoon">上传视频</t-button>
      </div>
    </header>
    ...
    <t-empty v-else-if="filteredVideos.length === 0" :description="query ? '没有匹配的视频' : '还没有视频，上传第一个视频吧'">
      <template #action><t-button v-if="!query" @click="showComingSoon">上传视频</t-button></template>
    </t-empty>
...
import AiPromptBar from '@/components/videohub/AiPromptBar.vue'
...
function showComingSoon() { MessagePlugin.info('上传功能正在建设中') }
onMounted(async () => { try { videos.value = await fetchVideoList() } finally { loading.value = false } })
</script>

<style scoped>
.video-list-page { height: 100%; overflow-y: auto; padding: 32px 32px 48px; background: var(--td-bg-color-container); color: var(--td-text-color-primary); }
