export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      pageHint: 'Песочница — изолированная среда, в которой выполняются скрипты навыков агента. В пространстве можно создать несколько конфигураций (Docker, E2B, CubeSandbox или локальный процесс); каждый агент выбирает одну. Пока конфигурация не выбрана, скрипты не выполняются.',
  ...
  agent: {
    ...
    editor: {
      ...
      sandboxNoConfigs: 'В этом пространстве ещё нет песочницы, поэтому скрипты навыков не будут выполняться. Добавьте её в «Настройки — Песочница».',
