export default {
  ...
  integrations: {
    ...
    api: {
      ...
      editApiKeyScopeDesc: "Изменить имя, права и настройки идентификации пользователей API key «{name}».",
      ...
      keyIdentityDesc: "Применяется только к этому API key. Изолирует сеансы и разрешения MCP OAuth; права операций задаются областью доступа ключа.",
      keySignedHint: "Подписывайте на сервере краткосрочный HS256 JWT с sub, tenant_id, aud=weknora, exp и identity_namespace этого ключа. Пример подписи доступен после сохранения.",
      keySecretDraftHint: "Скопируйте секрет на сервер. Он вступит в силу после создания или сохранения ключа и заменит его прежний секрет подписи.",
      keyModeChangeHint: "При изменении режима идентификации сеансы и разрешения MCP прежнего режима не переносятся.",
      keyExampleDesc: "Выберите API key, чтобы увидеть настройки идентификации, примеры запросов и Playground.",
      identityNamespace: "Пространство идентификации приложения",
      identityApplication: "Идентификация приложения",
      identityApplicationHint: "Новые приложения изолированы по умолчанию. Для замены ключа того же приложения выберите существующий ключ, чтобы сохранить пользователей, сеансы и разрешения MCP.",
      identityNewApplication: "Новое приложение (изолированное)",
      identityReuseHint: "Копирует режим идентификации и настройки подписи выбранного ключа. Права задаются этой формой. После создания ключ можно редактировать отдельно.",
      legacyIdentityHint: "Этот ключ сохраняет идентификацию до обновления. Одинаковые внешние ID пользователей могут разделять сеансы и разрешения MCP с другими старыми ключами.",
      ...
      modeTenant: "Без идентификации пользователя",
      ...
      requireDirectHeaderDesc: "Если включено, запросы без ID пользователя отклоняются. Иначе используется общая идентификация приложения. Старые ключи сохраняют идентификацию пространства.",
      ...
      playgroundTenantModeHint: "No external user identity is sent. Requests within this application identity share session ownership and MCP grants.",
