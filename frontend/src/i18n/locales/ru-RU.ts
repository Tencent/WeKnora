export default {
  ...
  system: {
    ...
    globalSettings: {
      ...
      keyDescriptions: {
        ...
        tenant: {
          plugin_remote_enabled: 'Могут ли администраторы пространств регистрировать собственные удалённые плагины (код на их серверах, без страниц, виден только этому пространству). Если выключено, новые регистрации и обновления запрещены; зарегистрированные продолжают работать. Применяется сразу.',
      ...
      keyLabels: {
        ...
        tenant: {
          plugin_remote_enabled: 'Разрешить пространствам свои плагины',
  ...
  knowledgeEditor: {
    ...
    chunking: {
      ...
      pluginStrategy: 'Предоставлено плагином {id}; если он не ответит, используются встроенные стратегии.',
      ...
      strategies: {
        plugin: {
          label: 'Плагин разбиения',
          tooltip: 'Плагин решает, где резать.'
        },
        legacy: {
          label: 'По длине',
  ...
  pluginAdmin: {
    ...
    menu: {
      detail: 'Подробнее',
      enable: 'Включить на платформе',
      disable: 'Отключить на платформе',
      more: 'Ещё'
    },
    ownedBy: 'Пространство {tenant}',
    runtime: {
    ...
    description: 'Установка и управление плагинами платформы. Каждое пространство включает их само.',
    ...
    platformSwitch: 'Включён на платформе',
    publisher: 'Издатель',
    ...
    egress: {
      mode: {
        ...
        proxy: 'Только прокси',
        unmanaged: 'Вне контроля'
      },
      hint: {
        sandboxed: 'Процесс плагина работает в собственном сетевом пространстве имён; наружу он выходит только через исходящий прокси, поэтому его разрешения соблюдаются принудительно.',
        networkPolicy: 'Kubernetes NetworkPolicy разрешает pod-у плагина обращаться только к DNS и WeKnora, чей исходящий прокси применяет разрешения. Действует, если сетевой плагин кластера поддерживает NetworkPolicy.',
        proxy: 'Процессу плагина передан исходящий прокси через HTTP(S)_PROXY, но код, игнорирующий эти переменные, может подключаться напрямую. Разрешите непривилегированные пространства имён пользователей в Linux, чтобы включить песочницу.',
        unmanaged: 'Плагин работает там, где WeKnora ничего не контролирует (удалённый сервис); его исходящие разрешения не соблюдаются принудительно.'
      },
    ...
    market: {
      loading: 'Чтение каталога…',
      loadFailed: 'Не удалось прочитать каталог',
      notConfigured: 'Каталог плагинов не настроен. Укажите на сервере WEKNORA_PLUGIN_INDEX_URL — адрес индекса плагинов, чтобы просматривать его здесь.',
      search: 'Поиск по названию, ID или издателю',
      empty: 'Подходящих плагинов нет',
      installed: 'Установлена v{version}',
      source: 'Индекс: {url}. Перед установкой пакет всё равно проверяется и сверяется с указанным дайджестом; доверие определяется подписью пакета.',
      action: {
        install: 'Установить',
        upgrade: 'Обновить',
        installed: 'Актуально',
        unavailable: 'Несовместим'
      }
    },
    audience: {
      owned: 'Собственный плагин пространства {tenant}; виден только ему.',
      title: 'Доступность',
      hint: 'При ограничении только выбранные пространства видят плагин и могут его включить. Остальные его не видят; их переключатель и настройки сохраняются и вернутся, если пространство снова добавят.',
      all: 'Все пространства',
      count: 'Пространств: {count}',
      some: 'Выбранные пространства',
      placeholder: 'Найдите и выберите пространства',
      none: 'Пространства не выбраны: плагин никому не доступен.',
      saved: 'Доступность обновлена',
      saveFailed: 'Не удалось обновить доступность'
    },
    install: {
      steps: {
        source: 'Выбор пакета',
      ...
      dropTitle: 'Перетащите сюда пакет .wkp или нажмите, чтобы выбрать',
      dropHint: 'До 64 МБ. Пакет сначала проверяется, а не устанавливается сразу.',
      inspecting: 'Проверка пакета…',
      next: 'Далее',
      back: 'Назад',
      finish: 'Готово',
      doneHint: 'Администраторы пространств могут включить его в центре плагинов.',
      doneConfig: 'Нужна настройка платформы — заполните её в сведениях о плагине.',
      trustRow: 'Доверие',
      anyHostHint: 'Убедитесь, что доверяете этому плагину',
      changeShort: {
        install: 'Новая установка',
        upgrade: 'Обновление',
        downgrade: 'Откат',
      ...
      confirmTo: {
        upgrade: 'Обновить до v{version}',
        downgrade: 'Откатить до v{version}'
      },
  ...
  pluginOff: {
    label: 'Плагин выключен',
    hint: 'Плагин, который это предоставляет, выключен в этом пространстве: новые добавить нельзя, существующие продолжают работать. Администратор может снова включить его в «Настройки → Плагины».'
  },
  pluginToolView: {
    empty: 'Нет результатов',
    pageUnavailable: 'Плагин больше недоступен, поэтому страницу результата нельзя показать'
  },
  pluginPages: {
    notResponding: 'Страница плагина не отвечает; возможно, она не загрузилась.',
    requestFailed: 'Запрос к плагину не выполнен',
    fromPlugin: 'Предоставлено плагином «{name}»'
  },
  pluginCenter: {
    tabs: {
    ...
    categories: {
      data: 'Данные',
      model: 'Модели',
      tool: 'Инструменты и поиск',
      channel: 'Каналы',
      ui: 'Страницы и процессы'
    },
    where: {
      modelVendors: 'Добавьте модели в «Управлении моделями»',
      connectors: 'Добавьте в «Источниках данных» базы знаний',
      imChannels: 'Добавьте в «IM-каналах» агента',
      webSearch: 'Добавьте в «Веб-поиске»',
      tools: 'Отметьте в инструментах агента',
      parsers: 'Выберите в «Движках разбора»',
      chunkers: 'Выберите в настройках фрагментации базы знаний',
      pipelineHooks: 'Работает в Q&A по базе знаний автоматически',
      skills: 'Включите в навыках агента',
      mcpServers: 'Отметьте в инструментах агента',
      pages: 'Откройте из «Панели инструментов»',
      settingsSections: 'В группе «Расширения» настроек',
      kbTabs: 'Вкладка на странице базы знаний',
      webhooks: 'Адреса ниже'
    },
    detail: {
      enabledHere: 'Включён здесь',
      config: 'Настройки пространства'
    },
    noneInstalled: 'Плагинов пока нет. Системный администратор может установить их в «Управлении плагинами».',
    off: 'Выключен',
    own: {
      section: 'Сервис и ключ',
      updateHint: 'Загрузите новую версию пакета, проверьте её и замените текущую.',
      register: 'Зарегистрировать свой плагин',
      title: 'Регистрация собственного плагина пространства',
      description: 'Загрузите пакет удалённого плагина (.wkp) или укажите его URL, а также адрес сервиса. Код плагина работает на ваших серверах.',
      notice: 'Плагин будет виден только этому пространству и сразу включится здесь. Принимаются только удалённые плагины, и они не могут добавлять страницы.',
      tag: 'Это пространство',
      manage: 'Управление',
      update: 'Обновить пакет',
      editUrl: 'Изменить URL сервиса',
      urlTitle: 'Изменить URL сервиса',
      remove: 'Удалить',
      removeConfirm: 'Удалить плагин? Его интеграции сразу перестанут работать.',
    ...
    webhooksHint: 'Укажите эти адреса в стороннем сервисе, чтобы он отправлял события этому плагину. Они содержат секрет рабочего пространства; не публикуйте их.',
    copy: 'Копировать',
    webhookCopied: 'Адрес вебхука скопирован',
    ...
    navGroup: 'Расширения',
    title: 'Плагины',
    description: 'Включайте плагины для этого пространства. После отключения новые интеграции из него создать нельзя, существующие продолжают работать.',
  ...
  schemaForm: {
    ...
    options: {
      loading: 'Загрузка…',
      empty: 'Плагин не предлагает вариантов',
      retry: 'Повторить',
      failed: 'Не удалось загрузить варианты'
    },
    oauth: {
      connected: 'Подключено',
      ...
      popupBlocked: 'Браузер заблокировал окно авторизации. Разрешите всплывающие окна и повторите попытку.',
      waitingBrowser: 'Страница авторизации открыта в браузере. Вернитесь сюда, когда закончите.',
      cancel: 'Отмена',
  ...
  settings: {
    ...
    sandbox: {
      ...
      templateLoadHint: 'Введите подключение к кластеру и обновите список. Отсутствующий официальный шаблон CLI собирается из образа Hub. Шаблоны рабочего стола больше по размеру — создайте их отдельной строкой ниже. После смены DNS или образа пересоберите шаблон на соответствующей карточке.',
      ...
      templateStepHint: 'На этом шаге перечисляются шаблоны кластера; официальный образ CLI собирается из Hub, если его ещё нет. Шаблоны рабочего стола (XFCE) тяжелее и создаются только по кнопке «Создать». После смены DNS или образа пересоберите шаблон на соответствующей карточке. Продолжить можно, когда шаблон готов.',
      loadingTemplates: 'Loading templates from the cluster...',
      templateBuildingHint: 'Шаблон собирается. Список обновится автоматически.',
      ...
      desktopTemplateProvisioning: 'Шаблон рабочего стола WeKnora создаётся. Обновите список чуть позже, чтобы увидеть статус.',
      desktopTemplateReplaced: 'Предыдущий шаблон рабочего стола удалён, начата пересборка. Дождитесь готовности. Шаблон CLI не затрагивается.',
      desktopTemplateTag: 'Рабочий стол',
      replaceDesktopTemplateConfirm: 'Шаблон рабочего стола WeKnora будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон рабочего стола не удаляется, пока замена не станет готовой к запуску. Шаблон CLI не затрагивается.',
