export default {
  ...
  knowledgeBase: {
    ...
    folder: {
      ...
      createUnder: 'Будет создана внутри «{name}»',
      ...
      deletePromote: 'Переместить её документы и подпапки на уровень выше (документы не удаляются)',
      deleteNotEmpty: 'Папка не пуста: сначала переместите содержимое или включите перенос на уровень выше',
      organizeByPath: 'Упорядочить файлы по пути',
      organizeConfirm: 'Документы, в именах которых есть относительный путь, будут разложены по соответствующим папкам. Можно повторять.',
      organizeResult: 'Упорядочено {organized} документов, создано {folders} папок',
      askInFolder: 'Спросить в этой папке',
      pickTarget: 'Выберите папку',
      noFolders: 'Папок пока нет',
      moveDocsTo: 'Переместить {count} документов в…',
      movedDocs: 'Перемещено {count} документов',
      searchPlaceholder: 'Поиск папок',
      newSubfolder: 'Новая подпапка',
      moveIntoSelf: 'Нельзя переместить папку в саму себя или в её подпапку',
      moveConfirmTitle: 'Подтвердите перемещение',
      moveConfirm: 'Переместить эту папку в «{target}»?',
      noMatchedFolders: 'Подходящих папок нет',
    },
