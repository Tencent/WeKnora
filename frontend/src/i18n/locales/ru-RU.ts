export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'SAI Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API SAI — загрузки, URL, Markdown и поиск.',
      ...
      installCtaHint: 'Установка SAI Skill · откроется в новой вкладке',
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted SAI: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В SAI Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными SAICloud.',
      ...
      hmacSecretDesc: 'Тот же secret, что в SAI; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
  ...
  kbSettings: {
    parser: {
      ...
      engines: {
        ...
        weknoracloud: {
          name: 'SAI Cloud',
          desc: 'Парсинг документов через SAI Cloud'
        },
        paddleocr_vl_cloud: {
  ...
  system: {
    ...
    versionDescription: 'Версия сервиса приложения',
    frontendVersionLabel: 'Версия UI',
    frontendVersionDescription: 'Версия сборки UI',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: 'Optional. SAI POSTs message_sent / message_received events to this HTTPS endpoint.',
    webhookSecret: 'Webhook secret',
    webhookSecretPlaceholder: 'HMAC-SHA256 secret (X-SAI-Signature header)',
  ...
  createChat: {
    title: 'Привет, я SAI — ваши знания всегда под рукой',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с SAI',
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки SAI.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте SAI; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине SAI — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      weknoraTemplateTitle: 'SAI standard template',
      weknoraDockerImage: 'SAI standard image',
      ...
      weknoraTemplateOverview: 'SAI provides the standard runtime. Templates are discovered after connecting and the standard one is created when missing.',
      weknoraTemplateDescription: 'Includes the Python, Node.js, CLI tools, workspace path, and non-root execution user expected by SAI skills.',
      ...
      templateLoadHint: 'Введите подключение к кластеру и обновите список. Если стандартного шаблона SAI нет, нажмите «Создать»; после смены DNS или образа — «Удалить и пересобрать».',
      templateLoadFailed: 'Failed to load templates',
      standardTemplateProvisioning: 'The SAI standard template is being created. Refresh shortly to see its status.',
      ...
      templateStepHint: 'На этом шаге только перечисляются шаблоны, уже есть в кластере; ничего не создаётся автоматически. Если стандартного шаблона нет, нажмите «Создать стандартный шаблон SAI»; после смены DNS или образа — «Удалить и пересобрать». Продолжить можно, когда шаблон готов.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — SAI пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Стандартный шаблон SAI',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон SAI будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон SAI. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
      ...
      dockerTlsCertPathHelp: 'Directory on the SAI host holding ca.pem, cert.pem and key.pem. Required for a remote daemon; certificates are mounted by the deployment, never stored here.',
      dockerIdleTtl: 'Idle reclaim (seconds)',
      dockerIdleTtlHelp: 'The Docker daemon has no idle timeout of its own. A container that runs no command for this long is reclaimed by SAI and rebuilt when the session continues. Empty means 1800 seconds.',
    ...
    weknoraCloud: {
      title: 'SAI Cloud',
      description: 'Настройте учётные данные APPID и APPSECRET для SAI Cloud. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные SAI Cloud истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения SAI Cloud',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения SAI Cloud',
      ...
      addModelsDisplayName: {
        chat: 'SAICloud Chat',
        embedding: 'SAICloud Embedding',
        rerank: 'SAICloud ReRank',
        vllm: 'SAICloud Vision'
      },
      modelsSection: {
        title: 'Облачные модели',
        descReady: 'Зарегистрируйте четыре стандартные модели SAI Cloud для чата, поиска, реранкинга и зрения.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и SAI автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
