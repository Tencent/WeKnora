export default {
  ...
  knowledgeBase: {
    ...
    batchReparseSkippedInFlight: 'Пропущено документов, которые ещё обрабатываются: {count}',
      batchReparseNoEligibleDocuments: 'Нет документов для пересборки: документы, которые уже обрабатываются, пропущены',
      batchReparseLimitExceeded: 'За одну операцию можно пересобрать не более {count} документов. Сузьте фильтр и повторите попытку.',
      batchReparsePartial: 'Отправлено задач пересборки: {submitted}; не удалось отправить документов: {failed}. Повторите попытку позже.',
    selectAllFilteredDocuments: 'Выбрать все результаты фильтра ({count})',
    allFilteredDocumentsSelected: 'Выбраны все результаты фильтра ({count})',
  ...
  knowledgeEditor: {
    activity: {
      ...
      actions: {
        ...
        'knowledge.batch_deleted': 'Знания удалены пакетом', 'knowledge.batch_reparsed': 'Пакетная пересборка знаний завершена', 'knowledge.reparse_started': 'Повторный разбор начат',
