export default {
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки TreeRAG.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте TreeRAG; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине TreeRAG — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      templateLoadHint: 'Введите подключение к кластеру и обновите список. Если стандартного шаблона TreeRAG нет, нажмите «Создать»; после смены DNS или образа — «Удалить и пересобрать».',
      ...
      templateStepHint: 'На этом шаге только перечисляются шаблоны, уже есть в кластере; ничего не создаётся автоматически. Если стандартного шаблона нет, нажмите «Создать стандартный шаблон TreeRAG»; после смены DNS или образа — «Удалить и пересобрать». Продолжить можно, когда шаблон готов.',
      ...
      weknoraStandardTemplate: 'Стандартный шаблон TreeRAG',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон TreeRAG будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон TreeRAG. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
