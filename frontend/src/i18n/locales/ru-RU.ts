export default {
  ...
  agentEditor: {
    ...
    im: {
      ...
      wecomRemoteImageHostAllowlist: 'Доверенные хосты удалённых изображений',
      wecomRemoteImageHostAllowlistHint: 'Необязательно. Хосты, с которых бот может загружать изображения и пересылать их в WeCom. Разделяйте хосты запятыми; поддерживается *.example.com. Если поле пусто, сохраняются только кликабельные ссылки. Внутренние хосты также нужно добавить в SSRF_WHITELIST.',
