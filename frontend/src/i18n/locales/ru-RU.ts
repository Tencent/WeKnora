export default {
  ...
  knowledgeEditor: {
    ...
    desensitization: {
      title: 'Обезличивание документов',
      description: 'Маскирует markdown после парсинга, до нарезки, embedding и суммаризации. По умолчанию выключено. Облачные парсеры при этом запрещены.',
      coverageTitle: 'Полное обнаружение не гарантируется',
      coverageNotice: 'Встроенный движок маскирует только выбранные типы и свои регулярные выражения. Изображения, сканы и исходные файлы не обрабатываются. Несовпавший текст попадает в нарезку, эмбеддинги и ответы как есть. Проверяйте образцы на этой странице; это не замена ручной проверке.',
      enableLabel: 'Включить обезличивание текста',
      enableDescription: 'Обрабатывается только извлечённый текст. Исходные файлы и изображения не изменяются.',
      engineLabel: 'Движок',
      engineDescription: 'Встроенный движок — регулярные выражения и контрольные суммы. Presidio сначала применяет те же правила, затем отправляет исходный текст в sidecar.',
      engineBuiltin: 'Встроенный (regex + checksum)',
      enginePresidio: 'Presidio sidecar',
      presidioFlowTitle: 'Сначала встроенный движок, затем sidecar',
      presidioFlowNotice: '1. Маскирование встроенными правилами ниже. 2. App делает POST исходного Markdown в Analyzer. 3. start/end/entity_type сопоставляются с выбранными типами и заменяются. Без sidecar или китайских recognizer ingest падает. Предпросмотр на этой странице — только шаг 1.',
      presidioApiLabel: 'API sidecar',
      presidioApiUrlHint: 'URL задаётся переменной PRESIDIO_ANALYZER_URL, не в карточке базы знаний.',
      presidioRequestLabel: 'Пример запроса',
      presidioResponseLabel: 'Пример ответа',
      engineLLM: 'Локальная LLM',
      llmModelLabel: 'LLM',
      llmModelDescription: 'Если пусто, используется модель суммаризации базы знаний.',
      llmModelPlaceholder: 'Выберите модель',
      maskStyleLabel: 'Стиль маскирования',
      maskStyleDescription: 'Плейсхолдер вроде <身份证> или частичная маска вроде 138****8000.',
      maskReplace: 'Плейсхолдер',
      maskPartial: 'Частичная маска',
      entityTypesLabel: 'Типы',
      entityTypesDescription: 'Идентификаторы материкового Китая. Можно выбрать несколько. Выбранные типы показываются ниже как правила только для чтения.',
      entities: {
        cn_id_card: 'Удостоверение личности',
        cn_mobile: 'Мобильный номер КНР',
        cn_landline: 'Городской телефон',
        cn_bank_card: 'Банковская карта (UnionPay)',
        cn_uscc: 'Единый код соц. кредита',
        cn_plate: 'Гражданский номер',
        cn_plate_std: 'Номер (обычный)',
        cn_plate_nev: 'Номер (NEV)',
        email: 'Email'
      },
      presetReadonly: 'Шаблон',
      customRulesLabel: 'Свои регулярные выражения',
      customRulesDescription: 'Применяются после предустановленных типов.',
      customRulesTitle: 'Проверьте правила перед сохранением',
      customRulesNotice: 'Слишком широкие шаблоны дают ложные срабатывания (маскируются номера заказов и серийники). Слишком узкие — пропускают настоящие значения. Прогоните предпросмотр на реальных примерах и проверьте и совпадения, и пропуски.',
      ruleName: 'Имя',
      rulePattern: 'Шаблон',
      ruleReplacement: 'Замена (необязательно)',
      addRule: 'Добавить правило',
      previewLabel: 'Проверка',
      previewDescription: 'Прогоняет текущие правила на образце текста. Предпросмотр только для встроенного движка, не отражает ingest Presidio.',
      previewBuiltinOnlyTitle: 'Предпросмотр только для встроенного движка',
      previewBuiltinOnlyNotice: 'При выборе Presidio эта страница не вызывает sidecar. Проверяйте ingest на реальном документе.',
      previewPlaceholder: 'Вставьте текст с телефоном или удостоверением…',
      previewRun: 'Предпросмотр',
      previewEmpty: 'Сначала введите текст',
      previewFailed: 'Предпросмотр не удался',
      cloudParserForbidden: 'При включённом обезличивании облачные парсеры недоступны. Выберите anydoc, simple или builtin.'
    },
    asr: {
