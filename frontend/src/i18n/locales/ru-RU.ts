...
/example\.(net|org)/`,
  ...
  datasource: {
    ...
    field: {
      ...
      operatorIdHint: 'API баз знаний DingTalk требуют личность пользователя. Укажите unionId (не userId) пользователя с доступом к целевой базе знаний.',
      dingtalkBaseUrlHint: 'Оставьте пустым для https://api.dingtalk.com. Для выделенных / частных развёртываний DingTalk укажите API-хост.',
    ...
    prereqBarText_dingtalk: 'Используете впервые? Нажмите, чтобы открыть руководство по DingTalk Docs',
    prereqStep1Brief_dingtalk: 'Создайте внутреннее приложение и получите учётные данные',
    prereqStep1Desc_dingtalk: 'Откройте консоль разработчика DingTalk → Разработка приложений → создайте внутреннее приложение → скопируйте AppKey / AppSecret',
    prereqStep2Brief_dingtalk: 'Выдайте права чтения базы знаний',
    prereqStep2Desc_dingtalk: 'Включите как минимум Document.Workspace.Read, Document.WorkspaceDocument.Read, Wiki.Node.Read и опубликуйте приложение',
    prereqStep3Brief_dingtalk: 'Укажите operator unionId',
    prereqStep3Desc_dingtalk: 'Используйте unionId пользователя с доступом к целевой базе знаний (не userId)',
