export default {
  ...
  agent: {
    ...
    editor: {
      ...
      promptsConfig: 'Инструкции и поведение',
      promptsConfigDesc: 'Бизнес-инструкции и структурированное поведение; протоколы поиска, инструментов и вывода управляются системой',
      ...
      webSearchConfigDesc: 'Настройка доступности веб-источников и политики поиска для быстрых ответов',
      webSearchModeOff: 'Отключено',
      webSearchModeOnDemand: 'По необходимости (рекомендуется)',
      webSearchModeAlways: 'Всегда объединять',
...
/example\.(net|org)/`,
  ...
  agentEditor: {
    ...
    promptProtocol: {
      managedNotice: 'Используется управляемый системой протокол промптов v{version}. Инструкции агента не могут переопределять маршрутизацию поиска, инструменты, безопасность, ссылки и формат вывода.',
      userInstructions: 'Пользовательские инструкции',
      userInstructionsDesc: 'Опишите роль, тон, аудиторию, предпочтения ответа и бизнес-правила. Управляемый протокол переопределить нельзя.',
      userInstructionsPlaceholder: 'Например: помогайте новым сотрудникам; сначала давайте вывод, затем пошаговые действия.',
      managedFallback: 'Решение о резервном ответе и его генерация управляются системой и обновляются вместе с версией протокола.',
    },
    selection: {
      all: 'Все',
      selected: 'Выбранные',
