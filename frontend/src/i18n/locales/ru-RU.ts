export default {
  ...
  modelSettings: {
    ...
    usage: {
      ...
      cacheCoverage: 'Охват отчётности',
      ...
      rangeAll: 'Все сохранённые данные',
      details: 'Детализация по назначению',
      hideDetails: 'Скрыть детализацию',
      partial: 'Частичная тарификация',
      partialCost: 'Учтены только вызовы с настроенными ценами; это не полная стоимость',
      embeddingCache: 'Векторный кэш',
      embeddingCacheHint: 'Доля embedding-запросов, обслуженных из памяти или постоянного кэша',
      embeddingAvoided: 'Вычислений сэкономлено',
      embeddingAvoidedHint: 'Число embedding-вычислений, исключённых кэшем и дедупликацией внутри пакета',
      wikiSummary: 'Сводка Wiki',
      loading: 'Обновление статистики моделей…',
      loadFailed: 'Не удалось загрузить статистику. Настройки моделей не затронуты.',
      retry: 'Повторить',
      noData: 'За выбранный период нет вызовов моделей или данных векторного кэша.',
      purposes: {
  ...
  evaluationSettings: {
    navLabel: 'Оценка RAG',
    title: 'Оценка RAG',
    description: 'Запускайте воспроизводимые оценки и сравнивайте качество, задержку, токены, кэш и стоимость.',
    refresh: 'Обновить', newRun: 'Новая оценка',
    newRunHint: 'Зафиксируйте набор данных и меняйте только одну переменную за запуск.',
    dataset: 'Набор данных', knowledgeBase: 'База знаний', chatModel: 'Модель чата', rerankModel: 'Модель ранжирования',
    autoSelect: 'Системное значение', adminOnly: 'Viewer может просматривать результаты; для запуска нужен Admin.',
    start: 'Запустить оценку', reset: 'Сбросить поля', chatRequired: 'Явно выберите модель чата, чтобы сравнение было воспроизводимым.', comparison: 'Сравнение запусков',
    comparisonHint: 'Первый запуск — базовый; можно сравнить до {count} моделей или конфигураций.',
    comparable: 'Проверка сравнимости пройдена: набор данных, база знаний, разбиение, конвейер и зависимые модели совпадают, кроме модели чата.',
    comparabilityUnknown: 'В некоторых запусках нет полного снимка; сравнимость нельзя проверить.', datasetDiffers: 'Содержимое набора данных отличается.',
    knowledgeBaseDiffers: 'Исходная база знаний отличается.', chunkingDiffers: 'Настройка разбиения отличается.',
    pipelineDiffers: 'Конвейер поиска или генерации отличается.', modelDependencyDiffers: 'Модель embedding или rerank отличается.',
    clear: 'Очистить', metric: 'Метрика', base: 'База', candidate: 'Кандидат', delta: 'Разница',
    duration: 'Полное время', cacheHitRate: 'Доля попаданий в кэш', cost: 'Оценочная стоимость',
    history: 'История запусков', historyHint: 'Выберите 2–4 успешных запуска для сравнения версий или моделей.',
    total: 'Запусков: {count}', pageStatus: 'Страница {current} из {total}', empty: 'Запусков оценки пока нет.', selectCompare: 'Выбрать для сравнения',
    exportEvidence: 'Экспорт доказательств', exportSucceeded: 'Отчёт с доказательствами загружен.', exportFailed: 'Не удалось экспортировать отчёт.',
    repeatSummary: 'Сводка повторных запусков', repeatSummaryHint: 'Агрегируются только успешные запуски с одинаковыми отпечатками кода, данных, конфигурации, базы знаний, модели и одинаковой нагрузкой вызовов.',
    repeatCount: 'Совпадающих запусков: {count}', medianRecall: 'Медиана Recall', medianDuration: 'Медиана времени', p95Duration: 'P95 времени',
    medianTokens: 'Медиана токенов', medianCacheHitRate: 'Медиана попаданий в кэш', medianCost: 'Медиана стоимости', latencyDistribution: 'Сравнение медианы и P95 времени', medianShort: 'Медиана',
    modelValue: 'Модель: {value}', knowledgeBaseValue: 'База знаний: {value}', defaultConfiguration: 'Системная конфигурация', unknownValue: 'Неизвестно',
    loadFailed: 'Не удалось загрузить данные оценки.', startFailed: 'Не удалось запустить оценку.', pollFailed: 'Не удалось трижды обновить статус оценки. Обновите страницу и повторите попытку.',
    invalidHint: 'Нет успешных вызовов модели чата; запуск нельзя использовать в официальном сравнении.',
    cacheBenchmarkTitle: 'Строгий A/B кеша Wiki', cacheBenchmarkHint: 'Выполняет три изолированных холодных запроса в формате обновления Wiki, затем побайтно идентичные тёплые запросы; экспортируются только отпечатки и метрики.',
    cacheBenchmarkModel: 'Чат-модель для теста',
    cacheBenchmarkStart: 'Запустить строгий A/B (6 вызовов)', cacheBenchmarkCostHint: 'Выбранная выше модель для теста будет вызвана 6 раз, что может расходовать квоту. Страницы Wiki не изменяются.',
    cacheBenchmarkPassed: 'Строгая проверка пройдена: три пары запросов совпадают, а Provider сообщил о попаданиях тёплого кеша.',
    cacheBenchmarkFailed: 'Строгая проверка не пройдена: {reason}', cacheBenchmarkRunFailed: 'Не удалось выполнить A/B кеша Wiki.',
    cacheBenchmarkSavings: 'Тёплая группа экономит {amount} относительно холодной ({percent}).', cacheBenchmarkIncrease: 'Тёплая группа дороже холодной на {amount} ({percent}).',
    coldCohort: 'Холодная группа', warmCohort: 'Тёплая группа', calls: 'Вызовы',
    status: { pending: 'Ожидание', running: 'Выполняется', success: 'Успешно', failed: 'Ошибка', invalid: 'Недействительный результат', unknown: 'Неизвестно' }
  },
  mcpSettings: {
