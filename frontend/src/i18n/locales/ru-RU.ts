export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'GRIDORA Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API GRIDORA — загрузки, URL, Markdown и поиск.',
      ...
      ecosystemNote: 'Skill размещён на ClawHub ({\'@\'}lyingbug/gridora). Полная документация API — на странице ClawHub.',
      installCta: 'Открыть ClawHub',
      installCtaHint: 'Установка GRIDORA Skill · откроется в новой вкладке',
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted GRIDORA: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В GRIDORA Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными GRIDORACloud.',
      ...
      hmacSecretDesc: 'Тот же secret, что в GRIDORA; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
      ...
      signedRequestStep0: '# 0. Подпишите JWT на backend (HS256, aud=gridora, sub=ID пользователя, tenant_id={tenantId}, exp не более 24 ч)',
  ...
  kbSettings: {
    parser: {
      ...
      engines: {
        ...
        gridoracloud: {
          name: 'GRIDORA Cloud',
          desc: 'Парсинг документов через GRIDORA Cloud'
        },
        paddleocr_vl_cloud: {
  ...
  system: {
    ...
    versionDescription: 'Версия сервиса приложения (gridora-app)',
    frontendVersionLabel: 'Версия UI',
    frontendVersionDescription: 'Версия сборки UI (gridora-ui)',
  ...
  createChat: {
    title: 'Привет, я GRIDORA — ваши знания всегда под рукой',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с GRIDORA',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте GRIDORA; скрипты и файлы остаются в том же контейнере',
      },
      ...
      templateLoadHint: 'Введите подключение к кластеру и обновите список. Если стандартного шаблона GRIDORA нет, нажмите «Создать»; после смены DNS или образа — «Удалить и пересобрать».',
      ...
      templateStepHint: 'На этом шаге только перечисляются шаблоны, уже есть в кластере; ничего не создаётся автоматически. Если стандартного шаблона нет, нажмите «Создать стандартный шаблон GRIDORA»; после смены DNS или образа — «Удалить и пересобрать». Продолжить можно, когда шаблон готов.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — GRIDORA пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      gridoraStandardTemplate: 'Стандартный шаблон GRIDORA',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон GRIDORA будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон GRIDORA. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: 'напр. gridora/images',
      prefixPlaceholder: 'напр. gridora',
    ...
    gridoraCloud: {
      title: 'GRIDORA Cloud',
      description: 'Настройте учётные данные APPID и APPSECRET для GRIDORA Cloud. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные GRIDORA Cloud истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения GRIDORA Cloud',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения GRIDORA Cloud',
      ...
      modelsSection: {
        ...
        descReady: 'Зарегистрируйте четыре стандартные модели GRIDORA Cloud для чата, поиска, реранкинга и зрения.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и GRIDORA автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в GRIDORA',
