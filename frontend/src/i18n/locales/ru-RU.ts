export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      skillUploadHint: 'Загрузка устанавливает навык в текущий образ и создаёт новый снимок. Это может занять несколько минут. Текущий ход диалога не прерывается; открытые сессии пересоздадут песочницу при следующем сообщении, и тогда черновики рабочей области сессии будут очищены.',
      skillUploadHintNewSession: 'Загрузка устанавливает навык в текущий образ и создаёт новый снимок. Это может занять несколько минут. Уже открытые сессии продолжают использовать текущую песочницу до своего завершения; этот навык появится только у новых сессий.',
  ...
  agent: {
    ...
    editor: {
      ...
      sandboxConfig: 'Песочница навыков',
      sandboxConfigDesc: 'Выберите песочницу, в которой будут выполняться скрипты навыков этого агента.',
      sandboxBackend: 'Конфигурация песочницы',
      sandboxBackendDefault: 'Не использовать песочницу',
      sandboxBackendHint: 'Действует только при включённом выполнении навыков. Если выбранная конфигурация будет удалена, выполнение навыков этого агента завершится ошибкой.',
      sandboxBackendMissing: 'Конфигурация удалена',
      sandboxNoConfigs: 'В этом пространстве ещё нет песочницы, поэтому скрипты навыков не будут выполняться. Добавьте её в «Настройки — Песочница навыков».',
      sandboxInfoTitle: 'Зачем выбирать конфигурацию?',
      sandboxInfoContent: 'Конфигурации отличаются аккаунтом, регионом и квотой. Когда песочница сессии уже создана, вложения, артефакты и удаление остаются привязаны к той конфигурации. Изменение здесь влияет только на песочницы, созданные позже.',
