  ...
  knowledgeEditor: {
    ...
    advanced: {
      ...
      multimodal: {
        ...
        imagePostProcessLabel: 'Постобработка изображений',
        imagePostProcessDescription: 'Когда включено, работает двухэтапный конвейер: сначала пакетная классификация и описание, затем OCR по таблице классов ниже, выбранные классы отключаются автоматически. Когда выключено — базовый режим: каждое изображение описывается и распознаётся по отдельности',
        imageBatchSizeLabel: 'Размер пакета описания',
        imageBatchSizeDescription: 'Сколько изображений объединять в один запрос описания. Экономит число запросов, но не токены — полезно для API с оплатой за запрос; 1 = по одному, максимум 16',
        imageDownscaleLabel: 'Уменьшение перед описанием',
        imageDownscaleDescription: 'Сжимает изображения до длинной стороны 640 px перед этапом описания, заметно сокращая токены; отключите для моделей, хорошо работающих с полным разрешением',
        imageClassPoliciesLabel: 'Политики классов',
        imageClassPoliciesDescription: 'Постобработка по классам: «Отключить» убирает ссылки класса из основного текста (изображение перестаёт попадать в модели через текст), при этом описание и OCR-текст всё равно создаются и остаются доступными для поиска; «OCR» запускает распознавание текста в изображении',
        policyDisabledCol: 'Отключить',
        policyOcrCol: 'OCR',
        imageClassChart: 'Диаграмма',
        imageClassDecorative: 'Декоративное',
        imageClassLogo: 'Логотип',
        imageClassPhoto: 'Фото',
        imageClassTableImage: 'Таблица (скриншот)',
        imageClassTextScreenshot: 'Скриншот текста',
        imageClassOther: 'Другое',
