export default {
  ...
  chat: {
    ...
    memoryHint: 'Это записи долговременной памяти, которые видел этот ответ. Удалённая запись больше не используется; профиль правится целиком в разделе «Моя память».',
    memoryOpenProfile: 'Открыть мой профиль',
    memoryEpisodeLoading: 'Загружаем изложение…',
    memoryEpisodeEmpty: 'У этого изложения нет текста.',
    memoryEpisodeFailed: 'Не удалось загрузить изложение.',
  ...
  memorySettings: {
    title: 'Моя память',
    description: 'То, что ассистент помнит о вас между разговорами: один профиль, изложение каждого разговора и фразы, которые вы попросили сохранить дословно. Всё это можно просмотреть, изменить и удалить.',
    workspaceDisabled: 'В этом пространстве долговременная память выключена. Переключатель начнёт действовать, когда её включит администратор.',
    enableLabel: 'Использовать долговременную память для меня',
    enableDescription: 'Когда выключено, ассистент не читает вашу память и ничего в неё не добавляет. Сохранённое остаётся и снова используется после включения.',
    agentDisabledHint: 'Отдельный агент тоже может выключить долговременную память у себя. В разговоре с таким агентом память не читается и не пополняется; на других агентов это не влияет.',
    usage: {
      title: 'Когда используется память',
      iconHint: 'Посмотреть, какие записи используются в разговоре',
      intro: 'Три вида содержимого попадают в разговор по-разному.',
      rows: {
        profile: {
          label: 'В каждом ответе',
          text: 'Профиль целиком добавляется к каждому ходу разговора'
        },
        episodes: {
          label: 'По теме вопроса',
          text: 'Изложения подбираются под текущий вопрос: чем чаще запись используется, тем легче найти её снова'
        },
        notes: {
          label: 'Дословно',
          text: 'Фразы, которые вы попросили сохранить; модель их не переписывает'
        }
      }
    },
    listTitle: 'Что сохранено',
    tabs: {
      profile: 'Профиль',
      episodes: 'Разговоры',
      notes: 'Дословные заметки'
    },
    export: 'Экспорт',
    exportTruncated: 'Сохранено больше, чем помещается в один файл, поэтому выгружена только часть.',
    rewrite: 'Переписать профиль',
    rewriteConfirm: 'Профиль будет заново составлен по недавним изложениям разговоров. Нынешний текст, включая ваши правки, будет заменён. Продолжить?',
    rewriteSuccess: 'Профиль переписан по изложениям разговоров: {count}',
    rewriteNothing: 'Профиль остался прежним',
    rewriteTooSoon: 'Профиль только что переписывался. Повторите чуть позже.',
    rewriteTooFewEpisodes: 'Пока слишком мало изложений разговоров, чтобы составить профиль',
    rewriteModelUnavailable: 'Модель недоступна, поэтому профиль остался без изменений',
    rewriteFailed: 'Не удалось переписать профиль',
    clear: 'Очистить всё',
    clearConfirm: 'Профиль, все изложения разговоров и все заметки будут удалены безвозвратно. Продолжить?',
    profile: {
      description: 'Общее представление ассистента о вас; добавляется к каждому ходу разговора. Текст можно править прямо здесь.',
      placeholder: 'Начинайте подзаголовок с ## и пишите под ним',
      emptyTitle: 'Профиля пока нет',
      emptyDescription: 'Когда наберётся несколько разговоров, по ним будет составлен профиль. После этого его можно править здесь в любой момент.',
      revision: 'Версия {revision}',
      builtFrom: 'Составлен по разговорам: {count}',
      updatedAt: 'Обновлён {time}',
      userEdited: 'Вы правили этот профиль. Следующая перезапись заменит ваш текст тем, что напишет модель.',
      length: '{count}/{max} символов',
      saved: 'Профиль сохранён',
      delete: 'Удалить профиль',
      deleteConfirm: 'Удалить профиль целиком? Дальнейшие разговоры перестанут его получать.',
      deleted: 'Профиль удалён'
    },
    episodes: {
      description: 'Когда разговор затихает, он записывается как изложение и потом вспоминается только при близком по теме вопросе.',
      emptyTitle: 'Изложений разговоров пока нет',
      emptyDescription: 'После того как разговор завершится и какое-то время будет тихо, он будет записан и появится здесь.',
      useCount: 'Использовано раз: {count}',
      outcomes: {
        success: 'Решено',
      ...
      summaryEmpty: 'У этого изложения нет текста.',
      summaryFailed: 'Не удалось загрузить изложение.',
      deleteConfirm: 'Удалить изложение этого разговора?',
      deleted: 'Изложение разговора удалено'
    },
    notes: {
      description: 'Фразы, которые вы прямо попросили запомнить. Они хранятся дословно, и модель их не переписывает.',
      placeholder: 'Напишите то, что нужно запомнить слово в слово',
      add: 'Добавить',
      added: 'Добавлено',
      count: '{count}/{max}',
      full: 'Сохранено предельное число заметок ({max}). Удалите одну, чтобы добавить новую.',
      emptyTitle: 'Дословных заметок пока нет',
      emptyDescription: 'Скажите в разговоре «запомни: …» или добавьте заметку выше.',
    ...
    kindHints: {
      digest: 'Из вашего профиля; добавляется к каждому ходу разговора',
      note: 'Фраза, которую вы просили сохранить дословно',
      episode: 'Изложение прежнего разговора'
    },
    toasts: {
      enabled: 'Долговременная память включена',
      disabled: 'Долговременная память выключена',
      cleared: 'Удалено записей памяти: {count}',
  ...
  memoryWorkspaceSettings: {
    ...
    interestThresholdLabel: 'Сколько разговоров считать возвращением к теме',
    interestThresholdDescription: 'Ключевое слово должно встретиться в стольких разных разговорах, чтобы считаться темой, к которой человек возвращается. Значение 1 засчитывает каждый случайный вопрос и обычно слишком шумно.',
    ...
    maxEpisodesLabel: 'Лимит изложений разговоров на участника',
    maxEpisodesDescription: 'При превышении удаляются наименее используемые изложения. Профиль и дословные заметки это не затрагивает.',
    toasts: {
