  ...
  knowledgeEditor: {
    ...
    advanced: {
      ...
      multimodal: {
        ...
        vllmChainLabel: 'Порядок моделей VLLM',
        vllmChainDescription: 'Модели понимания изображений пробуются по порядку; при ошибке, тайм-ауте или пустом ответе автоматически используется следующая',
        vllmChainPrimaryLabel: 'Основная VLLM модель',
        vllmChainFallbackLabel: 'Резервная модель',
        fallbackVllmPlaceholder: 'Выберите резервную VLLM модель',
        vllmChainAdd: 'Добавить резервную модель',
        vllmChainHint: 'Пробуются по порядку, максимум {max}; при ошибке, тайм-ауте или пустом ответе выполняется переход к следующей',
        vllmChainMoveUp: 'Вверх',
        vllmChainMoveDown: 'Вниз',
        vllmChainRemove: 'Удалить',
        vllmChainDuplicate: 'Эта VLLM модель уже есть в списке',
