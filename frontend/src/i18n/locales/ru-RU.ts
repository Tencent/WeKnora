...
/example\.(net|org)/`,
    ...
    globalSettings: {
      ...
      keyLabels: {
        ...
        model: {
		  ...
		  requests_per_minute: 'RPM/QPM модели по умолчанию',
		  tokens_per_minute: 'TPM модели по умолчанию',
		  interactive_concurrency_reserve: 'Резерв интерактивной параллельности',
        },
      },
      keyDescriptions: {
        auth: {
        ...
        model: {
          max_concurrency:
			'Общий лимит параллельности группы квоты модели, включая интерактивные и фоновые вызовы.',
		  requests_per_minute: 'RPM/QPM по умолчанию для группы квоты; 0 означает отсутствие глобального лимита.',
		  tokens_per_minute: 'TPM по умолчанию с резервированием до вызова и сверкой usage; 0 означает отсутствие лимита.',
		  interactive_concurrency_reserve: 'Слоты для интерактивных вызовов, которые не может занять фоновая индексация.',
        },
      },
      enumLabels: {
        auth: {
  ...
  model: {
    ...
    editor: {
	  ...
	  contextWindowPlaceholder: 'например: 128000',
	  contextWindowDesc: 'Общая ёмкость контекста для ввода и вывода, используемая при сжатии агента и предварительной проверке квоты токенов. Пустое значение использует настройку сервера.',
	  quotaGroupLabel: 'Группа квоты',
	  quotaGroupPlaceholder: 'например: openai-prod-account',
	  quotaGroupDesc: 'Моделям с одной учётной записью или пулом квоты задайте одинаковое имя. Пустое значение ограничивает каждую модель отдельно.',
	  maxConcurrencyLabel: 'Общий лимит параллельности',
	  maxConcurrencyPlaceholder: '0 — глобальный, -1 — отключить',
	  maxConcurrencyDesc: 'Учитывает интерактивные и фоновые вызовы; Redis разделяет лимит между репликами.',
	  rpmLabel: 'Запросов в минуту (RPM/QPM)',
	  tpmLabel: 'Токенов в минуту (TPM)',
	  quotaLimitPlaceholder: '0 — глобальный, -1 — отключить',
	  rpmDesc: 'Ограничивает скорость запросов группы квоты и допускает короткие всплески.',
	  tpmDesc: 'До вызова резервирует полный ввод и бюджет вывода, затем сверяет usage провайдера.',
	  interactiveReserveLabel: 'Резерв интерактивной параллельности',
	  interactiveReservePlaceholder: '0 — глобальный, -1 — отключить',
	  interactiveReserveDesc: 'Слоты для пользовательских диалогов, недоступные фоновой индексации.',
