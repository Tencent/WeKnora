export default {
  localBrowser: {
    ...
    "settingsDescription": "Подключите Chrome один раз для работы в разных диалогах.",
    ...
    "connectionNote": "Одно подключение для всех диалогов пространства. После перезапуска сервера расширение подключится автоматически.",
    "resumeNote": "Прерванные задачи остаются на паузе. Продолжите их через предпросмотр в диалоге.",
    "privacyNote": "Задачи используют отмеченную группу вкладок; существующим вкладкам нужно разрешение. Содержимое страниц и снимки передаются в WeKnora.",
    "installHelp": "Установка и использование",
    "installSteps": "Распакуйте расширение, включите режим разработчика на странице расширений Chrome и загрузите папку. После подключения опишите задачу в чате. Нажмите предпросмотр для перехода к вкладке задачи.",
    "running": "Выполняется",
  ...
  tenantMember: {
    ...
    audit: {
      ...
      description: 'Регистрирует события безопасности, изменения участников и очищенные команды терминала в этом пространстве, начиная с новых. Повторные отказы в течение минуты дедуплицируются.',
      ...
      action: {
        ...
        'sandbox.terminal_command': 'Команда терминала завершена'
      },
      columns: {
        time: 'Время',
        actor: 'Инициатор',
