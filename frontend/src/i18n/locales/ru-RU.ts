export default {
  ...
  mcpMetadata: {
    ...
    "unsyncedSaveHint": "Можно вручную указать назначение и сохранить сервис до синхронизации инструментов. Позднее они будут обнаружены при запросе.",
  ...
  mcpServiceDialog: {
    ...
    customHeaders: {
      ...
      desc: 'HTTP-заголовки для запросов MCP. Можно использовать user.email, external.user_id, im.user_id или request.headers.X-AAA; если значения нет, заголовок опускается. Экранирование буквального начала шаблона описано в руководстве API.',
      add: 'Добавить заголовок',
      keyPlaceholder: 'Имя заголовка',
      valuePlaceholder: 'Значение заголовка',
      insertVariable: 'Вставить идентификатор',
      metadataContext: 'Каталог инструментов общий для рабочей области; динамические заголовки разделяют только соединения. В настройках отсутствующий бизнес-заголовок опускается. Если сервер не отдаёт инструменты, можно вручную указать назначение и сохранить. В нативном IM нет request.headers; используйте im.user_id.',
      errors: {
        invalidName: 'Введите допустимое имя HTTP-заголовка.',
        duplicateName: 'Имена заголовков должны быть уникальными без учёта регистра.',
        invalidValue: 'Значение заголовка не может содержать управляющие символы.',
        unclosedExpression: 'В выражении шаблона отсутствует закрывающий разделитель.',
        invalidVariable: 'Шаблон содержит неподдерживаемую переменную или выражение.',
        protectedHeader: 'Заголовки учётных данных, идентификации и транспорта нельзя читать или задавать динамически.'
      }
    },
    toasts: {
