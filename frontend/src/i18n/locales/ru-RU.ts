...
/example\.(net|org)/`,
    ...
    globalSettings: {
      ...
      keyLabels: {
        ...
        model: {
          ...
          max_rpm: 'Лимит запросов в минуту модели по умолчанию',
          max_tpm: 'Лимит токенов в минуту модели по умолчанию',
        },
      },
      keyDescriptions: {
        auth: {
        ...
        model: {
          ...
          max_rpm:
            'Лимит по умолчанию на количество фоновых запросов в минуту к одной модели, привязанный к ID модели и общий для всех реплик, для соответствия квоте RPM провайдера. Применяется сразу без перезапуска. 0 или отрицательное значение отключает лимит по умолчанию (каждая модель всё равно соблюдает собственный лимит). Влияет только на фоновые задачи, не на интерактивный чат.',
          max_tpm:
            'Лимит по умолчанию на количество фоновых токенов в минуту к одной модели, привязанный к ID модели и общий для всех реплик, для соответствия квоте TPM провайдера (лимит, которого обогащение больших документов достигает первым). Применяется сразу без перезапуска. 0 или отрицательное значение отключает лимит по умолчанию (каждая модель всё равно соблюдает собственный лимит). Влияет только на фоновые задачи, не на интерактивный чат.',
        },
      },
      enumLabels: {
        auth: {
  ...
  model: {
    ...
    editor: {
      ...
      maxRpmLabel: 'Лимит фоновых запросов в минуту (RPM)',
      maxTpmLabel: 'Лимит фоновых токенов в минуту (TPM)',
      maxRatePlaceholder: '0 — использовать глобальное значение',
      maxRpmDesc: 'Ограничивает число фоновых запросов в минуту (индексация/обогащение) к этой модели, общее для модели по всем репликам, для соответствия квоте RPM провайдера. 0 или пусто — используется глобальное значение по умолчанию; интерактивный чат не затрагивается.',
      maxTpmDesc: 'Ограничивает число фоновых токенов в минуту (индексация/обогащение) к этой модели, общее для модели по всем репликам, для соответствия квоте TPM провайдера (лимит, которого обогащение больших документов достигает первым). 0 или пусто — используется глобальное значение по умолчанию; интерактивный чат не затрагивается.',
