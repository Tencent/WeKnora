export default {
  ...
  agentEditor: {
    ...
    im: {
      ...
      replyLanguageFixed: 'Фиксированный язык',
      replyLanguageFollowUser: 'Следовать языку пользователя',
      replyLanguageFollowHint: 'Язык определяется для каждого сообщения. Для коротких или неясных сообщений используется последний определённый язык беседы, затем язык развёртывания по умолчанию.',
