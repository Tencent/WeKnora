...
/example\.(net|org)/`,
  ...
  datasource: {
    ...
    field: {
      ...
      operatorId: 'UnionId оператора (необязательно)',
      operatorIdHint: 'Идентификатор пользователя, от имени которого вызываются API базы знаний. Оставьте пустым, чтобы автоматически использовать UnionId администратора организации (требуется право qyapi_get_member).',
    ...
    prereqStep1Desc_dingtalk: 'Войдите в консоль разработчика DingTalk → Разработка приложений → Создать приложение, затем скопируйте Client ID (AppKey) и Client Secret (AppSecret) в разделе «Учётные данные»',
    prereqStep2Brief_dingtalk: 'Выдайте необходимые права',
    prereqStep2Desc_dingtalk: 'В разделе «Управление правами» приложения включите Wiki.Workspace.Read и Wiki.Node.Read (чтение баз знаний), Storage.File.Read (экспорт/скачивание документов, категория «Хранилище») и qyapi_get_member (автоопределение оператора)',
    prereqStep3Brief_dingtalk: '(Опционально) Укажите UnionId оператора',
    prereqStep3Desc_dingtalk: 'API базы знаний авторизуются от имени оператора и синхронизируют только доступный ему контент; оставьте пустым, чтобы использовать администратора организации',
