export default {
  ...
  integrations: {
    ...
    api: {
      ...
      editApiKeyScopeDesc: "Change the name, permissions, and user identity configuration of API key \"{name}\".",
      ...
      keyIdentityDesc: "Applies to this API key. Isolates sessions and MCP OAuth grants; permissions remain controlled by the key scope.",
      keySignedHint: "Sign a short-lived HS256 JWT on your backend with sub, tenant_id, aud=weknora, exp, and this key’s identity_namespace. Save to see the signing example.",
      keySecretDraftHint: "Copy the secret to your backend. It takes effect when you create or save the key, replacing this key’s previous signing secret.",
      keyModeChangeHint: "Changing identity mode does not carry over sessions or MCP grants from the previous mode.",
      keyExampleDesc: "Select an API key to view its identity configuration, request examples, and Playground.",
      identityNamespace: "Application identity namespace",
      identityApplication: "Application identity",
      identityApplicationHint: "New applications are isolated by default. To rotate a key for the same application, select an existing key to retain its identities, sessions, and MCP grants.",
      identityNewApplication: "New application (isolated)",
      identityReuseHint: "Copies the selected key’s identity mode and signing configuration. Permissions follow this form. The new key can be edited independently.",
      legacyIdentityHint: "This key preserves its pre-upgrade identity. Matching external user IDs may share sessions and MCP grants with other legacy keys.",
      ...
      modeTenant: "No end-user identity",
      ...
      requireDirectHeaderDesc: "When enabled, requests without a user ID are rejected. Otherwise they share the application identity. Legacy keys retain their workspace fallback.",
