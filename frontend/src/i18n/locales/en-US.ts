...
\
  ...
  datasource: {
    ...
    noResourcesDesc_dingtalk: 'The app needs DingTalk wiki access, and the operator user must be able to access the target wiki space',
    ...
    guideStep1_dingtalk: 'Make sure the DingTalk app has wiki and member-read API permissions',
    guideStep2_dingtalk: 'Make sure the operator User ID can access the target DingTalk wiki space',
    guideStep3_dingtalk: 'Come back and click Retry, then select the wiki space or folder to sync',
    ...
    connectorDesc: {
      ...
      dingtalk: 'Sync documents from DingTalk wiki spaces',
    ...
    field: {
      ...
      operatorUserIdHint: 'Used to read DingTalk wiki content on behalf of this user; the app resolves its Union ID automatically.',
      dingtalkOnlineDocFetcher: 'Online document fetcher',
      dingtalkFetcherBlocks: 'Structured block read',
      dingtalkFetcherExport: 'Markdown auto export',
      dingtalkBlocksFetcherHint: 'Structured block read does not require gray/allowlisted scopes and is easier to configure, but it converts DingTalk blocks on a best-effort basis, so images and complex layouts may lose fidelity.',
      dingtalkExportFetcherHint: 'Markdown auto export uses DingTalk official export results, so images and complex content are more complete, but it requires AI assistant, event subscription, and gray/allowlisted scope setup from the official guide.',
      dingtalkMarkdownExportHint: 'Online document Markdown auto export requires DingTalk to enable the gray/allowlisted Document.AIAssistant.Read scope. Without it, only uploaded files or block reading can be synced.',
      dingtalkOnlineDocFetcherHint: 'To use uploaded file download, make sure the DingTalk organization has completed enterprise verification. If sync returns orgAuthLevelNotEnough, upgrade enterprise verification and retry.',
    ...
    prereqStep1Brief_dingtalk: 'Create or open a DingTalk internal app',
    prereqStep1Desc_dingtalk: 'Open the app in DingTalk Open Platform and copy Client ID / AppKey and Client Secret / AppSecret',
    prereqStep2Brief_dingtalk: 'Grant required API permissions',
    prereqStep2Desc_dingtalk: 'Grant member-read, wiki-read, wiki-node-read, enterprise file-read, and DingTalk document-read permissions. Uploaded file download requires enterprise verification; online document Markdown auto export requires the gray/allowlisted Document.AIAssistant.Read scope and the dingdoc_export_finish event subscription.',
    prereqStep3Brief_dingtalk: 'Enter the operator User ID',
    prereqStep3Desc_dingtalk: 'The operator must be able to access the target wiki; the app uses this identity to list spaces and sync docs.',
    prereqOpenConsole_dingtalk: 'Open DingTalk Open Platform',
    prereqMarkdownExportGuide_dingtalk: 'View Markdown export setup guide',
