export default {
  ...
  metadata: {
    settingsTitle: 'Document metadata',
    settingsDescription: 'Define classification, filtering fields, and automatic fill rules for this knowledge base.',
    createDefinition: 'New field', editDefinition: 'Edit field', editDocumentMetadata: 'Edit metadata',
    name: 'Field name', namePlaceholder: 'For example: Document type', description: 'Description', type: 'Type', governance: 'Rules',
    required: 'Required', requiredDescription: 'Empty values are flagged without blocking import or retrieval',
    filterable: 'Filterable', filterableDescription: 'Allow this field in document and retrieval filters',
    automaticRule: 'Automatic fill', automaticRuleDescription: 'Create candidate values from source fields or AI extraction', manualOnly: 'Manual only',
    emptyDefinitions: 'No metadata fields defined', noDescription: 'No description', typeLocked: 'Type is locked after values or an automatic rule exist',
    options: 'Allowed values', addOption: 'Add option', moveUp: 'Move up', moveDown: 'Move down',
    archiveTitle: 'Archive metadata field', archiveDescription: 'Archiving “{name}” removes it from editing and filtering while preserving existing values.', archiveAction: 'Archive',
    ruleStrategy: 'Fill method', sourceKey: 'Source field', sourceKeyPlaceholder: 'For example: product_line', instruction: 'Extraction instruction', modelId: 'Model ID', modelIdPlaceholder: 'Uses the KB summary model when empty',
    filter: 'Metadata', chatFilter: 'Knowledge metadata filters', chatFilterDescription: 'Set conditions per knowledge base to constrain retrieval for this message.', noFilterableFields: 'No filterable fields', filterTitle: 'Metadata filters', filterDescription: 'Conditions on different fields use AND logic.', filterEmpty: 'Add a field to narrow the document scope', addCondition: 'Add filter field', applyFilter: 'Apply filters', noValueNeeded: 'No value required',
    trueValue: 'Yes', falseValue: 'No', documentTitle: 'Document metadata', documentFields: 'Field status', incompleteCount: '{count} required fields incomplete', allComplete: 'Required fields complete', confirmAll: 'Confirm all', confirmOne: 'Confirm', rerunAutoFill: 'Rerun automatic fill', allowAutoOverwrite: 'Allow automatic overwrite',
    overwritePromptTitle: 'Save manual change', overwritePromptDescription: 'Manual changes lock the current value by default. Enable automatic overwrite to allow future sync or extraction results to replace it.',
    types: { text: 'Text', singleSelect: 'Single select', multiSelect: 'Multi select', number: 'Number', date: 'Date', boolean: 'Boolean' },
    ruleStrategies: { source_mapping: 'Source mapping', llm_extract: 'AI extraction' },
    operators: { equals: 'Equals', contains: 'Contains', in: 'Is one of', contains_any: 'Contains any', contains_all: 'Contains all', eq: 'Equals', gt: 'Greater than', gte: 'At least', lt: 'Less than', lte: 'At most', between: 'Between', on: 'On', before: 'Before', after: 'After', is_empty: 'Is empty', is_not_empty: 'Is not empty' },
    status: { incomplete: 'Incomplete', automatic: 'Automatic', pending: 'Pending review', confirmed: 'Confirmed', archived: 'Archived', overwriteAllowed: 'Auto overwrite on', overwriteLocked: 'Locked' },
    messages: { loadFailed: 'Failed to load metadata fields', nameRequired: 'Enter a field name', optionRequired: 'Add at least one complete option', optionDuplicate: 'Option labels must be unique', sourceKeyRequired: 'Enter a source field', instructionRequired: 'Enter extraction instructions', saved: 'Metadata field saved', saveFailed: 'Failed to save metadata field', archived: 'Metadata field archived', archiveFailed: 'Failed to archive field', sortFailed: 'Failed to save field order', documentLoadFailed: 'Failed to load document metadata', valueSaved: 'Metadata saved', valueSaveFailed: 'Failed to save metadata', versionConflict: 'This field changed elsewhere. The latest value has been loaded.', policySaveFailed: 'Failed to save overwrite policy', confirmed: 'Automatic values confirmed', confirmFailed: 'Failed to confirm values', autoFillQueued: 'Automatic fill queued', autoFillFailed: 'Failed to queue automatic fill' },
  },
  knowledgeBase: {
  ...
  agent: {
    ...
    editor: {
      ...
      sandboxConfig: 'Skill sandbox',
      sandboxConfigDesc: 'Choose which sandbox this agent\'s skill scripts run in.',
      sandboxBackend: 'Sandbox config',
      ...
      sandboxNoConfigs: 'This workspace has no sandbox yet, so skill scripts will not run. Add one under Settings - Skill Sandbox.',
      sandboxInfoTitle: 'Why pick a config?',
      sandboxInfoContent: 'Configs differ in account, region and quota. Once a session\'s sandbox exists, its attachments, artifacts and teardown stay pinned to the config it was created on — changing this only affects sandboxes created afterwards.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      pageHint: 'A skill sandbox is the isolated environment where agent skill scripts run. A workspace can have several configs (Docker, E2B, CubeSandbox, or a local process); each agent picks one. Scripts stay disabled until a config is selected.',
      ...
      skillUploadClick: 'Click to upload a skill bundle',
      skillUploadDrag: 'or drop a .zip file here',
      skillUploadHint: 'Upload installs the skill onto the current image and takes a new snapshot. This can take several minutes. The current chat turn is not interrupted; open sessions rebuild their sandbox on the next turn, which clears the session workspace scratch.',
      skillUploadHintNewSession: 'Upload installs the skill onto the current image and takes a new snapshot. This can take several minutes. Already-open sessions keep their current sandbox until they end; only newly started sessions pick up this install.',
      ...
      skillEmpty: 'No skills installed yet. Upload a zip bundle to install one.',
