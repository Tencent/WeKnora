...
\
  ...
  datasource: {
    ...
    field: {
      ...
      operatorId: 'Operator UnionId (optional)',
      operatorIdHint: 'The user identity used when calling knowledge base APIs. Leave empty to use the org admin\'s UnionId automatically (requires the qyapi_get_member contact scope).',
    ...
    prereqBarText_dingtalk: 'First time? See the DingTalk app setup guide',
    prereqStep1Brief_dingtalk: 'Create a DingTalk internal app',
    prereqStep1Desc_dingtalk: 'Log in to the DingTalk developer console → App Development → Create App, then copy the Client ID (AppKey) and Client Secret (AppSecret) from "Credentials & Basic Info"',
    prereqStep2Brief_dingtalk: 'Grant the required scopes',
    prereqStep2Desc_dingtalk: 'In the app\'s Permission Management, enable Wiki.Workspace.Read and Wiki.Node.Read (knowledge base read), Storage.File.Read (document export/download, under the Storage category), and qyapi_get_member (auto-resolve operator identity)',
    prereqStep3Brief_dingtalk: '(Optional) Specify an operator UnionId',
    prereqStep3Desc_dingtalk: 'Knowledge base APIs authorize as the operator and only sync content that user can access; leave empty to use the org admin automatically',
