export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      networkHint: 'Controls inbound and outbound networking for every sandbox using this configuration. Changes affect only newly created sandboxes; existing sandboxes keep their policy until reclaimed.',
      inboundAccess: 'Inbound access',
      inboundAccessHelp: 'Credentials are required by default: the public sandbox URL must carry the token issued at creation.',
      inboundTokenRequired: 'Require credentials (default)',
      inboundPublic: 'Publicly accessible',
      ...
      cubeL7RulesHelp: 'Every rule requires host or sni; the network layer derives allowed targets only from those fields. Fields are AND-ed and methods are OR-ed. Applies only to HTTP 80 / HTTPS 443.',
