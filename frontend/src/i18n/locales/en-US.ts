export default {
  ...
  knowledgeEditor: {
    ...
    desensitization: {
      title: 'Document Desensitization',
      description: 'Mask parsed markdown before chunking, embedding, and summarization. Off by default. Cloud parsers cannot be used while this is enabled.',
      coverageTitle: 'Complete detection is not guaranteed',
      coverageNotice: 'The built-in engine only masks selected presets and custom regex. Images, scans, and original uploads are not processed. Unmatched text is stored, embedded, and used in Q&A as-is. Use Try it and sample reviews; this is not a substitute for human checks.',
      enableLabel: 'Enable text desensitization',
      enableDescription: 'Only parsed text is masked. Original files and images are not modified.',
      engineLabel: 'Engine',
      engineDescription: 'The built-in engine uses regex and checksums. Presidio still runs those rules first, then sends the original text to the sidecar.',
      engineBuiltin: 'Built-in (regex + checksum)',
      enginePresidio: 'Presidio sidecar',
      presidioFlowTitle: 'Builtin first, then sidecar',
      presidioFlowNotice: '1. Mask with the builtin rules below. 2. The app POSTs the original parsed Markdown to the Analyzer. 3. Spans are mapped to the selected presets and replaced. If the sidecar is missing or has no Chinese recognizers, ingest fails. Preview on this page runs step 1 only.',
      presidioApiLabel: 'Sidecar endpoint',
      presidioApiUrlHint: 'The URL comes from PRESIDIO_ANALYZER_URL in the deployment env. It is not set per knowledge base.',
      presidioRequestLabel: 'Request example',
      presidioResponseLabel: 'Response example',
      engineLLM: 'Local LLM',
      llmModelLabel: 'LLM',
      llmModelDescription: 'Leave empty to reuse the knowledge-base summary model. Used only when the engine is Local LLM.',
      llmModelPlaceholder: 'Select a model',
      maskStyleLabel: 'Mask style',
      maskStyleDescription: 'Placeholder replacement such as <ID> or partial masking such as 138****8000.',
      maskReplace: 'Placeholder',
      maskPartial: 'Partial mask',
      entityTypesLabel: 'Preset types',
      entityTypesDescription: 'Mainland China identifiers with format or checksum validation. Selected types appear below as read-only regex rules.',
      entities: {
        cn_id_card: 'National ID card',
        cn_mobile: 'Mainland mobile number',
        cn_landline: 'Landline',
        cn_bank_card: 'Bank card (UnionPay)',
        cn_uscc: 'Unified social credit code',
        cn_plate: 'Civil license plate',
        cn_plate_std: 'License plate (standard)',
        cn_plate_nev: 'License plate (NEV)',
        email: 'Email'
      },
      presetReadonly: 'Preset',
      customRulesLabel: 'Custom regex',
      customRulesDescription: 'Applied after preset types. Replacement may be empty; the rule name is used by default.',
      customRulesTitle: 'Test thoroughly before saving',
      customRulesNotice: 'Over-broad patterns cause false positives (order IDs or serials get masked). Over-narrow patterns miss real values. Run Try it on real samples and check both hits and non-hits.',
      ruleName: 'Name',
      rulePattern: 'Pattern',
      ruleReplacement: 'Replacement (optional)',
      addRule: 'Add rule',
      previewLabel: 'Try it',
      previewDescription: 'Run the current presets and custom rules on sample text. Preview uses the builtin engine only and does not represent Presidio ingest.',
      previewBuiltinOnlyTitle: 'Preview is builtin-only',
      previewBuiltinOnlyNotice: 'Presidio is not called from this page. Ingest follows the sidecar; verify with a real document.',
      previewPlaceholder: 'Paste text that contains a phone number or ID card…',
      previewRun: 'Preview',
      previewEmpty: 'Enter sample text first',
      previewFailed: 'Desensitization preview failed',
      cloudParserForbidden: 'Cloud parsers cannot be used when desensitization is enabled. Switch to anydoc, simple, or builtin.'
    },
    asr: {
