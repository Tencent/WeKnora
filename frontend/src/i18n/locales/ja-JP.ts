export default {
  ...
  integrations: {
    ...
    api: {
      ...
      editApiKeyScopeDesc: "APIキー「{name}」の名前、権限範囲、ユーザー識別設定を変更します。",
      ...
      keyIdentityDesc: "この API キーにのみ適用されます。セッションと MCP OAuth 認可を分離し、操作権限はキーの権限範囲で制御します。",
      keySignedHint: "バックエンドで sub、tenant_id、aud=weknora、exp、このキーの identity_namespace を含む短期 HS256 JWT を署名します。保存後に署名例を確認できます。",
      keySecretDraftHint: "シークレットをバックエンドにコピーしてください。作成または保存すると有効になり、このキーの以前の署名シークレットを置き換えます。",
      keyModeChangeHint: "識別モードを変更すると、以前のモードのセッションや MCP 認可は引き継がれません。",
      keyExampleDesc: "API キーを選択して、識別設定、リクエスト例、Playground を確認します。",
      identityNamespace: "アプリの識別名前空間",
      identityApplication: "アプリの識別",
      identityApplicationHint: "新しいアプリは既定で分離されます。同じアプリのキーを更新する場合、既存キーを選ぶとユーザー、セッション、MCP 認可を引き継げます。",
      identityNewApplication: "新しいアプリ（分離）",
      identityReuseHint: "選択したキーの識別モードと署名設定をコピーします。操作権限はこのフォームに従います。作成後は個別に編集できます。",
      legacyIdentityHint: "このキーは更新前の識別情報を保持します。同じ外部ユーザー ID は他の旧キーとセッションや MCP 認可を共有する場合があります。",
      ...
      requireDirectHeaderDesc: "有効にするとユーザー ID のないリクエストを拒否します。無効の場合はアプリの共通識別を使用します。旧キーは従来のワークスペース識別を保持します。",
