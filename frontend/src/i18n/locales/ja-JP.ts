export default {
  ...
  knowledgeEditor: {
    ...
    desensitization: {
      title: '文書マスキング',
      description: '解析後の Markdown をチャンク、埋め込み、要約の前にマスクします。既定はオフです。有効時はクラウドパーサーを使えません。',
      coverageTitle: '完全な検出は保証しません',
      coverageNotice: '内蔵エンジンは選択したプリセットとカスタム正規表現のみ処理します。画像・スキャン・原本は対象外です。未検出のテキストはそのままチャンク、ベクトル、Q&A に入ります。試行と抜き取り確認が必要で、人手の点検の代わりにはなりません。',
      enableLabel: 'テキストマスキングを有効化',
      enableDescription: '解析テキストのみ処理します。原本と画像は変更しません。',
      engineLabel: 'エンジン',
      engineDescription: '内蔵エンジンは正規表現とチェックサムを使います。Presidio でも先に内蔵規則を実行し、原文をサイドカーへ送ります。',
      engineBuiltin: '内蔵（正規表現 + チェックサム）',
      enginePresidio: 'Presidio サイドカー',
      presidioFlowTitle: '先に内蔵、次にサイドカー',
      presidioFlowNotice: '1. 下の内蔵規則でマスク。2. App が解析 Markdown 原文を Analyzer へ POST。3. start/end/entity_type を選択済みプリセットへ写して置換。サイドカー未設定や中国語認識器がないと取り込み失敗。このページの試行は手順 1 のみ。',
      presidioApiLabel: 'サイドカー API',
      presidioApiUrlHint: 'URL は環境変数 PRESIDIO_ANALYZER_URL。知識ベースには書きません。',
      presidioRequestLabel: 'リクエスト例',
      presidioResponseLabel: 'レスポンス例',
      engineLLM: 'ローカル LLM',
      llmModelLabel: 'LLM',
      llmModelDescription: '空欄時はナレッジベースの要約モデルを使います。',
      llmModelPlaceholder: 'モデルを選択',
      maskStyleLabel: 'マスク形式',
      maskStyleDescription: '<身份证> のようなプレースホルダー、または 138****8000 のような部分マスク。',
      maskReplace: 'プレースホルダー',
      maskPartial: '部分マスク',
      entityTypesLabel: 'プリセット種別',
      entityTypesDescription: '中国大陸の識別子。複数選択可。選択した種別は下に読み取り専用の正規表現として表示されます。',
      entities: {
        cn_id_card: '住民身分証',
        cn_mobile: '中国大陸の携帯番号',
        cn_landline: '固定電話',
        cn_bank_card: '銀行カード（銀聯）',
        cn_uscc: '統一社会信用コード',
        cn_plate: '民用ナンバー',
        cn_plate_std: 'ナンバー（普通）',
        cn_plate_nev: 'ナンバー（新能源）',
        email: 'メール'
      },
      presetReadonly: 'プリセット',
      customRulesLabel: 'カスタム正規表現',
      customRulesDescription: 'プリセットの後に適用します。置換は空でもよく、既定はルール名です。',
      customRulesTitle: '保存前に十分検証してください',
      customRulesNotice: '広すぎると注文番号などが誤ってマスクされ、狭すぎると漏れます。このページの試行と実サンプルで、当たるべき値と当たってはいけない値を確認してください。',
      ruleName: '名前',
      rulePattern: '正規表現',
      ruleReplacement: '置換（任意）',
      addRule: 'ルールを追加',
      previewLabel: '試行',
      previewDescription: '現在のプリセットとカスタムルールでサンプルをマスクします。試行は内蔵エンジンのみで、Presidio 取り込みを代表しません。',
      previewBuiltinOnlyTitle: '試行は内蔵エンジンのみ',
      previewBuiltinOnlyNotice: 'Presidio 選択時はこのページからサイドカーを呼びません。取り込み結果は実文書で確認してください。',
      previewPlaceholder: '携帯番号や身分証を含むテキストを貼り付けてください…',
      previewRun: 'プレビュー',
      previewEmpty: '先にテキストを入力してください',
      previewFailed: 'プレビューに失敗しました',
      cloudParserForbidden: 'マスキング有効時はクラウドパーサーを使えません。anydoc、simple、または builtin に切り替えてください。'
    },
    asr: {
      title: '音声認識',
