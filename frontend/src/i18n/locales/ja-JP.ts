export default {
  ...
  skillDiscovery: {
    ...
    communityHint: '固定バージョンを取り込み、インストール先のサンドボックスを選択します。標準イメージには含まれません。',
    cardUnknownHint: "このイメージには認識可能なスキル一覧がありません。サンドボックス設定でイメージまたはテンプレートを確認してください。",
    cardBuiltins: "内蔵スキル",
  ...
  settings: {
    ...
    skills: {
      ...
      addStepRegisterDesc: "リンク、ZIP、またはインストール指示からスキルを追加します。",
      addMethodSource: "ソースリンク",
      addMethodFile: "ZIPをアップロード",
      addMethodPrompt: "Promptでインストール",
      promptInstructions: "指示",
      promptInstallDesc: "サンドボックスを選択して開始します。進捗確認、追加指示、再試行ができます。",
      promptTitle: "インストール指示を入力",
      promptHint: "スキル名、手順書、コマンドや要件を入力します。サンドボックス選択後、Agentが手順書を読み、ツールを準備してスキルを取得・検証します。",
      promptPlaceholder: "例：手順書に従って指定IDのスキルをインストール。CLIを確認し、なければ導入する。",
