export default {
  ...
  agentStream: {
    ...
    skillCards: {
      found: '{count} 件のスキルが見つかりました',
      resolved: 'スキル {name} を解決しました',
      notFound: '該当するスキルが見つかりません',
      unavailable: '{sources} に接続できません',
      temporaryNotice: '{name} はこの会話で一時的に読み込まれただけで、ワークスペースにはインストールされていません。スキル一覧には表示されず、ほかの会話では使えません。',
      temporaryNoticeUnnamed: 'このスキルはこの会話で一時的に読み込まれただけで、ワークスペースにはインストールされていません。スキル一覧には表示されず、ほかの会話では使えません。',
      official: '公式',
      ...
      retry: '再試行',
      copySource: 'ソースをコピー',
      sourceCopied: 'スキルのソースをコピーしました',
      viewSource: '詳細を見る',
      adminOnly: 'ワークスペース管理者によるインストールが必要です。ソースをコピーして管理者に送ってください',
      noSandbox: 'この会話にはスキルをインストールできるサンドボックスがありません',
      alreadyInstalled: 'このサンドボックスには同名のスキルがインストール済みです。インストールすると置き換わります',
      alreadyInstalling: 'このサンドボックスで同名のスキルをインストール中です',
      sameNameInCatalog: 'ワークスペースに同名のスキルがあります。インストールするとこのソースで置き換わります',
      installing: 'インストール中 {percent}%',
      ready: 'インストールしました。次のメッセージから使えます',
      readyNewSession: 'インストールしました。新しい会話から使えます',
      selectInAgent: 'このエージェントは選択したスキルのみ使うため、エージェント設定で選択してください',
      failed: 'インストールに失敗しました',
      failedWithReason: 'インストールに失敗しました：{reason}',
      installerModelRequired: '先にスキルのインストールに使うモデルを選択してください',
      confirmTitle: 'スキル「{name}」をインストールしますか？',
      confirmBody: 'このサードパーティのスキルを {source} から取得し、この会話が使うサンドボックスにインストールします。インストール時にスキル付属のコードが実行され、以後そのサンドボックスを使うすべての会話で利用できます。この会話のサンドボックスはその後再構築され、/workspace の一時ファイルは失われます。信頼できるソースの場合のみ続行してください。',
      confirmBodyNewSession: 'このサードパーティのスキルを {source} から取得し、この会話が使うサンドボックスにインストールします。インストール時にスキル付属のコードが実行されます。このサンドボックスの更新ポリシーにより、使えるのはこの後に始めた会話だけで、進行中の会話は変わりません。信頼できるソースの場合のみ続行してください。'
    },
    skillFiles: {
      heading: 'スキルファイル',
      script: 'スクリプト',
