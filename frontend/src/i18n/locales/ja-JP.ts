export default {
  ...
  knowledgeBase: {
    ...
    urlLabel: 'リンク',
    urlPlaceholder: '1 行に 1 つずつリンクを入力してください。例:\nhttps://example.com/article\nhttps://www.youtube.com/watch?v=VIDEO_ID\nhttps://www.youtube.com/playlist?list=PLAYLIST_ID',
    urlTip: 'Web ページまたは YouTube のリンクを 1 行に 1 つずつ貼り付けてください。Web ページは取得して解析します。各 YouTube 動画は字幕（字幕がない場合は音声認識）をもとに個別のドキュメントになり、各 YouTube 再生リストは再生リスト名のフォルダに取り込まれます。&list= を含む動画リンクはその動画のみを取り込むため、再生リスト全体は再生リストのリンクを使用してください',
    urlPreviewWebPages: 'Web ページ {count} 件',
    urlPreviewYouTubeVideos: 'YouTube 動画 {count} 本',
    urlPreviewYouTubePlaylists: 'YouTube 再生リスト {count} 件（それぞれ個別のフォルダへ）',
    urlPreviewInvalid: '無効な行 {count} 件（スキップされます）',
