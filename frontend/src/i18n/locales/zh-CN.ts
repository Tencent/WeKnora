export default {
  ...
  settings: {
    ...
    intentPolicy: {
      title: '意图策略',
      description: 'IntentGate 策略判定 Agent 的工具调用是否符合用户意图与约束。新策略一律从 observe 起步（只记录、不拦截）。',
      empty: '还没有意图策略',
      wholeTenant: '整个空间',
      riskHigh: '高风险',
      riskLow: '低风险',
      currentBadge: '当前版本',
      supersededBadge: '已被替代',
      scopes: {
        tool: '工具',
        service: 'MCP 服务',
        agent: 'Agent',
      ...
      editor: {
        createTitle: '新建意图策略',
        editTitle: '编辑策略（保存为 v{version}）',
      },
      form: {
        scopeType: '作用域',
        scopeRef: '作用域标识',
        constraintText: '约束（自然语言）',
        constraintPlaceholder: '例如：单笔退款不得超过 75',
        ruleExpr: '规则表达式（可选）',
        ruleExprPlaceholder: '例如：value <= 75 —— 可编译的约束完全不经过 LLM judge',
        argPath: '参数路径（可选）',
        riskTier: '风险档',
        mode: '模式',
      },
      scopeRefHints: {
        tool: 'service_id:tool_name；支持 *:wiki_* 前缀通配',
        service: 'MCP 服务 ID',
        agent: 'Agent ID',
        workspace: '工作区 ID',
      },
      enforceConfirm: {
        title: '切换为强制执行模式？',
        body: 'enforce 模式会真实拦截违反该策略的工具调用：Agent 会看到拒绝原因并自我修正；observe 模式只记录判定。确认继续？',
        confirm: '强制执行',
      },
      toasts: {
        loadFailed: '意图策略加载失败',
        enabled: '策略已启用',
        disabled: '策略已停用',
        toggleFailed: '策略状态更新失败',
        created: '策略已创建',
        updated: '新版本已保存',
        saveFailed: '策略保存失败',
        constraintRequired: '约束原文为必填项',
        scopeRefRequired: '该作用域类型必须填写作用域标识',
      },
    },
    versionInfo: '版本信息',
    taskQueue: '任务队列',
