export default {
  ...
  uploadConfirm: {
    ...
    overviewDesc: "确认解析配置后开始上传",
    basicSettings: "常用配置",
    advancedSettings: "高级能力",
    navParserDefault: "默认",
  ...
  datasource: {
    ...
    field: {
      ...
      operatorIdHint: "钉钉知识库 API 需要操作人身份。请填写有权访问目标知识库的用户 unionId（不是 userId）",
      dingtalkBaseUrlHint: "留空使用 https://api.dingtalk.com；专属钉钉或私有化部署请填写对应 API 域名",
    ...
    prereqBarText_dingtalk: "首次使用？点击查看钉钉文档接入指引",
    prereqStep1Brief_dingtalk: "创建企业内部应用并获取凭证",
    prereqStep1Desc_dingtalk: "登录钉钉开放平台 → 应用开发 → 创建企业内部应用 → 凭证与基础信息中复制 AppKey / AppSecret",
    prereqStep2Brief_dingtalk: "开通知识库与文档读权限",
    prereqStep2Desc_dingtalk: "至少开通 Document.Workspace.Read、Document.WorkspaceDocument.Read、Wiki.Node.Read，并发布应用",
    prereqStep3Brief_dingtalk: "填写操作人 unionId",
    prereqStep3Desc_dingtalk: "使用有权访问目标知识库的用户 unionId（可在通讯录/个人信息页查看；勿填 userId）",
    prereqOpenConsole_dingtalk: "前往钉钉开放平台",
    noResourcesDesc_dingtalk: "应用需要获得钉钉知识库访问权限，且操作人 unionId 需对目标知识库有读权限",
    guideStep1_dingtalk: "确认应用已开通知识库相关读权限并完成发布",
    guideStep2_dingtalk: "确认 operator_id 填写的是有权限的用户 unionId",
    guideStep3_dingtalk: "在钉钉中将应用或该用户加入目标知识库，然后回到这里重新加载",
