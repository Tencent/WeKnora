export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      pageHint: '技能沙箱是智能体执行技能脚本的隔离环境。一个空间可以添加多份配置（Docker、E2B、CubeSandbox 或本地进程），每个智能体选择一份；未选择时不会执行技能脚本。',
      ...
      skillUploadClick: '点击上传技能压缩包',
      skillUploadDrag: '或将 .zip 拖到此处',
      skillUploadHint: '上传后会在当前镜像上安装该技能并生成新快照，可能需要数分钟。当前这一轮对话不会中断；下一轮提问时才会按新镜像重建沙箱，会话工作区草稿会在那时清空。',
      skillUploadHintNewSession: '上传后会在当前镜像上安装该技能并生成新快照，可能需要数分钟。已打开的会话继续使用原沙箱，直到该会话结束；只有新打开的会话才会带上这次安装的技能。',
  ...
  agent: {
    ...
    editor: {
      ...
      sandboxConfig: '技能沙箱',
      sandboxConfigDesc: '选择该智能体的技能脚本在哪个沙箱中执行。',
      sandboxBackend: '沙箱配置',
      sandboxBackendDefault: '不启用沙箱',
      sandboxBackendHint: '仅在启用技能执行时生效。所选配置被删除后，该智能体的技能执行会失败。',
      sandboxBackendMissing: '配置已删除',
      sandboxNoConfigs: '当前空间还没有沙箱配置，技能脚本不会执行。可在「设置 - 技能沙箱」中添加。',
      sandboxInfoTitle: '为什么要选配置？',
      sandboxInfoContent: '不同配置对应不同的账号、区域与配额。同一会话的沙箱一旦创建，后续的附件、产物与销毁都会锁定在创建时那份配置上，改这里只影响之后新建的沙箱。',
  ...
  metadata: {
    settingsTitle: "文档元数据",
    settingsDescription: "定义本知识库的分类、筛选字段与自动填写规则。",
    createDefinition: "新建字段",
    editDefinition: "编辑字段",
    editDocumentMetadata: "编辑元数据",
    name: "字段名称",
    namePlaceholder: "例如：文档类型",
    description: "字段说明",
    type: "类型",
    governance: "规则",
    required: "必填",
    requiredDescription: "空值显示待完善，但不阻止导入和检索",
    filterable: "允许筛选",
    filterableDescription: "允许在文档列表和知识检索中使用该字段",
    automaticRule: "自动填写",
    automaticRuleDescription: "通过数据源字段或智能识别生成候选值",
    manualOnly: "仅人工填写",
    emptyDefinitions: "尚未定义元数据字段",
    noDescription: "暂无说明",
    typeLocked: "已有文档值或自动规则，类型不可修改",
    options: "允许值",
    addOption: "添加选项",
    moveUp: "上移",
    moveDown: "下移",
    archiveTitle: "归档元数据字段",
    archiveDescription: "归档“{name}”后将不再用于新编辑和筛选，已有文档值会保留。",
    archiveAction: "归档",
    ruleStrategy: "填写方式",
    sourceKey: "数据源字段",
    sourceKeyPlaceholder: "例如：product_line",
    instruction: "识别说明",
    modelId: "模型 ID",
    modelIdPlaceholder: "留空时使用知识库摘要模型",
    filter: "元数据",
    chatFilter: "知识库元数据筛选",
    chatFilterDescription: "每个知识库独立设置条件，发送时共同限制检索范围。",
    noFilterableFields: "没有可筛选字段",
    filterTitle: "元数据筛选",
    filterDescription: "不同字段之间使用 AND 条件。",
    filterEmpty: "添加字段以限制文档范围",
    addCondition: "添加筛选字段",
    applyFilter: "应用筛选",
    noValueNeeded: "无需填写值",
    trueValue: "是",
    ...
    documentTitle: "文档元数据",
    documentFields: "字段状态",
    incompleteCount: "{count} 个必填字段待完善",
    allComplete: "必填字段已完善",
    confirmAll: "确认全部",
    confirmOne: "确认",
    rerunAutoFill: "重新自动填写",
    allowAutoOverwrite: "允许自动覆盖",
    overwritePromptTitle: "保存人工修改",
    overwritePromptDescription: "人工修改会默认锁定当前值。需要后续同步或智能识别更新该字段时，可开启自动覆盖。",
    types: { text: "文本", singleSelect: "单选", multiSelect: "多选", number: "数字", date: "日期", boolean: "布尔值" },
    ruleStrategies: { source_mapping: "数据源映射", llm_extract: "智能识别" },
    operators: { equals: "等于", contains: "包含", in: "属于", contains_any: "包含任一", contains_all: "包含全部", eq: "等于", gt: "大于", gte: "大于等于", lt: "小于", lte: "小于等于", between: "介于", on: "日期为", before: "早于", after: "晚于", is_empty: "为空", is_not_empty: "不为空" },
    status: { incomplete: "待完善", automatic: "自动填写", pending: "待确认", confirmed: "已确认", archived: "已归档", overwriteAllowed: "可自动覆盖", overwriteLocked: "已锁定" },
    messages: { loadFailed: "元数据定义加载失败", nameRequired: "请输入字段名称", optionRequired: "请至少添加一个完整选项", optionDuplicate: "选项名称不能重复", sourceKeyRequired: "请输入数据源字段", instructionRequired: "请输入识别说明", saved: "元数据字段已保存", saveFailed: "元数据字段保存失败", archived: "元数据字段已归档", archiveFailed: "归档失败", sortFailed: "排序保存失败", documentLoadFailed: "文档元数据加载失败", valueSaved: "元数据已保存", valueSaveFailed: "元数据保存失败", versionConflict: "该字段已被更新，已加载最新值", policySaveFailed: "覆盖策略保存失败", confirmed: "自动填写值已确认", confirmFailed: "确认失败", autoFillQueued: "自动填写任务已提交", autoFillFailed: "自动填写任务提交失败" },
  },
  knowledgeBase: {
    title: '知识库',
    fileContent: '文件内容',
