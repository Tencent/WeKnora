export default {
  ...
  modelSettings: {
    ...
    usage: {
      ...
      hideDetails: '收起明细',
      partial: '部分计价',
      partialCost: '仅包含已配置单价的调用，不能视为全部成本',
      embeddingCache: '向量缓存',
      embeddingCacheHint: 'Embedding 输入从内存或持久缓存直接命中的比例',
      embeddingAvoided: '避免计算',
      embeddingAvoidedHint: '缓存命中和批内去重共同避免的向量计算次数',
      wikiSummary: 'Wiki 专项汇总',
      loading: '正在刷新模型用量…',
      loadFailed: '模型用量加载失败，模型配置本身不受影响。',
      retry: '重试',
      noData: '所选区间暂无模型调用或向量缓存数据。',
      purposes: {
        unspecified: '未标注用途',
        knowledge_qa: '知识库问答',
        query_rewrite: '查询改写',
  ...
  evaluationSettings: {
    ...
    total: '共 {count} 次', pageStatus: '第 {current} / {total} 页', empty: '暂无评测运行。', selectCompare: '选择比较',
    exportEvidence: '导出证据', exportSucceeded: '证据报告已下载。', exportFailed: '证据报告导出失败。',
    repeatSummary: '重复实验汇总', repeatSummaryHint: '仅聚合代码、数据、配置、知识库、模型指纹和调用工作负载完全一致的成功运行。',
    repeatCount: '{count} 次匹配运行', medianRecall: 'Recall 中位数', medianDuration: '耗时中位数', p95Duration: '耗时 P95',
    medianTokens: 'Token 中位数', medianCacheHitRate: '缓存命中率中位数', medianCost: '成本中位数', latencyDistribution: '耗时中位数与 P95 对比', medianShort: '中位数',
