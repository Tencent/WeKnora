export default {
  ...
  knowledgeEditor: {
    ...
    desensitization: {
      title: '文档脱敏',
      description: '解析得到 markdown 后立刻打码，再进行切块、Embedding 和摘要。默认关闭。开启后不能使用云解析引擎。',
      coverageTitle: '无法保证全部检出',
      coverageNotice: '内置脱敏只处理已勾选的预置类型和自定义正则，不覆盖图片、扫描件和上传原件。未命中的内容会原样进入切块、向量索引和问答。请结合本页试跑与抽检，不能替代人工复核。',
      enableLabel: '启用文本脱敏',
      enableDescription: '仅处理解析出的文本，不修改上传原件，也不处理图片。',
      engineLabel: '脱敏引擎',
      engineDescription: '内置引擎使用正则与校验。选 Presidio 时仍会先跑内置规则，再把原文发给侧车补漏。',
      engineBuiltin: '内置（正则+校验）',
      enginePresidio: 'Presidio 侧车',
      presidioFlowTitle: '入库时先内置、再侧车',
      presidioFlowNotice: '1. 先用下方内置规则打码（身份证校验、银联卡等）。2. App 把解析出的 Markdown 原文 POST 到 Analyzer。3. 按返回的 start/end/entity_type 映射到已勾选的预置类型，再换成占位符。侧车未配置或中文识别器不可用时，整篇解析失败。本页试跑只跑第 1 步，不调用侧车。',
      presidioApiLabel: '侧车接口',
      presidioApiUrlHint: '地址由部署环境变量 PRESIDIO_ANALYZER_URL 提供，不在知识库里填写。',
      presidioRequestLabel: '请求示例',
      presidioResponseLabel: '响应示例',
      engineLLM: '本地大模型',
      llmModelLabel: '大模型',
      llmModelDescription: '留空时使用知识库摘要模型。仅在引擎选择「本地大模型」时调用。',
      llmModelPlaceholder: '请选择模型',
      maskStyleLabel: '打码样式',
      maskStyleDescription: '占位符替换如 <身份证>，局部遮罩如 138****8000。',
      maskReplace: '占位符',
      maskPartial: '局部遮罩',
      entityTypesLabel: '预置类型',
      entityTypesDescription: '针对中国大陆场景，均带格式或校验，可多选。勾选后下方显示对应正则，预置规则只读。',
      entities: {
        cn_id_card: '居民身份证',
        cn_mobile: '大陆手机号',
        cn_landline: '固定电话',
        ...
        cn_uscc: '统一社会信用代码',
        cn_plate: '民用车牌',
        cn_plate_std: '民用车牌（普通）',
        cn_plate_nev: '民用车牌（新能源）',
        email: '电子邮箱'
      },
      presetReadonly: '预置',
      customRulesLabel: '自定义正则',
      customRulesDescription: '在预置类型之后应用。替换模板可为空，默认使用规则名称。',
      customRulesTitle: '请充分验证后再保存',
      customRulesNotice: '规则过宽会造成误脱敏（例如订单号、流水号被打码），过窄会漏报。请用本页试跑和真实业务样例，同时检查应命中与不应命中的文本。',
      ruleName: '名称',
      rulePattern: '正则',
      ruleReplacement: '替换模板（可选）',
      addRule: '添加规则',
      previewLabel: '试跑',
      previewDescription: '用当前预置类型和自定义规则对样例文本打码。试跑只覆盖内置引擎，不能代表 Presidio 侧车入库。',
      previewBuiltinOnlyTitle: '试跑仅为内置引擎',
      previewBuiltinOnlyNotice: '选 Presidio 时本页不调用侧车。入库行为以 Presidio 配置为准，请用真实文档验证。',
      previewPlaceholder: '粘贴一段含手机号或身份证的文本…',
      previewRun: '预览结果',
      previewEmpty: '请先输入试跑文本',
      previewFailed: '脱敏预览失败',
      cloudParserForbidden: '开启脱敏后不能使用云解析引擎，请改用 anydoc、simple 或 builtin'
    },
    asr: {
      title: '音频语音识别',
