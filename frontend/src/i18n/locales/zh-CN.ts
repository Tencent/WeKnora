export default {
  ...
  agent: {
    ...
    searchResults: {
      none: "未找到匹配的内容",
      summary: "找到 {count} 个结果",
      kbSummary: "找到 {count} 个结果，来自 {kbCount} 个文件",
    },
    webSearchResults: {
      summary: "找到 {count} 个网络搜索结果",
    },
    grepResults: {
      none: "未找到匹配的内容",
      summary: "找到 {count} 处匹配",
      shown: "（显示 {count} 个）",
    },
    manual: {
      emptyQuestion: "（无提问内容）",
    ...
    answerMessages: {
      emptyAnswerCopy: "当前回答为空，无法复制",
      copiedToClipboard: "已复制到剪贴板",
      copyFailed: "复制失败，请手动复制",
      emptyAnswerSave: "当前回答为空，无法保存到知识库",
      openedEditor: "已打开编辑器，请选择知识库后保存",
    },
    title: "智能体",
    ...
    editor: {
      ...
      builtinAgentNotice: "这是内置智能体，名称和描述不可修改，但可以调整配置参数",
      modelConfig: "模型配置",
      ...
      placeholderHintWithClick: "点击插入，或输入 {'{{'} 唤起列表",
      placeholderInsertHint: "（点击插入）",
      temperature: "温度",
      temperatureDesc: "控制输出的随机性，0 最确定，1 最随机",
      thinking: "思考模式",
      thinkingDesc: "启用模型的扩展思考能力（需要模型支持）",
      welcomeMessage: "欢迎消息",
      ...
      webSearchDesc: "启用后智能体可以搜索互联网获取信息",
      webSearchMaxResultsDesc: "每次搜索返回的最大结果数量",
      knowledgeBases: "关联知识库",
      knowledgeBasesDesc: "选择智能体可访问的知识库范围",
      ...
      selectKnowledgeBasesDesc: "选择要关联的知识库",
      ...
      maxIterationsDesc: "Agent 执行任务时的最大推理步骤数",
      allowedTools: "允许的工具",
      allowedToolsDesc: "选择 Agent 可以使用的工具",
      toolDisabledHint: "（需要配置知识库）",
      mcpServiceLabel: "MCP 服务",
      mcpServiceDesc: "选择 Agent 可以调用的 MCP 服务",
      mcpServiceAll: "全部",
      mcpServiceSelected: "指定",
      mcpServiceNone: "禁用",
      mcpServiceSelectLabel: "选择 MCP 服务",
      mcpServiceSelectDesc: "选择要启用的 MCP 服务",
      mcpServiceSelectPlaceholder: "选择 MCP 服务",
      supportedFileTypesLabel: "支持的文件类型",
      supportedFileTypesDesc: "限制可选择的文件类型，留空表示支持所有类型",
      supportedFileTypesPlaceholder: "全部类型",
      faqStrategyTitle: "FAQ 优先策略",
      faqStrategyTooltip: "当知识库中包含 FAQ（问答对）时，可以启用此策略让 FAQ 答案优先于普通文档",
      faqPriorityEnabledLabel: "启用 FAQ 优先",
      faqPriorityEnabledDesc: "FAQ 答案将优先于普通文档被引用，提高回答准确性",
      faqDirectAnswerThresholdLabel: "直接回答阈值",
      faqDirectAnswerThresholdDesc: "当问题与 FAQ 相似度超过此值时，直接使用 FAQ 答案",
      faqScoreBoostLabel: "FAQ 分数加权",
      faqScoreBoostDesc: "FAQ 结果的相关性分数乘以此系数，使其排序更靠前",
      multiTurn: "多轮对话",
      historyTurns: "保留轮数",
      conversationSettingsDesc: "配置多轮对话和问题改写相关参数",
      multiTurnDesc: "开启后将保留历史对话上下文",
      historyTurnsDesc: "保留最近几轮对话作为上下文",
      enableRewriteDesc: "多轮对话时自动改写用户问题，消解指代和补全省略",
      rewritePromptSystemDesc: "用于问题改写的系统提示词（留空使用默认）",
      rewritePromptUserDesc: "用于问题改写的用户提示词模板（留空使用默认）",
      // 检索策略
      retrievalStrategy: "检索策略",
      retrievalStrategyDesc: "配置知识库检索和排序的参数",
      embeddingTopK: "向量召回数量",
      embeddingTopKDesc: "向量检索返回的最大结果数量",
      keywordThreshold: "关键词阈值",
      keywordThresholdDesc: "关键词检索的最低相关性分数",
      vectorThreshold: "向量阈值",
      vectorThresholdDesc: "向量检索的最低相似度分数",
      rerankTopK: "重排数量",
      rerankTopKDesc: "重排序后保留的最大结果数量",
      rerankThreshold: "重排阈值",
      rerankThresholdDesc: "重排序的最低相关性分数",
      // 多轮对话
      ...
      contextTemplateDesc: "定义如何将检索到的内容格式化后传递给模型",
      ...
      maxCompletionTokensDesc: "模型生成回复的最大 Token 数量",
      fallbackStrategy: "兜底策略",
      fallbackStrategyDesc: "当无法从知识库找到相关内容时的处理方式",
      fallbackStrategyFixed: "固定回复",
      fallbackStrategyModel: "模型生成",
      fallbackResponse: "固定回复内容",
      fallbackResponseDesc: "当无法回答时返回的固定文本",
      fallbackResponsePlaceholder: "抱歉，我无法回答这个问题。",
      fallbackPrompt: "兜底提示词",
      fallbackPromptDesc: "当无法从知识库找到答案时，引导模型生成回复的提示词",
      fallbackPromptPlaceholder: "留空使用系统默认提示词",
      tools: {
        thinking: { label: "思考", desc: "动态和反思性的问题解决思考工具" },
        todoWrite: { label: "制定计划", desc: "创建结构化的研究计划" },
        grepChunks: { label: "关键词搜索", desc: "快速定位包含特定关键词的文档和分块" },
        knowledgeSearch: { label: "语义搜索", desc: "理解问题并查找语义相关内容" },
        listKnowledgeChunks: { label: "查看文档分块", desc: "获取文档完整分块内容" },
        queryKnowledgeGraph: { label: "查询知识图谱", desc: "从知识图谱中查询关系" },
        getDocumentInfo: { label: "获取文档信息", desc: "查看文档元数据" },
      ...
      fileTypes: {
        ...
        docx: { label: "Word", desc: "Word 文档 (.docx/.doc)" },
        txt: { label: "文本", desc: "纯文本文件 (.txt)" },
        md: { label: "Markdown", desc: "Markdown 文档" },
        csv: { label: "CSV", desc: "逗号分隔值文件" },
        xlsx: { label: "Excel", desc: "Excel 表格 (.xlsx/.xls)" },
        jpg: { label: "图片", desc: "图片文件 (.jpg/.jpeg/.png)" },
      },
    },
    selector: {
      title: "选择智能体",
  ...
  model: {
    ...
    customModel: "自定义模型",
    builtinModel: "内置模型",
    builtinDescription: "内置模型对所有租户可见，敏感信息会被隐藏，且不可编辑或删除。",
    builtinGuide: "查看内置模型管理指南",
