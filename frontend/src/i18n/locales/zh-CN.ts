export default {
    ...
    globalSettings: {
      ...
      keyLabels: {
        ...
        model: {
          ...
          max_rpm: "模型默认每分钟请求数上限",
          max_tpm: "模型默认每分钟 token 数上限",
        },
      },
      keyDescriptions: {
        auth: {
        ...
        model: {
          ...
          max_rpm:
            "后台任务对单个模型的默认每分钟请求数（RPM）上限，按模型 ID 全副本共享，用于对齐" +
            "模型服务商的 RPM 配额。修改后立即生效、无需重启。0 或负数表示关闭默认限制" +
            "（各模型仍会尊重自身配置的上限）。仅影响后台任务，不影响交互式对话。",
          max_tpm:
            "后台任务对单个模型的默认每分钟 token 数（TPM）上限，按模型 ID 全副本共享，用于对齐" +
            "模型服务商的 TPM 配额（大文档富化最先触及）。修改后立即生效、无需重启。0 或负数表示关闭默认限制" +
            "（各模型仍会尊重自身配置的上限）。仅影响后台任务，不影响交互式对话。",
        },
      },
      enumLabels: {
  ...
  model: {
    ...
    editor: {
      ...
      maxRpmLabel: "后台每分钟请求数（RPM）上限",
      maxTpmLabel: "后台每分钟 token 数（TPM）上限",
      maxRatePlaceholder: "0 表示使用全局默认",
      maxRpmDesc: "限制文档入库/富化等后台任务对该模型的每分钟请求数（按模型全副本共享），用于对齐服务商 RPM 配额。0 或留空表示沿用全局默认；不影响交互式对话。",
      maxTpmDesc: "限制文档入库/富化等后台任务对该模型的每分钟 token 数（按模型全副本共享），用于对齐服务商 TPM 配额（大文档富化最先触及）。0 或留空表示沿用全局默认；不影响交互式对话。",
