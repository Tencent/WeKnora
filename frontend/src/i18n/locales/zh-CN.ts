export default {
  localBrowser: {
    ...
    copyPairing: '复制配对链接',
    copied: '已复制',
    windowHint: '任务在 Chrome 的 TreeRAG 标签组内运行。借用已有标签页需确认。',
    preview: '本机浏览器任务预览',
    waiting: '等待任务页面',
  ...
  tenantInvitation: {
    ...
    confirmInviteBody: '将向 {email} 发出加入邀请（角色：{role}，组织：{orgUnit}）。TA 在站内接受后才会成为成员。',
    confirmSend: '发送',
    orgUnitLabel: '所属组织',
  ...
  tenantMember: {
    ...
    errors: {
      ...
      generic: '操作失败，请稍后重试。',
      outsideManageScope: '只能管理同级非管理员或下级成员。',
    },
    roleChange: {
      success: '角色已更新',
    },
  ...
  integrations: {
    ...
    claw: {
      title: 'TreeRAG Skill',
      subtitle: '通过 TreeRAG REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 TreeRAG Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 TreeRAG 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 TreeRAG 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。',
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。',
    ...
    api: {
      ...
      capabilityManageModelsHint: '允许管理模型配置、模型凭据、模型连通性测试和 TreeRAGCloud 凭据。',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 TreeRAG 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 TreeRAG 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  agentEditor: {
    ...
    im: {
      ...
      sectionStatus: '运行状态',
      wechatOaBindSuccess: '公众号绑定成功',
      wechatOaBinding: '授权中...',
      wechatOaHint: '需配置 TreeRAG Cloud 凭证与公网回调地址（APP_EXTERNAL_URL 或 WECHAT_OA_CALLBACK_BASE_URL）；绑定后粉丝消息由智能体回复',
      wechatOaNeedCallback: '请先配置公网回调地址（APP_EXTERNAL_URL 或 WECHAT_OA_CALLBACK_BASE_URL，不可为 localhost）',
      wechatOaNeedCredentials: '请先在设置页配置 TreeRAG Cloud APPID / APPSECRET',
      wechatOaQRExpired: '授权二维码已过期，请重新获取',
      wechatOaRebind: '重新授权',
      wechatOaScanBind: '扫码绑定公众号',
      wechatOaScanning: '请使用公众号管理员微信扫码授权',
    },
    embed: {
      title: '网页嵌入',
    ...
    logs: {
      ...
      colUser: '使用者',
      dashboard: '数据看板',
      deleteMessageBody: '确认删除该条消息？删除后将无法恢复。',
      ...
      emptyCute: '还没有日志噢~',
      export: '导出',
      exportEmpty: '没有可导出的日志',
      feedbackEmpty: '暂无用户反馈',
      recordCount: '{count} 条记录',
      searchPlaceholder: '搜索标题/会话ID',
      source: '来源',
      sourceAll: '全部',
      sourceGuest: '免登录窗口',
      sourceIm: 'IM',
    ...
    publish: {
      ...
      apiCurlTitle: '调用示例',
      apiDocBack: '返回',
      apiDocSubtitle: '使用发布渠道 API Key 调用 OpenAI 兼容对话接口',
      apiDocTitle: '发布渠道 API 访问说明',
      apiEmpty: '暂无 API 密钥',
      apiHint: '这些 key 归属当前空间；调用时可通过 agent_id={agentId} 指定本智能体',
      apiKeysTitle: 'API 密钥管理',
      apiPlaintextTitle: '请妥善保存密钥',
      apiPlaintextWarn: '密钥仅展示一次，关闭后将无法再次查看，请立即复制保存',
      apiRevoke: '吊销',
      apiRevokeConfirm: '确认吊销密钥「{name}」？吊销后立即失效且不可恢复。',
      apiRevokeFailed: '吊销密钥失败',
      apiRevokeSuccess: '密钥已吊销',
      apiRoot: 'API 根地址',
      ...
      embedEmpty: '暂无网页嵌入渠道',
      imEmpty: '暂无该平台的机器人渠道',
      portalDesc: '门户即主站登录后的对话入口；组织内共享可在应用配置中管理',
      startUse: '开始使用',
      types: {
        api: 'API 访问',
        apiDesc: '通过 API 接入已有系统',
        dingtalk: '钉钉机器人',
        dingtalkDesc: '通过 API 直接接入钉钉机器人',
        embed: '网页嵌入',
        embedDesc: '以 iframe 或挂件嵌入已有网站',
        feishu: '飞书机器人',
        feishuDesc: '通过 API 直接接入飞书机器人',
        guest: '免登录窗口',
        guestDesc: '短链直开，无需登录，无需嵌入外部网站',
        portal: '门户',
        portalDesc: '用户登录门户后可直接与应用对话',
        wechat: '微信公众号接入',
        wechatDesc: '通过 API 直接接入微信公众号',
        wechat_oa: '微信公众号接入',
        wechat_oaDesc: '管理员扫码授权，接管公众号粉丝聊天',
        wecom: '企业微信',
        wecomDesc: '接入企业微信智能机器人或自建应用',
      },
      viewDocs: '查看文档',
    },
    workspace: {
      ...
      debugClear: '清空会话',
      debugEmpty: '输入问题开始调试',
      debugHint: '使用已保存的配置进行试对话；修改后请先保存再调试。',
      debugNeedSave: '保存智能体后可在此调试预览',
      debugPlaceholder: '输入测试问题…',
      debugSend: '发送',
      debugTitle: '调试预览',
      logsLoadFailed: '加载对话日志失败',
  ...
  mcpServiceDialog: {
    ...
    shareWithDescendantsTip: '勾选后，下级机构可只读引用本 MCP 服务；默认不共享。',
    authConfig: '认证配置',
    authType: '认证方式',
  ...
  knowledgeEditor: {
    ...
    basic: {
      ...
      shareWithDescendantsLabel: '是否共享给下级机构',
      shareWithDescendantsTip: '勾选后，下级机构可只读引用本知识库；默认不共享。',
    },
    errors: {
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，TreeRAG 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  auth: {
    ...
    firstTime: '首次使用 TreeRAG？',
    registerSuccess: '注册成功，请登录',
    registerFailed: '注册失败',
    subtitle: 'RAG 问答、ReAct 智能体与 Wiki 知识库，大模型驱动的企业级知识框架',
    registerSubtitle: '创建账户并开始使用 TreeRAG',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是 TreeRAG 的系统或部署配置。',
    helpAria: '沙箱密钥说明',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用 TreeRAG 所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: 'TreeRAG 标准模板',
      weknoraDockerImage: 'TreeRAG 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: 'TreeRAG 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 TreeRAG 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有 TreeRAG 标准模板时点「创建」；改 DNS 或镜像后点「删除并重建」。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建 TreeRAG 标准模板，请稍后刷新查看状态',
      ...
      templateStepHint: '进入此步只会列出集群已有模板，不会自动创建。没有标准模板时点「创建 TreeRAG 标准模板」；改 DNS 或镜像后点「删除并重建」。模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 TreeRAG 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: 'TreeRAG 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 TreeRAG 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      ...
      cubeDnsServersHelp: '可选。写入 TreeRAG 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'TreeRAG 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 TreeRAG 回收，会话继续时重建。留空按 1800 秒。',
    ...
    parser: {
      ...
      checking: '检测中…',
      paddleocrVlApiKeyHint: '若服务开启鉴权，将以 Authorization: Bearer <key> 发送；官方 PaddleX 默认无需填写',
      paddleocrVlApiKeyPlaceholder: '自建站点 API Key（可选）',
    },
    weknoraCloud: {
      title: 'TreeRAG Cloud',
      description: '配置 TreeRAG Cloud 的 APPID 和 APPSECRET 凭证。凭证用于模型服务和文档解析引擎。',
      viewDocs: '查看文档',
      unconfigured: '尚未配置凭证，请填写 APPID 和 APPSECRET',
      configured: '凭证已配置，状态正常',
      expired: 'TreeRAG Cloud 凭证已失效',
      expiredDefault: '服务重启后加密密钥已变更，已保存的凭证无法解密。请重新填写凭证。',
      reconfigure: '重新配置',
      appIdLabel: 'APPID',
      appIdDesc: 'TreeRAG Cloud 的应用 ID',
      appIdPlaceholder: '请输入 APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'TreeRAG Cloud 的应用密钥',
      appSecretPlaceholder: '请输入 APPSECRET',
      saveHint: '保存后将验证服务可达性并加密存储凭证',
      saveBtn: '保存凭证',
      usageTitle: '使用说明',
      usageSteps: '1. 填写并保存 APPID / APPSECRET\n2. 在下方「云模型接入」中按行添加 chat、embedding、rerank、vlm\n3. 文档解析：知识库设置 → 解析引擎，选择 TreeRAG Cloud 引擎',
      fillRequired: '请填写 APPID 和 APPSECRET',
      saveSuccess: '凭证保存成功',
      saveFailed: '凭证保存失败',
      credentialConfigured: 'TreeRAGCloud 凭证已配置',
      credentialExpired: '凭证已失效，请重新配置。',
      credentialUnconfigured: '尚未配置 TreeRAGCloud 凭证，请先填写 APPID 和 APPSECRET。',
      checkingStatus: '正在检查凭证状态...',
      goToSettings: '前往设置中配置',
      modelHintConfigured: 'TreeRAGCloud 凭证已配置。支持的模型可参考',
      ...
      addModelsDisplayName: {
        chat: 'TreeRAGCloud 对话',
        embedding: 'TreeRAGCloud Embedding',
        rerank: 'TreeRAGCloud ReRank',
        vllm: 'TreeRAGCloud 视觉',
      },
      modelsSection: {
        title: '云模型接入',
        descReady: '将 TreeRAG Cloud 提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。',
  ...
  knowledgeBase: {
    ...
    folderTree: {
      title: '目录',
      rootRow: '根目录',
      rootRowTip: '知识库全部文档；点击左侧目录可进入子文件夹',
    ...
    loadDocumentsFailed: '文档列表加载失败，请刷新重试',
  ...
  orgUnit: {
    sectionDescription: '平台组织树：根组织由系统管理员创建；成员登录后自动进入「组织名的空间」，本组织与下级组织用户共享该空间。',
    sectionDescriptionScoped: '从您所属的组织节点起管理下级：可在本级下新增、调整下级组织，不能改动上级或平级。',
  },
  batchManage: {
    selectAll: '全选',
    cancel: '取消',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，TreeRAG 会自动解析并建立索引。点击这里进入知识库。',
      },
      welcome: {
        title: '欢迎使用 TreeRAG',
  ...
  guestLinkPublish: {
    ...
    defaultLocale: '默认语言',
    defaultName: '免登录窗口',
    deleteConfirm: '确定删除该免登录窗口链接？访客将无法再通过该链接访问，需要重新创建。',
    deleteFailed: '删除失败',
    deleted: '已删除',
    disabled: '已停用',
    empty: '暂无免登录窗口链接',
    ...
    rateLimitDayLabel: '每日请求总上限',
    rateLimitLabel: '每分钟请求上限',
    rateLimitUnlimited: '不限制',
    rateLimitUnlimitedDesc: '关闭后可设置每分钟与每日请求上限；开启后访客请求不受配额限制。',
    saveFailed: '保存失败',
    ...
    sectionLimits: '使用限制',
    settings: '设置',
    settingsTitle: '免登录窗口设置',
    showSuggestedQuestions: '推荐问题',
    title: '标题',
    titleHint: '用于管理端卡片与访客页面标题；留空则使用智能体名称',
    titlePlaceholder: '默认使用智能体名称',
    updated: '已保存',
    welcomeMessage: '欢迎语',
    welcomePlaceholder: '你好，有什么可以帮您？',
  },
