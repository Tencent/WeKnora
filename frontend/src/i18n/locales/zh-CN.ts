export default {
  ...
  datasource: {
    ...
    noResourcesDesc_nutstore: "请确认根目录路径正确，且该目录下有文件",
    retryLoadResources: "重新加载",
    guideStep1: "在飞书中创建一个群聊，在群设置「群机器人」中添加你的应用",
    guideStep1_nutstore: "在坚果云「设置 → 安全选项 → 第三方应用管理」中生成应用密码",
    guideStep2: "打开知识库「设置」→「成员设置」→ 添加成员，搜索该群聊名称并添加",
    guideStep2_nutstore: "填写正确的账号（邮箱）和应用密码，点击「测试连接」",
    ...
    guideStep3_nutstore: "确认根目录路径存在且拼写正确，然后点重新加载",
    ...
    connectorDesc: {
      ...
      yuque: "同步语雀知识库中的文档",
      nutstore: "通过 WebDAV 同步坚果云中的文档和文件",
    },
    field: {
      appId: "App ID",
