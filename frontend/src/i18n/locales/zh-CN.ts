export default {
  ...
  knowledgeBase: {
    ...
    urlExists: '该URL已存在',
    urlLabel: '链接',
    urlPlaceholder: '每行一个链接，例如：\nhttps://example.com/article\nhttps://www.youtube.com/watch?v=VIDEO_ID\nhttps://www.youtube.com/playlist?list=PLAYLIST_ID',
    urlTip: '支持粘贴网页或 YouTube 链接，每行一个。网页会被抓取解析；每个 YouTube 视频会成为一篇独立文档，优先使用字幕，无字幕时通过语音识别转写；每个 YouTube 播放列表会导入到以列表名称命名的文件夹中。带有 &list= 的视频链接只导入该视频，导入整个列表请使用播放列表链接',
    urlPreviewWebPages: '{count} 个网页',
    urlPreviewYouTubeVideos: '{count} 个 YouTube 视频',
    urlPreviewYouTubePlaylists: '{count} 个 YouTube 播放列表，各自导入独立文件夹',
    urlPreviewInvalid: '{count} 行无效链接，将被跳过',
