export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'GRIDORA Skill',
      subtitle: '通过 GRIDORA REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      ecosystemNote: 'Skill 托管于 ClawHub（{\'@\'}lyingbug/gridora），完整 API 说明与版本历史请参见 ClawHub 页面。',
      installCta: '前往 ClawHub',
      installCtaHint: '安装 GRIDORA Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 GRIDORA 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 GRIDORA 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      capabilityManageModelsHint: '允许管理模型配置、模型凭据、模型连通性测试和 GRIDORACloud 凭据。',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 GRIDORA 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=gridora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 GRIDORA 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
      ...
      signedRequestStep0: '# 0. 在你的后端签发 JWT（HS256，aud=gridora，sub=用户 ID，tenant_id={tenantId}，exp 不超过 24 小时）',
  ...
  platform: {
    subtitle: 'One Grid. Many Agents.',
    description: '知识问答、文档分析和专业智能体，从一个受控入口开始。',
    rag: '回答有依据',
    agent: '关键结论由人确认',
    wiki: '知识与智能体统一管理',
    hybridSearch: '私有化安全保障',
  ...
  system: {
    ...
    retry: '重试',
    versionLabel: '应用版本',
    versionDescription: '当前应用服务（gridora-app）的版本号',
    frontendVersionLabel: 'UI 版本',
    frontendVersionDescription: '当前 UI 界面（gridora-ui）的构建版本号',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，GRIDORA 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
    webhookSecret: 'Webhook 签名密钥',
    webhookSecretPlaceholder: '用于 HMAC-SHA256 签名（X-GRIDORA-Signature）',
  ...
  auth: {
    ...
    firstTime: '首次使用 GRIDORA？',
    registerSuccess: '注册成功，请登录',
    registerFailed: '注册失败',
    subtitle: 'RAG 问答、ReAct 智能体与 Wiki 知识库，大模型驱动的企业级知识框架',
    registerSubtitle: '创建账户并开始使用 GRIDORA',
  ...
  settings: {
    ...
    sandbox: {
      ...
      gridoraTemplateTitle: 'GRIDORA 标准模板',
      gridoraDockerImage: 'GRIDORA 标准镜像',
      gridoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      gridoraTemplateOverview: 'GRIDORA 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      gridoraTemplateDescription: '包含 GRIDORA 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有 GRIDORA 标准模板时点「创建」；改 DNS 或镜像后点「删除并重建」。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建 GRIDORA 标准模板，请稍后刷新查看状态',
      ...
      templateStepHint: '进入此步只会列出集群已有模板，不会自动创建。没有标准模板时点「创建 GRIDORA 标准模板」；改 DNS 或镜像后点「删除并重建」。模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 GRIDORA 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      gridoraStandardTemplate: 'GRIDORA 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 GRIDORA 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      ...
      cubeDnsServersHelp: '可选。写入 GRIDORA 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'GRIDORA 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 GRIDORA 回收，会话继续时重建。留空按 1800 秒。',
    ...
    storage: {
      ...
      configurable: '可配置',
      pathPrefix: '路径前缀（可选）',
      pathPrefixPlaceholder: '如 gridora/images',
      prefixPlaceholder: '如 gridora',
    ...
    gridoraCloud: {
      ...
      addModelsDisplayName: {
        chat: 'GRIDORACloud 对话',
        embedding: 'GRIDORACloud Embedding',
        rerank: 'GRIDORACloud ReRank',
        vllm: 'GRIDORACloud 视觉'
      },
      modelsSection: {
        title: '云模型接入',
        descReady: '将 GRIDORA Cloud 提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，GRIDORA 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 GRIDORA',
