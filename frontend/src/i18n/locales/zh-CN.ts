export default {
  ...
  integrations: {
    ...
    api: {
      ...
      editApiKeyScopeDesc: "修改 API Key「{name}」的名称、授权范围和用户身份配置。",
      ...
      keyIdentityDesc: "仅对这个 API Key 生效，用于隔离对话 Session 和 MCP OAuth 授权；操作权限仍由权限范围决定。",
      keySignedHint: "后端签发短期 HS256 JWT，包含 sub、tenant_id、aud=weknora、exp 和本 Key 的 identity_namespace。保存后可查看完整签发示例。",
      keySecretDraftHint: "请先复制密钥到业务后端配置；点击创建或保存后才会生效，并替换此 Key 原来的签名密钥。",
      keyModeChangeHint: "切换身份模式后，原模式的会话和 MCP 授权不会自动沿用。",
      keyExampleDesc: "选择 API Key 查看对应的身份配置、请求示例并运行 Playground。",
      identityNamespace: "应用身份域",
      identityApplication: "应用身份",
      identityApplicationHint: "新应用默认独立隔离。为同一应用轮换 Key 时，可选择已有 Key，沿用其用户身份、会话和 MCP 授权。",
      identityNewApplication: "新应用（独立隔离）",
      identityReuseHint: "复制所选 Key 的身份模式和签名配置；操作权限按当前表单设置。创建后可单独编辑。",
      legacyIdentityHint: "此 Key 保留升级前的身份关联；相同外部用户 ID 可能与其他旧 Key 共享会话和 MCP 授权。",
      ...
      requireDirectHeaderDesc: "开启后，缺少用户 ID 的请求将被拒绝；关闭后，这些请求按应用统一处理，不区分终端用户。旧 Key 保留历史空间级回退。",
      ...
      playgroundTenantModeHint: "此模式不发送外部用户身份。同一应用身份下的请求共用会话归属和 MCP 授权。",
