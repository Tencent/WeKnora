export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      networkHint: '控制该配置下所有沙箱的出入网。改动只影响之后新建的沙箱，已有沙箱按原策略运行到回收。',
      inboundAccess: '入站访问',
      inboundAccessHelp: '默认要求凭证：沙箱公网 URL 必须携带创建时签发的 token。',
      inboundTokenRequired: '要求凭证（默认）',
      inboundPublic: '公网可访问',
      egressDefault: '出站默认',
      ...
      cubeL7RulesHelp: '每条规则必须填 host 或 sni，网络层只从这两个字段提取放行目标。字段之间是 AND，method 列表内部是 OR。仅 HTTP 80 / HTTPS 443 生效。',
