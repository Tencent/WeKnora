export default {
  ...
  agent: {
    ...
    editor: {
      ...
      promptsConfig: "指令与行为",
      promptsConfigDesc: "配置业务指令和结构化行为；检索、工具与输出协议由系统托管",
      modelConfig: "模型配置",
      ...
      webSearchConfig: "网络搜索",
      webSearchConfigDesc: "配置智能体可使用的网络信息源及快速问答检索策略",
      webSearchModeOff: "关闭",
      webSearchModeOnDemand: "按需补充（推荐）",
      webSearchModeAlways: "始终联合检索",
      configuration: "配置项",
      agentId: "智能体 ID",
  ...
  agentEditor: {
    ...
    promptProtocol: {
      managedNotice: "当前使用系统托管 Prompt 协议 v{version}。升级会自动应用，检索路由、工具调用、安全、引用和输出格式不再由智能体自定义 Prompt 覆盖。",
      userInstructions: "用户指令",
      userInstructionsDesc: "用于定义角色、语气、目标用户、回答偏好和业务规则。它不能覆盖系统托管协议。",
      userInstructionsPlaceholder: "例如：你是面向新员工的产品助手；回答先给结论，再给操作步骤；术语首次出现时补充简短解释。",
      managedFallback: "模型兜底的判断与生成协议由系统托管，会随协议版本统一升级。",
    },
    selection: {
      all: "全部",
      selected: "指定",
