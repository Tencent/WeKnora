export default {
  ...
  modelSettings: {
    ...
    analytics: {
      ...
      callsUnit: '次调用',
      compositionUnavailable: '调用构成与总数不一致，暂无法展示。',
      inputDetail: '输入 {value}',
      outputDetail: '输出 {value}',
      observedCoverage: '观测 {observed} / {applicable} 次',
      notObserved: '暂无观测值',
      usageTrend: '用量趋势',
      trendDescription: '按创建时间汇总的服务端模型用量',
      tokens: 'Token',
      latency: '延迟',
      noMetricData: '该指标暂无观测数据。',
      promptCache: 'Prompt 缓存',
      providerCache: '模型提供商 Prompt 缓存',
      callHitRate: '调用命中率',
      tokenCacheRatio: 'Token 缓存比例',
      promptCacheDetail: '命中 {hit} · 未命中 {miss} · 可判定 {eligible}',
      embeddingCache: 'Embedding 缓存',
      weknoraCache: 'WeKnora Embedding 缓存',
      inputHitRate: '输入命中率',
      embeddingCacheDetail: '命中 {hit} · 未命中 {miss} · 可判定输入 {eligible}',
      loadFailed: '模型用量分析加载失败',
      retry: '重试',
      empty: '该时间范围内暂无模型用量数据。',
      modelsLoadFailed: '模型筛选暂不可用，仍可查看全部模型的用量分析。'
    },
    copySuffix: ' 副本',
    builtinTag: '内置',
