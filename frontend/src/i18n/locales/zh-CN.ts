  ...
  knowledgeEditor: {
    ...
    advanced: {
      ...
      multimodal: {
        ...
        imagePostProcessLabel: '图片后处理',
        imagePostProcessDescription: '开启后采用两轮管线：先按批「分类＋描述」，再按下方类别策略决定是否 OCR 并自动屏蔽所选类别；关闭则沿用基础模式：所有图片逐张描述并全部 OCR',
        imageBatchSizeLabel: '识别批处理数量',
        imageBatchSizeDescription: '多少张图片合成一次识别请求，省请求数不省 token，按请求计费的 API 受益；1 为逐张识别，最大 16',
        imageDownscaleLabel: '识别前降分辨率',
        imageDownscaleDescription: '识别前把图片压到长边 640，token 消耗大幅降低；模型支持高清时可关闭',
        imageClassPoliciesLabel: '类别策略',
        imageClassPoliciesDescription: '按图片类别设置后处理动作：「屏蔽」＝该类图片的引用从正文摘除、不再随正文送入模型，其描述与 OCR 文本仍会生成并保留检索；「OCR」＝对图内文字再跑一轮识别',
        policyDisabledCol: '屏蔽',
        policyOcrCol: 'OCR',
        imageClassChart: '图表',
        imageClassDecorative: '装饰性图片',
        imageClassLogo: 'Logo 标志',
        imageClassPhoto: '照片',
        imageClassTableImage: '文字表格',
        imageClassTextScreenshot: '文字截图',
        imageClassOther: '其他',
