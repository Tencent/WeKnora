export default {
  localBrowser: {
    ...
    "invalidArguments": "浏览器工具参数不完整，智能体需要修正后再执行。",
    "commandInterrupted": "浏览器操作已中断，请先确认页面状态，再从小预览继续任务。",
    "actionFailedHint": "未能完成这次浏览器操作，可展开查看原因。",
    ...
    "settingsTitle": "浏览器连接",
    "settingsDescription": "连接你的 Chrome，各个对话共用，无需逐个配对。",
    ...
    "copyAgain": "再次复制",
    "connectionNote": "配对一次，同一空间的各个对话共用；服务重启后扩展会自动重连。",
    "resumeNote": "连接恢复后，中断的任务仍会暂停，请在对话预览中点击继续。",
    "privacyNote": "任务使用带标识的标签组；借用已有标签页会请求确认。页面内容和截图会返回 WeKnora。",
    "installHelp": "如何安装与使用？",
    "installSteps": "解压扩展包，在 Chrome 的扩展程序页面开启开发者模式，选择「加载已解压的扩展程序」。连接后回到对话描述任务，小预览会自动出现，点击可定位到任务标签。",
    "running": "运行中",
    "locateWindow": "定位到浏览器",
