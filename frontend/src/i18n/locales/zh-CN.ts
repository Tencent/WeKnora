export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      inventorySummary: '这份配置上还有 {count} 个沙箱没释放。',
      inventoryEmpty: '现在没有运行中的沙箱。',
      inventorySessions: '占用中的会话',
      inventorySessionsHint: '结束或删除这些会话后，沙箱会被回收。',
      inventoryAgents: '绑定的智能体',
      inventoryNoAgents: '没有智能体绑定这份配置。',
      inventoryAgentWarning: '删除这份配置后，这些智能体会在下次执行技能时报错。',
      inventoryOpenSession: '打开',
  ...
  agent: {
    ...
    editor: {
      ...
      skillsConfigDesc: '选择技能脚本运行的沙箱，并配置该沙箱中可供 Agent 使用的 Skills。可用 Skills 会随所选沙箱变化。',
      skillsSelection: 'Skills 选择',
      skillsSelectionDesc: '选择 Agent 可以使用的 Skills 范围。列表来自上方所选沙箱中已安装的技能。',
      skillsAll: '全部',
      skillsSelected: '指定',
      skillsNone: '禁用',
      selectSkills: '选择 Skills',
      selectSkillsDesc: '选择要启用的 Skills',
      noSkillsAvailable: '该沙箱暂无预装 Skills，可在技能沙箱设置中安装。',
      skillsNeedSandbox: '请先选择沙箱配置。可用 Skills 来自该沙箱已安装的技能。',
      goSandboxSettings: '去技能沙箱设置中配置',
      sandboxBackend: '沙箱配置',
      sandboxBackendDefault: '不启用沙箱',
      sandboxBackendHint: '技能脚本在所选沙箱中执行。未选择时不会执行技能脚本；所选配置被删除后，技能执行会失败。',
      sandboxBackendMissing: '配置已删除',
      sandboxNoConfigs: '当前空间还没有沙箱配置，技能脚本不会执行。',
      skillsInfoTitle: 'Skills 与沙箱如何联动？',
      skillsInfoContent: 'Skills 是预装的专业知识模块，脚本在所选沙箱中隔离执行。可用列表来自该沙箱已安装的技能；同一会话的沙箱一旦创建，后续附件、产物与销毁都会锁定在创建时那份配置上，改沙箱只影响之后新建的会话。'
    },
    messages: {
      created: '智能体创建成功',
      updated: '智能体更新成功',
