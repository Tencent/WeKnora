export default {
  ...
  datasource: {
    ...
    noResourcesDesc_dingtalk: "应用需要具备钉钉知识库访问权限，并且操作者需要能访问目标知识库",
    ...
    guideStep1_dingtalk: "确认钉钉应用已开通知识库和成员信息读取权限",
    guideStep2_dingtalk: "确认填写的操作者 User ID 可访问目标钉钉知识库",
    guideStep3_dingtalk: "回到这里点击重新加载，选择要同步的知识库空间或文件夹",
    ...
    field: {
      ...
      operatorUserId: "操作者 User ID",
      operatorUserIdHint: "用于代表该用户读取钉钉知识库；应用会自动解析其 Union ID",
      dingtalkOnlineDocFetcher: "在线文档获取方式",
      dingtalkFetcherBlocks: "结构化读取",
      dingtalkFetcherExport: "Markdown 自动导出",
      dingtalkBlocksFetcherHint: "结构化读取无需灰度权限，配置成本低；但会按钉钉 block 能力尽力转换，图片和复杂排版的保真度有限",
      dingtalkExportFetcherHint: "Markdown 自动导出会使用钉钉官方导出结果，图片和复杂内容更完整；但需要按官方指南完成 AI 助理、事件订阅和灰度权限配置",
      dingtalkMarkdownExportHint: "在线文档 Markdown 自动导出需要钉钉开通 Document.AIAssistant.Read 灰度权限；未开通时只能同步附件或走 block 读取能力",
      dingtalkOnlineDocFetcherHint: "如果想使用普通文件下载功能，请确保当前钉钉组织已经完成企业认证；如同步时报 orgAuthLevelNotEnough，请升级企业认证后重试",
    ...
    prereqStep1Brief_dingtalk: "创建或打开钉钉内部应用",
    prereqStep1Desc_dingtalk: "在钉钉开放平台进入应用，获取 Client ID / AppKey 与 Client Secret / AppSecret",
    prereqStep2Brief_dingtalk: "开通所需 API 权限",
    prereqStep2Desc_dingtalk: "开通成员信息读、知识库读、知识库节点读、企业存储文件读、钉钉文档读权限；普通文件下载需要钉钉组织完成企业认证，在线文档 Markdown 自动导出需要开通 Document.AIAssistant.Read 灰度权限并订阅 dingdoc_export_finish 事件",
    prereqStep3Brief_dingtalk: "填写操作者 User ID",
    prereqStep3Desc_dingtalk: "操作者需要能访问目标知识库，应用会用该身份列出空间并同步文档内容",
    prereqOpenConsole_dingtalk: "前往钉钉开放平台配置",
    prereqMarkdownExportGuide_dingtalk: "查看 Markdown 导出配置指南",
