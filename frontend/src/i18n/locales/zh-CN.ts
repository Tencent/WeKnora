export default {
  ...
  agentStream: {
    ...
    skillCards: {
      ...
      resolved: '已解析技能 {name}',
      notFound: '没有找到合适的技能',
      unavailable: '{sources} 暂时无法访问',
      temporaryNotice: '{name} 只是在本次对话中临时加载，并没有安装到空间：它不在技能列表里，其他对话用不了。',
      temporaryNoticeUnnamed: '这个技能只是在本次对话中临时加载，并没有安装到空间：它不在技能列表里，其他对话用不了。',
      official: '官方',
      downloads: '{count} 次下载',
      installs: '{count} 次安装',
      files: '{count} 个文件',
      ...
      retry: '重试',
      copySource: '复制来源',
      sourceCopied: '已复制技能来源',
      viewSource: '查看详情',
      adminOnly: '需要空间管理员安装，可以复制来源发给管理员',
      noSandbox: '当前对话没有可以安装技能的沙箱',
      alreadyInstalled: '当前沙箱已安装同名技能，再次安装会替换它',
      alreadyInstalling: '当前沙箱正在安装同名技能',
      sameNameInCatalog: '空间里已有同名技能，安装会用这个来源替换它',
      installing: '安装中 {percent}%',
      ready: '已安装，下一轮对话即可使用',
      readyNewSession: '已安装，新建对话后可用',
      selectInAgent: '此智能体只使用选定的技能，需要在智能体设置中勾选后才能使用',
      failed: '安装失败',
      failedWithReason: '安装失败：{reason}',
      installerModelRequired: '请先为技能安装选择一个模型',
      confirmTitle: '安装技能「{name}」？',
      confirmBody: '将从 {source} 获取这个第三方技能，并安装到当前对话使用的沙箱中。安装过程会运行技能自带的代码，装好后使用这个沙箱的所有对话都能用它。当前对话的沙箱随后会重建，/workspace 里的临时文件会丢失。请确认你信任这个来源。',
      confirmBodyNewSession: '将从 {source} 获取这个第三方技能，并安装到当前对话使用的沙箱中。安装过程会运行技能自带的代码。按这个沙箱的更新策略，之后新建的对话才能用它，已经开始的对话保持不变。请确认你信任这个来源。'
    },
    skillFiles: {
      heading: '技能文件',
      script: '脚本',
