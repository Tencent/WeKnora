export default {
  ...
  datasource: {
    ...
    field: {
      ...
      operatorId: "操作者 UnionId（可选）",
      operatorIdHint: "调用知识库接口时使用的操作者身份。留空时自动使用企业管理员的 UnionId（需要为应用开通通讯录读权限 qyapi_get_member）",
    ...
    prereqStep1Desc_dingtalk: "登录钉钉开发者后台 → 应用开发 → 创建应用，在「凭证与基础信息」中获取 Client ID（AppKey）和 Client Secret（AppSecret）",
    prereqStep2Brief_dingtalk: "为应用开通所需权限",
    prereqStep2Desc_dingtalk: "在应用「权限管理」中开通：Wiki.Workspace.Read 与 Wiki.Node.Read（知识库读取）、Storage.File.Read（文档导出下载，位于「存储」分类）、qyapi_get_member（自动解析操作者身份）",
    prereqStep3Brief_dingtalk: "（可选）指定操作者 UnionId",
    prereqStep3Desc_dingtalk: "知识库接口以操作者身份鉴权，仅同步该用户有权限访问的知识库；留空时自动使用企业管理员身份",
