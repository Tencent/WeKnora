export default {
  sandboxBrowser: {
    ...
    unavailableHint: "当前沙箱未提供浏览器技能，请选择带浏览器技能的沙箱配置。",
    reconnectHint: "浏览器连接暂时中断，可重试启动。",
    ...
    "release": "交还控制",
    "takeControl": "接管浏览器",
    "previewHint": "预览每两秒刷新。接管期间智能体会等待，关闭面板后交还控制。",
    "address": "网页地址",
    "go": "前往",
    "viewport": "沙箱浏览器预览",
    "emptyHint": "浏览器尚未启动。点击下方按钮，将启动当前会话的沙箱和浏览器。",
    "start": "启动浏览器",
    "textHint": "在网页选中输入框后，在此输入文字（支持中文）",
    "insert": "输入",
  ...
  skillDiscovery: {
    ...
    cardUnknown: "预装信息未知",
    cardNone: "暂无",
    cardLoadFailed: "安装信息读取失败",
    cardIncompatible: "另有 {count} 项版本不匹配",
    cardCollapse: "收起",
    cardExpand: "展开全部技能",
    cardDisabled: "已停用",
    cardAvailabilityUnknown: "可用性未知",
    cardNotInImage: "当前镜像不可用",

    canInstallManually: "未识别到预装，可手动安装",
    alreadyInstalled: "已安装同名技能",
    preinstalledShort: '已预装',
    preinstalledOnName: '已预装 · {name}',
    preinstalledSandboxCount: '{count} 个沙箱预装',
    alreadyPreinstalled: "已预装，可直接使用",
    checkingPreinstalled: "正在读取预装信息…",
    includedWithTemplate: "随模板提供",
    invalidManifest: "声明格式无效，请选择发布者提供的技能清单 JSON。",
    clearManifest: "移除声明",
    importManifest: "导入预装声明",
    cloudManifestHint: "云平台未返回技能清单时，可导入镜像发布者提供的声明。它会绑定当前模板版本，仅保存元数据。",
    preinstalledUnknown: "预装信息未知；不会启动沙箱或自动安装。",
    preinstalledMismatch: "部分技能与当前应用版本不匹配，暂不提供。",
    ...
    activationTitle: "选择在哪些沙箱配置中使用「{name}」",
    activate: "启用到沙箱",
    activationHint: "仅安装到你选择的沙箱。已预装的环境可直接使用，无需重复安装。",
    activationAccepted: "正在启用技能",
    localBrowser: "本机浏览器 · 需安装扩展",
    "title": "技能发现",
    "mySkills": "我的技能",
    "search": "搜索技能、用途或发布者",
    "runtimeHint": "内置技能随模板提供，可直接使用；其他沙箱可在此选择安装。",
    "retry": "重试",
    "builtins": "内置技能",
    "recommended": "精选推荐",
    "builtinHint": "办公、数据分析和浏览器技能。可使用已预装的模板，也可安装到其他沙箱。",
    "externalHint": "来自外部发布者的技能参考。仅收录链接，不包含代码，也不会安装到沙箱或自动向智能体注入指令。",
    "linkOnly": "外部链接",
    "source": "查看来源",
    "manage": "管理已有技能",
    "add": "启用技能",
    "officialGuide": "官方使用说明",
    "noResults": "没有找到匹配的技能",
    "loadFailed": "技能目录加载失败",
    "addFailed": "添加技能失败",
    "migrateTitle": "升级旧沙箱",
    "migrateHint": "新建使用新版镜像的沙箱配置，再按原版本迁移已有技能。",
    "migrate": "迁移技能",
    "migrateDetails": "请先在沙箱设置中新建使用新版镜像的配置。这里会校验原始技能包并重新安装到目标配置；保留源配置与现有会话。安装完成后，在智能体设置中选择新配置，并在新会话中验证。",
    "configureSandbox": "打开沙箱设置",
    "sourceSandbox": "源沙箱配置",
    "targetSandbox": "新沙箱配置",
    "preview": "检查迁移内容",
    "startMigration": "迁移可用技能",
    "readyToMigrate": "原始技能包校验通过",
    "noSkillsToMigrate": "源配置中没有技能",
    "migrationStarted": "已提交 {count} 个安装任务，可在“我的技能”中查看进度。新配置内的凭据和用户环境变量请单独检查。",
    ...
    "blockers": {
        "source_not_ready": "源技能尚未安装完成",
        "source_disabled": "源技能已停用，不自动启用",
        "target_name_conflict": "目标已有同名技能，将保留",
        "missing_digest": "旧记录缺少校验值，请重新导入原始包",
        "missing_original_archive": "原始技能包不可用，不会替换为最新版本"
    }
},
  platformApiKeys: {
    title: '平台 API Key',
  ...
  agent: {
    ...
    editor: {
      ...
      skillsConfig: '技能',
      skillsConfigDesc: '选择运行沙箱，再选用其中的技能。预装技能可直接使用，也可以安装目录中的其他技能。',
      skillsSelection: '技能列表',
      skillsSelectionDesc: '可用技能包括沙箱预装和自行安装的技能。',
      skillsAll: '全部',
      skillsSelected: '指定',
      skillsNone: '禁用',
      selectSkills: '选择技能',
      selectSkillsDesc: '勾选此智能体可以使用的技能，预装技能无需再次安装。',
      skillsAllListHint: '使用此沙箱中所有可用的预装技能和已安装技能。',
      skillsGroupAvailable: '可用',
      skillsGroupUnavailable: '不可用',
      noSkillsAvailable: '此沙箱暂无可用技能，空间目录中也没有其他技能。',
