export default {
  localBrowser: {
    ...
    "storeInstall": "前往 Chrome 商店",
    "manualInstall": "手动安装（备用）",
    "storeInstallHint": "在 Chrome 应用商店安装 BrowserSkill，再回到这里配对。",
  ...
  datasource: {
    ...
    guideStep1_feishu_links: '打开要同步的飞书文档，分享 → 添加协作者，搜索并添加你的应用，权限选「可阅读」',
    guideStep2_feishu_links: '复制该文档链接（知识库 copylink 或云文档 /docx/ 链接均可）',
    guideStep3_feishu_links: '粘贴到链接列表并点击「解析链接」',
    guideStep1_lark_links: '打开要同步的 Lark 文档，分享 → 添加协作者，搜索并添加你的应用，权限选 Can view',
    guideStep2_lark_links: '复制该文档链接（wiki copylink 或 /docx/ 链接均可）',
    guideStep3_lark_links: '粘贴到链接列表并点击「解析链接」',
    ...
    prereqBarText_feishu_links: '首次使用？点击查看飞书应用配置指引',
    prereqStep1Brief_feishu_links: "创建飞书自建应用",
    prereqStep1Desc_feishu_links: "登录飞书开放平台 → 创建企业自建应用",
    prereqStep2Brief_feishu_links: "添加机器人能力",
    prereqStep2Desc_feishu_links: "开放平台 → 你的应用 → 添加应用能力 → 机器人",
    prereqStep3Brief_feishu_links: "配置应用权限",
    prereqStep3Desc_feishu_links: "为应用开通 wiki:wiki:readonly, drive:drive.metadata:readonly（或 drive:drive）, drive:drive:readonly, drive:export:readonly, docx:document:readonly 权限",
    prereqBarText_lark_links: '首次使用？点击查看 Lark 应用配置指引',
    prereqStep1Brief_lark_links: "创建 Lark 自建应用",
    prereqStep1Desc_lark_links: "登录 Lark 开放平台 → 创建企业自建应用",
    prereqStep2Brief_lark_links: "添加机器人能力",
    prereqStep2Desc_lark_links: "开放平台 → 你的应用 → 添加应用能力 → 机器人",
    prereqStep3Brief_lark_links: "配置应用权限",
    prereqStep3Desc_lark_links: "为应用开通 wiki:wiki:readonly, drive:drive.metadata:readonly（或 drive:drive）, drive:drive:readonly, drive:export:readonly, docx:document:readonly 权限",
    ...
    connectorDesc: {
      ...
      feishu_links: '通过文档链接同步飞书知识库或云文档中的单篇内容',
      lark_links: '通过文档链接同步 Lark 知识库或云文档中的单篇内容（飞书国际版）',
    ...
    links: {
      title: '文档链接',
      hint: '每行一条飞书文档链接。机器人需对该篇有可阅读或可编辑权限；只同步链接指向的那一篇，不会拉取知识库目录或云盘文件夹。',
      placeholder: 'https://xxx.feishu.cn/wiki/...\nhttps://xxx.feishu.cn/docx/...',
      parse: '解析链接',
      urlsRequired: '请至少粘贴一条飞书文档链接',
      noneResolved: '没有可同步的文档，请检查链接是否正确、应用是否有阅读权限',
      deduped: '已去掉 {n} 条重复链接',
      resolvedCount: '已解析 {n} 篇文档',
      error: {
        wiki_space: '这是知识库空间链接，请改用「飞书」数据源，或粘贴某一篇的 /wiki/ 文档链接',
        drive_folder: '这是云盘文件夹链接，请改用「飞书云盘」数据源',
        unrecognized: '无法识别的链接，请粘贴飞书文档 /wiki、/docx、/docs、/sheets、/base 或 /file 链接',
        unsupported_type: '不支持该文档类型（如思维笔记、幻灯片）',
        no_permission: '应用无权阅读该文档，请在飞书中把应用加为协作者（可阅读即可）',
        not_found: '文档不存在或已删除',
        resolve_failed: '解析失败，请确认链接和应用权限',
      },
    },
    syncError: {
  ...
  settings: {
    ...
    sandbox: {
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有官方标准模板时，会从 Docker Hub 镜像自动构建。桌面模板体积更大，需在下方单独点「创建」。改 DNS 或镜像后在对应卡片上点「重建」。',
      ...
      templateStepHint: '进入此步会列出集群模板；若还没有官方标准模板，会从 Docker Hub 镜像自动构建。桌面模板（XFCE）体积较大，需单独点「创建」。改 DNS 或镜像后在对应卡片上点「重建」。选好并就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '模板正在构建，列表会自动刷新。',
      ...
      desktopTemplateProvisioning: '已开始在集群中创建 WeKnora 桌面模板，请稍后刷新查看状态',
      desktopTemplateReplaced: '已删除原桌面模板并开始重建，请等待状态就绪。标准（CLI）模板不受影响。',
      desktopTemplateTag: '桌面',
      replaceDesktopTemplateConfirm: '将用当前配置（含 DNS）重建 WeKnora 桌面模板。新模板就绪前不会删除仍可启动的旧桌面模板，也不会删除标准（CLI）模板。',
  ...
  knowledgeBase: {
    ...
    channelFeishuLinks: "飞书链接",
    channelLarkLinks: "Lark 链接",
    channelDingtalk: '钉钉',
    channelSlack: 'Slack',
    channelIm: 'IM 渠道',
