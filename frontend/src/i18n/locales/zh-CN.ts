export default {
  ...
  input: {
    addModel: '添加模型',
    placeholder: '直接向模型提问',
    placeholderWithContext: '输入问题，将基于上方选中的知识库/文件夹/文件回答',
    ...
    messages: {
      ...
      folderCapabilityUnavailable: '文件夹问答暂不可用，请等待滚动升级完成后重试，或移除已选择的文件夹。',
      folderUnsupportedInSmartReasoning: '智能推理暂不支持文件夹范围，请切换到快速问答或移除已选择的文件夹。',
  ...
  mentionDetail: {
    unifiedSearchTitle: '搜索引用范围',
    unifiedSearchHint: '同时匹配知识库、文件夹和文件',
    searchingAllTypes: '正在搜索全部类型...',
    typeKnowledgeBase: '知识库',
    typeFolder: '文件夹',
    typeFile: '文件',
    typeTag: '标签',
  ...
  uploadConfirm: {
    title: '上传文档确认',
    folderStructureHint: '将保留目录结构，并在「{destination}」下创建或复用 {count} 个文件夹',
    folderConflictTitle: '遇到同名文件夹时',
    folderConflictMerge: '合并',
    folderConflictSkip: '跳过',
    folderConflictRename: '重命名',
    folderConflictMergeHint: '复用同名文件夹，并继续创建其下缺少的目录',
    folderConflictSkipHint: '跳过同名文件夹对应的整个上传子目录',
    folderConflictRenameHint: '新建“文件夹名 (1)”等独立目录，不与原目录混合',
    emptyFoldersNotPreserved: '浏览器不会提供空文件夹，因此仅包含空目录的层级不会上传',
    duplicateStrategyHint: '重复文件将跳过，原有文档不会被覆盖；结果会保留在上传清单中',
    parseConfig: '解析配置',
    configNav: '解析配置导航',
    navParserDefault: '默认',
  ...
  knowledgeBase: {
    ...
    uploadingFolder: '正在保留目录结构并上传 {total} 个文件...',
    uploadPreparingFolders: '正在准备目录结构',
    uploadBatchRunning: '正在上传文件',
    uploadBatchComplete: '上传任务已完成',
    uploadBatchProgress: '已处理 {completed} / {total}',
    uploadBatchSuccessCount: '成功 {count}',
    uploadBatchSkippedCount: '跳过 {count}',
    uploadBatchFailedCount: '失败 {count}',
    uploadBatchCancelledCount: '取消 {count}',
    uploadCancel: '取消剩余上传',
    uploadCancelling: '正在取消...',
    uploadCancelledItem: '已取消上传',
    uploadCancelledSummary: '已停止剩余上传：成功 {success} 个，取消 {cancelled} 个',
    uploadAllSkippedSummary: '已按当前策略跳过 {count} 个文件，未上传任何内容',
    uploadBatchAlreadyRunning: '已有上传任务正在进行，请等待完成或先取消',
    ...
    folderBrowseHint: '进入文件夹查看当前层文档',
    folderDirectDocuments: '当前层 {count} 个文档',
    folderContainsSubfolders: '含子文件夹',
    folderSearchResultCount: '{count} 个匹配',
    folderSearchEmpty: '没有匹配的文件夹',
    folderLoadMore: '加载更多文件夹',
    folderNoSubfoldersHint: '你仍可以在这里直接存放文档',
    folderNoDocumentsTitle: '当前目录还没有文档',
    folderListLabel: '文件夹',
    noFolders: '暂无文件夹',
    newFolder: '新建文件夹',
    uploadHere: '上传到此处',
    ...
    folderNameTooLong: '文件夹名称不能超过 255 字节（当前 {count} 字节）',
    retry: '重试',
    loadFoldersFailed: '加载文件夹失败',
    createFolderFailed: '创建文件夹失败',
    createFolderSuccess: '文件夹已创建',
    folderUploadPrepareFailed: '无法为「{path}」创建目录：{error}',
    folderUploadConflictSkipped: '同名文件夹已存在，已按当前策略跳过该目录',
    rename: '重命名',
    renameFolder: '重命名文件夹',
    ...
    deleteFolderWithName: '删除“{name}”',
    deleteFolderImpactSummary: '共 {folders} 个文件夹 · {documents} 个文档',
    deleteFolderChooseMode: '选择删除方式',
    deleteFolderKeepDocuments: '删除目录结构，保留文档',
    deleteFolderKeepDocumentsDescription: '当前文件夹及所有子文件夹会被删除，{count} 个文档将移动到知识库根目录。',
    deleteFolderStructureLost: '原有文件夹结构将丢失。',
    deleteFolderDeleteAll: '永久删除全部',
    deleteFolderDeleteAllDescription: '永久删除全部文件夹、文档、分块和向量数据。',
    deleteFolderIrreversible: '此操作无法恢复。',
    deleteFolderDocumentsProcessing: '其中 {count} 个文档仍在解析。请等待解析完成后再保留文档，或选择永久删除全部。',
    deleteFolderEmptyTreeConfirm: '当前文件夹及其目录树中的 {count} 个文件夹将被删除。该目录树中没有文档。',
    deleteFolderConfirmAction: '删除并保留文档',
    deleteFolderDeleteAllAction: '永久删除',
    deleteFolderTaskSubmitted: '删除任务已提交，完成后列表将自动刷新',
    deleteFolderFailed: '删除失败',
    deleteFolderSuccess: '文件夹已删除',
    renameFailed: '重命名失败',
    currentLocation: '当前位置',
    documentCount: '{count} 个文档',
    folderTreeTitle: '目录导航',
    showFolderTree: '显示目录树',
    hideFolderTree: '隐藏目录树',
    folderTreeSearchPlaceholder: '搜索全部目录',
    documentScopeLabel: '文档范围',
    documentScopeCurrent: '当前目录',
    folderMentionTitle: '选择问答目录',
    folderMentionDescription: '目录范围会自动包含所有子文件夹中的文档',
    folderMentionChooseKb: '先选择一个文档知识库',
    folderMentionChooseKbHint: '进入知识库后可逐层浏览目录',
    folderMentionUseCurrent: '使用当前目录',
    folderMentionUseCurrentHint: '问答范围包含此目录及全部子目录',
    folderMentionRootHint: '请选择一个具体目录作为问答范围',
    folderMentionEmpty: '此位置没有子文件夹',
    folderMentionBackToKb: '返回知识库列表',
