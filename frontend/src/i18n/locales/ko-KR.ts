export default {
  ...
  knowledgeEditor: {
    ...
    desensitization: {
      title: '문서 비식별화',
      description: '파싱된 마크다운을 청크/임베딩/요약 전에 마스킹합니다. 기본 꺼짐. 사용 중에는 클라우드 파서를 쓸 수 없습니다.',
      coverageTitle: '전건 검출을 보장하지 않습니다',
      coverageNotice: '내장 엔진은 선택한 사전 설정과 사용자 정규식만 처리합니다. 이미지·스캔본·원본 파일은 대상이 아닙니다. 미검출 텍스트는 청크·임베딩·질의응답에 그대로 들어갑니다. 미리보기와 샘플 점검이 필요하며, 사람 검토를 대체하지 않습니다.',
      enableLabel: '텍스트 비식별화 사용',
      enableDescription: '파싱된 텍스트만 처리합니다. 원본 파일과 이미지는 수정하지 않습니다.',
      engineLabel: '엔진',
      engineDescription: '기본 엔진은 정규식과 체크섬을 사용합니다. Presidio를 선택해도 내장 규칙을 먼저 실행한 뒤 원문을 사이드카로 보냅니다.',
      engineBuiltin: '내장 (정규식 + 체크섬)',
      enginePresidio: 'Presidio 사이드카',
      presidioFlowTitle: '내장 먼저, 사이드카 다음',
      presidioFlowNotice: '1. 아래 내장 규칙으로 마스킹. 2. App이 파싱된 Markdown 원문을 Analyzer에 POST. 3. start/end/entity_type을 선택한 프리셋에 매핑해 치환. 사이드카가 없거나 중국어 인식기가 없으면 적재 실패. 이 페이지 미리보기는 1단계만 실행합니다.',
      presidioApiLabel: '사이드카 API',
      presidioApiUrlHint: 'URL은 배포 환경 변수 PRESIDIO_ANALYZER_URL이며 지식베이스에 입력하지 않습니다.',
      presidioRequestLabel: '요청 예시',
      presidioResponseLabel: '응답 예시',
      engineLLM: '로컬 LLM',
      llmModelLabel: 'LLM',
      llmModelDescription: '비워 두면 지식베이스 요약 모델을 사용합니다. 엔진이 로컬 LLM일 때만 호출됩니다.',
      llmModelPlaceholder: '모델 선택',
      maskStyleLabel: '마스킹 스타일',
      maskStyleDescription: '<身份证> 같은 자리 표시 또는 138****8000 같은 부분 마스킹.',
      maskReplace: '자리 표시',
      maskPartial: '부분 마스킹',
      entityTypesLabel: '사전 설정 유형',
      entityTypesDescription: '중국 대륙 식별 정보. 다중 선택. 선택한 유형은 아래에 읽기 전용 정규식 규칙으로 표시됩니다.',
      entities: {
        cn_id_card: '주민신분증',
        cn_mobile: '중국 휴대폰 번호',
        cn_landline: '유선전화',
        cn_bank_card: '은행 카드 (은련)',
        cn_uscc: '통일사회신용코드',
        cn_plate: '민용 번호판',
        cn_plate_std: '민용 번호판 (일반)',
        cn_plate_nev: '민용 번호판 (신에너지)',
        email: '이메일'
      },
      presetReadonly: '사전 설정',
      customRulesLabel: '사용자 정규식',
      customRulesDescription: '사전 설정 이후에 적용됩니다.',
      customRulesTitle: '저장 전에 충분히 검증하세요',
      customRulesNotice: '규칙이 너무 넓으면 주문번호·일련번호 등이 잘못 마스킹되고, 너무 좁으면 누락됩니다. 이 페이지의 미리보기와 실제 샘플로, 맞아야 할 값과 맞으면 안 되는 값을 함께 확인하세요.',
      ruleName: '이름',
      rulePattern: '정규식',
      ruleReplacement: '치환 (선택)',
      addRule: '규칙 추가',
      previewLabel: '미리보기',
      previewDescription: '현재 규칙으로 샘플 텍스트를 마스킹합니다. 미리보기는 내장 엔진만 사용하며 Presidio 적재를 대표하지 않습니다.',
      previewBuiltinOnlyTitle: '미리보기는 내장 엔진만',
      previewBuiltinOnlyNotice: 'Presidio를 선택한 경우 이 페이지는 사이드카를 호출하지 않습니다. 실제 문서로 적재를 확인하세요.',
      previewPlaceholder: '휴대폰 번호나 신분증이 포함된 텍스트를 붙여 넣으세요…',
      previewRun: '미리보기',
      previewEmpty: '먼저 텍스트를 입력하세요',
      previewFailed: '미리보기에 실패했습니다',
      cloudParserForbidden: '비식별화가 켜져 있으면 클라우드 파서를 사용할 수 없습니다. anydoc, simple 또는 builtin으로 변경하세요.'
    },
    asr: {
      title: '오디오 음성 인식',
