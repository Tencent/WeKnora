export default {
  ...
  organization: {
    ...
    settings: {
      ...
      permissionCalcFormula: '공간 권한은 공유할 때 해당 공간에 설정한 권한입니다. 내 실제 권한 = min(공간 권한, 이 공간에서의 내 역할)이며, 공유 에이전트가 가져온 지식베이스는 에이전트 설정 화면에서 삭제해야 합니다.',
      permissionCalcTip: '적용 권한은 공유 공간 권한과 내 공유 공간 역할 중 낮은 값이며, 읽기 전용 구성원은 이 지식베이스에 읽기만 가능합니다.',
      directKbSource: '직접 공유',
      agentKbSource: '{name} 에이전트가 가져옴',
      directAndAgentKbSource: '직접 공유 및 {name} 에이전트가 가져옴',
      inviteMembers: '회원 초대',
