export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      pageHintTitle: '코드 샌드박스란?',
      pageHint: '코드 샌드박스는 에이전트 스킬 스크립트가 실행되는 격리 환경입니다. 워크스페이스에 Docker, E2B, CubeSandbox 또는 로컬 프로세스 설정을 여러 개 둘 수 있고, 각 에이전트가 하나를 선택합니다. 선택하지 않으면 스크립트는 실행되지 않습니다.',
      ...
      skillUploadHint: '업로드하면 현재 이미지에 스킬을 설치하고 새 스냅샷을 만듭니다. 몇 분이 걸릴 수 있습니다. 현재 대화 턴은 중단되지 않으며, 이미 열린 세션은 다음 턴에서 새 이미지로 샌드박스를 다시 만들고 그때 세션 작업 공간 초안이 지워집니다.',
      skillUploadHintNewSession: '업로드하면 현재 이미지에 스킬을 설치하고 새 스냅샷을 만듭니다. 몇 분이 걸릴 수 있습니다. 이미 열린 세션은 세션이 끝날 때까지 기존 샌드박스를 유지하며, 이번에 설치한 스킬은 새로 시작한 세션에만 적용됩니다.',
  ...
  agent: {
    ...
    editor: {
      ...
      sandboxConfig: '코드 샌드박스',
      sandboxConfigDesc: '이 에이전트의 스킬 스크립트가 실행될 샌드박스를 선택하세요.',
      sandboxBackend: '샌드박스 설정',
      sandboxBackendDefault: '샌드박스 사용 안 함',
      sandboxBackendHint: '스킬 실행이 켜져 있을 때만 적용됩니다. 선택한 설정이 삭제되면 이 에이전트의 스킬 실행이 실패합니다.',
      sandboxBackendMissing: '설정이 삭제됨',
      sandboxNoConfigs: '이 워크스페이스에 샌드박스가 없어 스킬 스크립트가 실행되지 않습니다. 설정 - 코드 샌드박스에서 추가하세요.',
      sandboxInfoTitle: '왜 설정을 고르나요?',
      sandboxInfoContent: '설정마다 계정, 리전, 할당량이 다릅니다. 세션의 샌드박스가 한 번 만들어지면 첨부 파일, 산출물, 회수는 그 설정에 고정됩니다. 여기서 바꾸면 이후에 새로 만들어지는 샌드박스에만 영향을 줍니다.',
