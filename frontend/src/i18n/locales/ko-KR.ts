export default {
  ...
  settings: {
    ...
    sandbox: {
      ...
      pageHint: '스킬 샌드박스는 에이전트 스킬 스크립트가 실행되는 격리 환경입니다. 워크스페이스에 Docker, E2B, CubeSandbox 또는 로컬 프로세스 설정을 여러 개 둘 수 있고, 각 에이전트가 하나를 선택합니다. 선택하지 않으면 스크립트는 실행되지 않습니다.',
      ...
      inventorySummary: '이 설정이 아직 샌드박스 {count}개를 점유하고 있습니다.',
      inventoryEmpty: '실행 중인 샌드박스가 없습니다.',
      inventorySessions: '샌드박스를 점유한 세션',
      inventorySessionsHint: '세션을 종료하거나 삭제하면 샌드박스가 회수됩니다.',
      inventoryAgents: '이 설정에 연결된 에이전트',
      inventoryNoAgents: '이 설정을 사용하는 에이전트가 없습니다.',
      inventoryAgentWarning: '이 설정을 삭제하면 다음 스킬 실행에서 실패합니다.',
      inventoryOpenSession: '열기',
  ...
  agent: {
    ...
    editor: {
      ...
      skillsConfigDesc: '스킬 스크립트가 실행될 샌드박스를 선택한 뒤, 해당 샌드박스에서 Agent가 사용할 Skills를 구성합니다. 사용 가능한 Skills는 선택한 샌드박스에 따라 달라집니다.',
      skillsSelection: 'Skills 선택',
      skillsSelectionDesc: 'Agent가 사용할 수 있는 Skills 범위를 선택하세요. 목록은 위에서 선택한 샌드박스에 설치된 스킬에서 가져옵니다.',
      skillsAll: '전체',
      skillsSelected: '지정',
      skillsNone: '비활성화',
      selectSkills: 'Skills 선택',
      selectSkillsDesc: '활성화할 Skills 선택',
      noSkillsAvailable: '이 샌드박스에 사전 설치된 Skills가 없습니다. 스킬 샌드박스 설정에서 설치할 수 있습니다.',
      skillsNeedSandbox: '먼저 샌드박스 설정을 선택하세요. 사용 가능한 Skills는 해당 샌드박스에 설치된 스킬에서 가져옵니다.',
      goSandboxSettings: '스킬 샌드박스 설정에서 구성',
      sandboxBackend: '샌드박스 설정',
      sandboxBackendDefault: '샌드박스 사용 안 함',
      sandboxBackendHint: '스킬 스크립트는 선택한 샌드박스에서 실행됩니다. 선택하지 않으면 실행되지 않으며, 선택한 설정이 삭제되면 스킬 실행이 실패합니다.',
      sandboxBackendMissing: '설정이 삭제됨',
      sandboxNoConfigs: '이 워크스페이스에 샌드박스가 없어 스킬 스크립트가 실행되지 않습니다.',
      skillsInfoTitle: 'Skills와 샌드박스는 어떻게 연동되나요?',
      skillsInfoContent: 'Skills는 사전 설치된 전문 지식 모듈이며, 스크립트는 선택한 샌드박스에서 격리 실행됩니다. 목록은 해당 샌드박스에 설치된 스킬에서 가져옵니다. 세션 샌드박스가 한 번 만들어지면 첨부 파일, 산출물, 회수는 그 설정에 고정되며, 여기서 바꾸면 이후에 새로 만들어지는 세션에만 영향을 줍니다.'
    },
    messages: {
