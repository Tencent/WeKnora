export default {
    ...
    globalSettings: {
      ...
      keyLabels: {
        ...
        model: {
          ...
          max_rpm: "모델 기본 분당 요청 수 상한",
          max_tpm: "모델 기본 분당 토큰 수 상한",
        },
      },
      keyDescriptions: {
        auth: {
        ...
        model: {
          ...
          max_rpm:
            "백그라운드 작업이 단일 모델에 대해 갖는 기본 분당 요청 수(RPM) 상한이며, 모델 ID 기준으로 모든 복제본이 공유하여 제공자 RPM 할당량에 맞춥니다. 재시작 없이 즉시 적용됩니다. 0 또는 음수는 기본 상한을 해제합니다(각 모델은 자체 상한을 계속 준수함). 백그라운드 작업에만 영향을 주며 대화형 채팅에는 영향을 주지 않습니다.",
          max_tpm:
            "백그라운드 작업이 단일 모델에 대해 갖는 기본 분당 토큰 수(TPM) 상한이며, 모델 ID 기준으로 모든 복제본이 공유하여 제공자 TPM 할당량에 맞춥니다(대용량 문서 보강이 가장 먼저 도달함). 재시작 없이 즉시 적용됩니다. 0 또는 음수는 기본 상한을 해제합니다(각 모델은 자체 상한을 계속 준수함). 백그라운드 작업에만 영향을 주며 대화형 채팅에는 영향을 주지 않습니다.",
        },
      },
      enumLabels: {
        auth: {
  ...
  model: {
    ...
    editor: {
      ...
      maxRpmLabel: "백그라운드 분당 요청 수(RPM) 상한",
      maxTpmLabel: "백그라운드 분당 토큰 수(TPM) 상한",
      maxRatePlaceholder: "0이면 전역 기본값 사용",
      maxRpmDesc: "문서 인덱싱/보강 등 백그라운드 작업이 이 모델을 호출하는 분당 요청 수를 제한하여(모델별로 모든 복제본이 공유) 제공자 RPM 할당량에 맞춥니다. 0 또는 비워 두면 전역 기본값을 사용하며, 대화형 채팅에는 영향을 주지 않습니다.",
      maxTpmDesc: "문서 인덱싱/보강 등 백그라운드 작업이 이 모델을 호출하는 분당 토큰 수를 제한하여(모델별로 모든 복제본이 공유) 제공자 TPM 할당량에 맞춥니다(대용량 문서 보강이 가장 먼저 도달함). 0 또는 비워 두면 전역 기본값을 사용하며, 대화형 채팅에는 영향을 주지 않습니다.",
