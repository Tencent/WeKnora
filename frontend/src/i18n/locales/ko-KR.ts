export default {
  ...
  agentStream: {
    ...
    skillCards: {
      found: '스킬 {count}개를 찾았습니다',
      resolved: '스킬 {name}을(를) 확인했습니다',
      notFound: '맞는 스킬을 찾지 못했습니다',
      unavailable: '{sources}에 연결할 수 없습니다',
      temporaryNotice: '{name}은(는) 이 대화에서 임시로 불러온 것일 뿐 워크스페이스에 설치되지 않았습니다. 스킬 목록에 나타나지 않고, 다른 대화에서는 쓸 수 없습니다.',
      temporaryNoticeUnnamed: '이 스킬은 이 대화에서 임시로 불러온 것일 뿐 워크스페이스에 설치되지 않았습니다. 스킬 목록에 나타나지 않고, 다른 대화에서는 쓸 수 없습니다.',
      official: '공식',
      ...
      retry: '다시 시도',
      copySource: '소스 복사',
      sourceCopied: '스킬 소스를 복사했습니다',
      viewSource: '자세히 보기',
      adminOnly: '워크스페이스 관리자가 설치해야 합니다. 소스를 복사해 관리자에게 보내세요',
      noSandbox: '이 대화에는 스킬을 설치할 샌드박스가 없습니다',
      alreadyInstalled: '이 샌드박스에 같은 이름의 스킬이 이미 설치되어 있습니다. 설치하면 교체됩니다',
      alreadyInstalling: '이 샌드박스에서 같은 이름의 스킬을 설치하는 중입니다',
      sameNameInCatalog: '워크스페이스에 같은 이름의 스킬이 있습니다. 설치하면 이 소스로 교체됩니다',
      installing: '설치 중 {percent}%',
      ready: '설치했습니다. 다음 메시지부터 사용할 수 있습니다',
      readyNewSession: '설치했습니다. 새 대화부터 사용할 수 있습니다',
      selectInAgent: '이 에이전트는 선택한 스킬만 사용하므로 에이전트 설정에서 선택해야 합니다',
      failed: '설치하지 못했습니다',
      failedWithReason: '설치하지 못했습니다: {reason}',
      installerModelRequired: '먼저 스킬 설치에 사용할 모델을 선택하세요',
      confirmTitle: '스킬 "{name}"을(를) 설치할까요?',
      confirmBody: '이 서드파티 스킬을 {source}에서 가져와 이 대화가 사용하는 샌드박스에 설치합니다. 설치 중에 스킬에 포함된 코드가 실행되며, 이후 이 샌드박스를 쓰는 모든 대화에서 사용할 수 있습니다. 이 대화의 샌드박스는 그다음 다시 만들어지며 /workspace의 임시 파일은 사라집니다. 신뢰하는 소스일 때만 계속하세요.',
      confirmBodyNewSession: '이 서드파티 스킬을 {source}에서 가져와 이 대화가 사용하는 샌드박스에 설치합니다. 설치 중에 스킬에 포함된 코드가 실행됩니다. 이 샌드박스의 업데이트 정책에 따라 이후에 시작한 대화에서만 사용할 수 있고, 진행 중인 대화는 그대로입니다. 신뢰하는 소스일 때만 계속하세요.'
    },
    skillFiles: {
      heading: '스킬 파일',
      script: '스크립트',
