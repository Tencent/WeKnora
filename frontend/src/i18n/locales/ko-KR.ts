export default {
  ...
  integrations: {
    ...
    api: {
      ...
      editApiKeyScopeDesc: "API Key \"{name}\"의 이름, 권한 범위 및 사용자 식별 설정을 변경합니다.",
      ...
      keyIdentityDesc: "이 API Key에만 적용됩니다. 세션과 MCP OAuth 권한을 분리하며 작업 권한은 Key의 권한 범위로 제어합니다.",
      keySignedHint: "백엔드에서 sub, tenant_id, aud=weknora, exp 및 이 Key의 identity_namespace를 포함한 단기 HS256 JWT를 서명하세요. 저장 후 서명 예제를 확인할 수 있습니다.",
      keySecretDraftHint: "시크릿을 백엔드에 복사하세요. 생성 또는 저장하면 적용되며 이 Key의 기존 서명 시크릿을 대체합니다.",
      keyModeChangeHint: "사용자 식별 모드를 변경하면 이전 모드의 세션과 MCP 권한은 이어지지 않습니다.",
      keyExampleDesc: "API Key를 선택하여 사용자 식별 설정, 요청 예제와 Playground를 확인하세요.",
      identityNamespace: "앱 식별 네임스페이스",
      identityApplication: "앱 식별",
      identityApplicationHint: "새 앱은 기본적으로 격리됩니다. 동일한 앱의 Key를 교체할 때 기존 Key를 선택하면 사용자, 세션, MCP 권한이 유지됩니다.",
      identityNewApplication: "새 앱 (격리)",
      identityReuseHint: "선택한 Key의 사용자 식별 모드와 서명 설정을 복사합니다. 작업 권한은 현재 폼을 따르며 생성 후 개별 편집할 수 있습니다.",
      legacyIdentityHint: "이 Key는 업그레이드 이전의 사용자 연결을 유지합니다. 동일한 외부 사용자 ID는 다른 기존 Key와 세션 및 MCP 권한을 공유할 수 있습니다.",
      ...
      requireDirectHeaderDesc: "켜면 사용자 ID가 없는 요청을 거부합니다. 끄면 앱의 공통 사용자 식별을 사용합니다. 기존 Key는 종전의 워크스페이스 식별을 유지합니다.",
      tokenHeader: '토큰 헤더',
