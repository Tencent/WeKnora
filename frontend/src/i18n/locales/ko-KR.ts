  ...
  knowledgeEditor: {
    ...
    advanced: {
      ...
      multimodal: {
        ...
        imagePostProcessLabel: '이미지 후처리',
        imagePostProcessDescription: '켜면 2단계 파이프라인이 실행됩니다: 먼저 배치로 분류+설명을 수행한 뒤, 아래 클래스 정책에 따라 OCR 여부를 결정하고 선택한 클래스를 자동 비활성화합니다. 끄면 기본 모드: 모든 이미지를 하나씩 설명하고 모두 OCR합니다',
        imageBatchSizeLabel: '인식 배치 크기',
        imageBatchSizeDescription: '한 번의 인식 요청에 묶을 이미지 수. 요청 수를 절약합니다(토큰은 불변). 요청당 과금 API에 유용. 1=한 장씩, 최대 16',
        imageDownscaleLabel: '인식 전 축소',
        imageDownscaleDescription: '인식 전에 이미지를 긴 변 640px로 축소하여 토큰 소모를 크게 절감. 고해상도 처리에 강한 모델은 끌 수 있습니다',
        imageClassPoliciesLabel: '클래스 정책',
        imageClassPoliciesDescription: '클래스별 후처리: 비활성화는 본문에서 참조를 제거하여 이미지가 본문을 통해 모델에 전달되지 않게 합니다(설명과 OCR 텍스트는 계속 생성되어 검색 대상에 남습니다). OCR은 이미지 내 텍스트 추출을 실행합니다',
        policyDisabledCol: '비활성화',
        policyOcrCol: 'OCR',
        imageClassChart: '차트',
        imageClassDecorative: '장식 이미지',
        imageClassLogo: '로고',
        imageClassPhoto: '사진',
        imageClassTableImage: '표 스크린샷',
        imageClassTextScreenshot: '텍스트 스크린샷',
        imageClassOther: '기타',
