export default {
  ...
  datasource: {
    ...
    field: {
      ...
      operatorId: "운영자 unionId",
      operatorIdHint: "딩톡 위키 API는 실제 사용자 신원이 필요합니다. 대상 지식베이스에 접근 가능한 사용자의 unionId(userId 아님)를 입력하세요.",
      dingtalkBaseUrlHint: "비워두면 https://api.dingtalk.com 을 사용합니다. 전용/사설 딩톡 배포 시 API 호스트를 입력하세요.",
    ...
    prereqBarText_dingtalk: "처음 사용하시나요? 클릭하여 딩톡 문서 설정 가이드를 확인하세요",
    prereqStep1Brief_dingtalk: "내부 앱을 만들고 자격 증명 받기",
    prereqStep1Desc_dingtalk: "딩톡 개발자 콘솔 → 앱 개발 → 내부 앱 생성 → 자격 증명에서 AppKey / AppSecret 복사",
    prereqStep2Brief_dingtalk: "지식베이스 읽기 권한 부여",
    prereqStep2Desc_dingtalk: "최소한 Document.Workspace.Read, Document.WorkspaceDocument.Read, Wiki.Node.Read 를 활성화한 뒤 앱을 게시하세요",
    prereqStep3Brief_dingtalk: "운영자 unionId 입력",
    prereqStep3Desc_dingtalk: "대상 지식베이스에 접근 가능한 사용자의 unionId를 사용하세요 (userId 아님)",
