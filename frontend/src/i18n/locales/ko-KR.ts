export default {
  ...
  datasource: {
    ...
    connectorDesc: {
      ...
      dingtalk: "딩톡 지식베이스에서 온라인 문서 동기화",
    ...
    field: {
      ...
      operatorId: "운영자 UnionId (선택)",
      operatorIdHint: "지식베이스 API 호출 시 사용할 사용자 신원입니다. 비워 두면 조직 관리자의 UnionId를 자동으로 사용합니다 (qyapi_get_member 연락처 권한 필요).",
    ...
    prereqBarText_dingtalk: "처음 사용하시나요? 클릭하여 딩톡 앱 설정 가이드를 확인하세요",
    prereqStep1Brief_dingtalk: "딩톡 기업 내부 앱 생성",
    prereqStep1Desc_dingtalk: "딩톡 개발자 콘솔 → 앱 개발 → 앱 생성 후 「자격 증명 및 기본 정보」에서 Client ID (AppKey)와 Client Secret (AppSecret)을 복사하세요",
    prereqStep2Brief_dingtalk: "필요한 권한 부여",
    prereqStep2Desc_dingtalk: "앱의 권한 관리에서 Wiki.Workspace.Read 및 Wiki.Node.Read (지식베이스 읽기), Storage.File.Read (문서 내보내기/다운로드, 스토리지 분류), qyapi_get_member (운영자 신원 자동 확인)를 활성화하세요",
    prereqStep3Brief_dingtalk: "(선택) 운영자 UnionId 지정",
    prereqStep3Desc_dingtalk: "지식베이스 API는 운영자 신원으로 인증하며 해당 사용자가 접근 가능한 콘텐츠만 동기화합니다. 비워 두면 조직 관리자를 자동으로 사용합니다",
    prereqOpenConsole_dingtalk: "딩톡 개발자 콘솔로 이동",
