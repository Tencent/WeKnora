...
async function enhanceUploadedVideo(file: File, videoId: string, cancel: UploadCancel, trace: UploadTrace): Promise<void> {
	if (cancel.cancelled) return
	const poster = await generateVideoPoster(file).catch((error) => {
		logUploadEvent('poster_generate_failed', trace, { video_id: videoId, error: errorMessage(error) }, 'warn')
		return null
	})
	if (!poster || cancel.cancelled) return
	await uploadVideoPoster(videoId, poster, trace).catch((error) => {
