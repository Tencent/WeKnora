...
export function recordPageView(payload: { knowledge_base_id: string; slug: string; duration: number }) {
  return post<{ success: boolean; mastery?: number; recently_active?: boolean; last_active?: string }>(
    `/api/v1/knowledge-bases/${encodeURIComponent(payload.knowledge_base_id)}/memory/page-view`,
    { slug: payload.slug, duration: payload.duration },
  )
}
...
export function getMasteryProfile(kbId: string) {
  return get<{ data: MasteryNodeDetail[]; total: number }>(
    `/api/v1/knowledge-bases/${encodeURIComponent(kbId)}/memory/mastery`,
  )
}
...
export function recordExposure(payload: { knowledge_base_id: string; trigger_slug: string; candidate_slugs: string[] }) {
  return post<{ success: boolean }>(`/api/v1/knowledge-bases/${encodeURIComponent(payload.knowledge_base_id)}/memory/exposure`, { trigger_slug: payload.trigger_slug, candidate_slugs: payload.candidate_slugs })
}

/** 记录一次边界候选点击（供 PPR 点击率验证）。 */
export function markExposureClicked(payload: { knowledge_base_id: string; candidate_slug: string }) {
  return post<{ success: boolean }>(`/api/v1/knowledge-bases/${encodeURIComponent(payload.knowledge_base_id)}/memory/exposure/click`, { candidate_slug: payload.candidate_slug })
}
...
export function exportMasteryProfile(kbId: string): Promise<Blob> {
  return getDown(`/api/v1/knowledge-bases/${encodeURIComponent(kbId)}/memory/mastery/export`)
}
