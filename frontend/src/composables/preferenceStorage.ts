 ...
 *
 * Storage layout: GRIDORA_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (GRIDORA_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('gridora_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[GRIDORA] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `GRIDORA_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `GRIDORA_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `GRIDORA_anon_${suffix}`
    const legacyKey = `GRIDORA_${suffix}`
