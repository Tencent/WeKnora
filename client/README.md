...
11. **会话沙箱文件**：列出、上传、下载、重命名、删除会话已绑定沙箱的 `/workspace/output` 文件（`ListSandboxLiveFiles` / `OpenSandboxLiveFile` / `UploadSandboxLiveFile` / `RenameSandboxLiveFile` / `DeleteSandboxLiveFile`）
12. **长期记忆**：当前用户的跨会话记忆（设置开关、条目增删改、确认/否决、主题、文档亲和度、导出、立刻整理）
13. **认证**：登录、刷新令牌、切换激活空间（`SwitchTenant` 会写入最近活跃租户偏好）
