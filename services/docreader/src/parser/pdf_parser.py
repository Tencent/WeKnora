...
class PDFParser(BaseParser):
    """
    PDF Document Parser using pdfplumber, with correct API usage for filtering.
    """
    ...
    def parse_into_text(self, content: bytes) -> Union[str, Tuple[str, Dict[str, Any]]]:
        ...
        try:
                ...
                for page_num, page in enumerate(pdf.pages):
                    ...
                    try:
                        ...
                        for img_index, img in enumerate(page.images):
                            image_name = f"page{page_num + 1}_img{img_index}.png"
                            bbox = (img['x0'], img['top'], img['x1'], img['bottom'])
                            cropped_image = page_image.crop(bbox)
                            image_map[image_name] = cropped_image
                            #markdown image placeholder
                            image_placeholder = f"![{image_name}]({image_name})\n"
                            page_content_parts.append(image_placeholder)
                            logger.info(f"Extracted image '{image_name}' from page {page_num + 1}")
                    except Exception as e:
            ...
            logger.info(f"PDF parsing complete. Extracted {len(final_text)} text chars and {len(image_map)} images.")
            
            return final_text, image_map
            
        except Exception as e:
            logger.error(f"Failed to parse PDF document with pdfplumber: {str(e)}")
            import traceback
