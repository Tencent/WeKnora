...
`GET /api/v1/artifacts` 列出当前用户可见会话中的所有生成文件（范围与会话列表一致：本人会话及历史上无归属的租户级会话），供首页侧栏「产物」页使用。同一会话中 `source_path` 相同的文件视为同一文件的多个版本，只返回最新一版，`version_count` 给出版本数。已删除的会话或消息中的文件不返回。权限要求与上表相同。
