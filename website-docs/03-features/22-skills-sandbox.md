...
技能让智能体按一份 SKILL.md 使用脚本、模板和资料；沙箱提供执行这些脚本的环境。先把技能加入空间目录，再安装到沙箱，最后在智能体中选择该沙箱和技能，即可在对话中使用。
...
5. 进入对话提问，或用 `@技能` 提醒智能体优先使用它。需要文件时上传附件，生成的交付文件在对话的文件面板预览、下载。
...
默认执行账号是沙箱内 `root`；模板保留 `user` 供显式选择。隔离依赖容器或远端沙箱，`/workspace` 路径约定不构成 root 命令的文件系统权限边界。
...
个人技能变量名称必须已在技能声明中，允许技能声明所需的 `WEKNORA_*` 凭据名；个人沙箱自定义变量拒绝 `WEKNORA_*`、`PATH` 等保留名。执行命令中识别的变量仅补未设置的个人值，不覆盖已有个人/空间配置。详情和 curl 见[个人变量 API](../04-api/02-api-sandbox-skills.md#个人环境变量)。
