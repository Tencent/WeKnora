...
`local` 宿主机进程后端已移除：它在本机裸跑，没有任何隔离。Lite 的 `host` 不是这条配置的替代项。`host` 由操作系统强制隔离，工作区是真实主机路径，没有 `/workspace`。用户在新对话页选择项目；不选则使用自动会话目录。配了 Docker、E2B 或 Cube 的智能体仍走远程沙箱。当前配置的完整字段见[沙箱与技能 API](../04-api/02-api-sandbox-skills.md)。
