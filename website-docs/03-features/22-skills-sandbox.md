...
在对话里用 shell 下载技能（`npx skills add`、`clawhub install`、`git clone` 到 `skills/` 目录等）只是**临时加载**：本次对话可以读取、执行这些文件，但技能没有登记到空间，不在技能列表里，其他对话用不了。远程沙箱重建后这些文件也会消失；Lite 的主机工作区里文件会留在磁盘上，但同样不算已安装。系统允许这样做，并会：
...
`local` 宿主机进程后端已移除。桌面客户端另有基于操作系统隔离的 `host` 后端，不属于上述空间命名配置。当前配置的完整字段见[沙箱与技能 API](../04-api/02-api-sandbox-skills.md)。
