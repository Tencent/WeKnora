...
1. 新建文档知识库，或编辑尚无内容的库，在「索引策略」勾选「Wiki 知识库」；
2. 配置对话模型并上传文档；已有内容的库会锁定索引策略，不能直接切换；
...
仅体验 Wiki 与练习时，可取消「RAG 检索」，无需配置 Embedding 或 Neo4j。完整建库、阅读、测验与 Agent 操作见[引导式学习使用手册](24-guided-learning.md)。
