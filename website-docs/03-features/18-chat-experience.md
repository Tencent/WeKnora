...
问题大纲可直接跳到某次提问；时间分隔帮助区分不同日期的消息。侧栏展示仍在执行的会话，API 会话显示来源/归属标记。Agent 时间线展示命令执行、输出及文件写入进度，最终结果仍以工具完成状态和回答为准。

### 生成文件面板

绑定沙箱的 Agent 可把结果写到 `/workspace/output`。回答结束时收集产物并持久化，会话文件面板集中列出结果，支持按文件类型预览及下载；每条回答也能查看自己的产物。普通中间文件不应放入 output。

上传文件进入 `/workspace/input`，可用 read_file 或 shell_exec 读取和处理；二进制文档需由适当工具解析。输入附件与输出产物分别管理，删除输入附件不会自动把输出变成知识库文档。完整约定见[技能与沙箱](22-skills-sandbox.md)，列表/下载见[聊天 API](../04-api/02-api-chat.md)。

### 个人长期记忆

空间启用记忆后，可在个人记忆中维护偏好、确认待确认项，并跨会话使用。记忆按空间和调用者隔离，关闭或清空的区别、自动提取与立即整理见[长期记忆](23-memory.md)。
