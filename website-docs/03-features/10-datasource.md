...
- **批量而非流式**：一次同步会把**本次所有待入库文件的内容同时读进内存**再交给入库流程（单个文件仍受 `MAX_FILE_SIZE_MB` 限制，但总量不受限）。因此内存峰值约等于本次新增或变更文件的大小之和——首次接入一个大库时最明显，之后的增量同步只包含真正变化的文件。库很大时建议先用 include 规则分批接入。
