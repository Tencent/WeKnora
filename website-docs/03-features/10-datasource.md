...
- **目录结构（`settings` 配置项）**：默认所有文档平铺在知识库根目录。两个键控制这一点：
  ...
  - **`toc_only`**（默认 `false`）：只同步出现在该书目录节点列表中的文档。判据是"是否出现在目录中"，不是"是否有路径"——挂在目录下但未归入任何分组的文档仍会保留。**它是准入过滤器，不是清理器**：被过滤的文档只是不入库，已经同步进知识库的副本原样保留，不会被报成删除。语雀 Web UI 不显示"通过 API 创建、从未挂到目录"的文档，若把这类文档判为已删除，其内容会在源端和知识库两侧都不可达。
  - **`include_book_title`（已移除）**：书名现在是 `toc` 模式下无条件的路径首层。它是"属于哪本书"这一维度的唯一载体（自动标签的粒度是每个数据源一个，不是每本书一个），省掉它会静默合并不同书的同名顶层分组；该键若仍存在会被忽略，不影响同步。
