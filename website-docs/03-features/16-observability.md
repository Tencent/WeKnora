...
- 消费方：`knowledgebase.go` / `custom_agent.go` 仓储的 `ListModelUsages` 复用上述 scope，返回租户内活动对象的最小投影（`id`、`name`、合并后的 `bindings`）；`internal/application/service/model.go` 的删除守卫以该详情为唯一判断输入，并在 HTTP 400 的 `error.details` 中返回它。`CountByModelID` 保留用于兼容和仓储级测试。
