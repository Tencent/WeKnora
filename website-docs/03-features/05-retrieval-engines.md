...
- **关键词检索**：对 `content_sparse` 做 BM25 稀疏向量检索（Milvus 2.5+ 原生全文检索），查询时根据问题脚本传入 `analyzer_name`。
...
- 配置：`MILVUS_ADDRESS` / `MILVUS_USERNAME` / `MILVUS_PASSWORD` / `MILVUS_DB_NAME` / `MILVUS_METRIC_TYPE`（改后需重建 collection）。旧 Collection 的 schema 不能直接改成多语言分析器；可运行 `go run ./cmd/milvus-migrate --source <旧前缀> --target <新前缀>` 复用已有稠密向量迁移，确认检索正常后把 `MILVUS_COLLECTION` 改为新前缀。
