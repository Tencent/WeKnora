...
**租户切换即换发 token**：`SwitchTenant` 校验目标租户的 active 成员资格（跨租户超级用户除外）后，签发携带新 `tenant_id` claim 的新 token 对，并 best-effort 撤销旧 refresh token。换签成功后还会把目标空间记入「最近活跃租户」偏好（`Preferences.LastActiveTenantID`），下次登录即落在最近激活的空间。
