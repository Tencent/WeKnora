...
打开文档详情抽屉，点击顶部下载按钮旁的历史图标，即可在小浮层中查看版本号、上传日期、文件大小和下载入口；当前版本会单独标记。文件卡片或列表的「更多 → 文件版本」也可直接打开该浮层。有写权限且符合原文件下载权限的用户，可以从文件卡片或列表的三点菜单点击「上传新版本」，选择文件后在历史浮层中确认上传，也可使用浮层底部的上传入口；只读用户仅查看版本信息。
