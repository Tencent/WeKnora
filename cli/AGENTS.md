...
```json
{
  ...
  "error": {
    ...
    "retry_argv": ["weknora", "auth", "login", "--host", "https://kb.example.com"],
...
below). `hint` is prose; `retry_argv` is an optional directly-executable argv array. It is present only
when the CLI can reproduce the requested operation completely without
embedding secret or stdin-backed content. Consumers MUST execute the array
directly and MUST NOT join it into a shell command string. File-path arguments
are preserved as paths and are read again when the retry runs. For exit-10
(`input.confirmation_required`), it is informational only — the human must
approve the write explicitly. If absent, inspect
`error.detail.unreplayable_flags` and ask the human to re-run the original
invocation with `-y`; do not synthesize a partial command or copy stdin/secret
content into argv.
...
| | |
|---|---|
| **WeKnora** | two separate fields: `retry_argv` (suggested next argv, directly-executable for non-destructive errors; informational only on exit-10) + `hint` (prose) |
...
2. **Auto-execute the retry_argv without approval.** The retry_argv is *informational* --
