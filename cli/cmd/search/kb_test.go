...
func TestKBSearch_Substring(t *testing.T) {
	...
	require.NoError(
		...
		runKBSearch(
			context.Background(),
			&KBSearchOptions{Query: "marketing", Limit: 20},
...
func TestKBSearch_CaseInsensitive(t *testing.T) {
	...
	require.NoError(
		...
		runKBSearch(
			context.Background(),
			&KBSearchOptions{Query: "engineering", Limit: 20},
			&cmdutil.FormatOptions{Mode: cmdutil.FormatText},
			svc,
		),
	)
...
func TestKBSearch_LimitHardCap(t *testing.T) {
	...
	require.NoError(
		...
		runKBSearch(
			context.Background(),
			&KBSearchOptions{Query: "match", Limit: 2},
			&cmdutil.FormatOptions{Mode: cmdutil.FormatText},
			svc,
		),
	)
...
func TestKBSearch_NoMatches(t *testing.T) {
	...
	require.NoError(
		...
		runKBSearch(
			context.Background(),
			&KBSearchOptions{Query: "bar", Limit: 20},
			&cmdutil.FormatOptions{Mode: cmdutil.FormatText},
			svc,
		),
	)
...
func TestKBSearch_NetworkError(t *testing.T) {
	...
	err := runKBSearch(
		context.Background(),
		&KBSearchOptions{Query: "x", Limit: 20},
		&cmdutil.FormatOptions{Mode: cmdutil.FormatText},
		svc,
	)
	require.Error(t, err)
