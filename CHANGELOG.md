...
- **NEW**: **Knowledge Base Parse Stats** — `GET /knowledge-bases/{id}/parse-stats` returns per-status parse distribution for the KB; compact progress bar in the document list header shows real-time counts for completed / processing / finalizing / pending / failed / cancelled.
