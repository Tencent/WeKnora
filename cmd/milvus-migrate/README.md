...
```powershell
go run ./cmd/milvus-migrate --source weknora_embeddings --target weknora_embeddings_multilingual --metric-type COSINE
```

如果当前 PowerShell 会话已经导入了 `MILVUS_ADDRESS`、`MILVUS_COLLECTION` 和 `MILVUS_METRIC_TYPE`，可以省略对应参数。仅仅把变量写在 `.env.local` 中不会自动注入 `go run` 进程；不确定时建议显式传参：
...
迁移完成后，把 WeKnora 的 `MILVUS_COLLECTION` 改为目标前缀并重启服务：
...
# 运行迁移；迁移只复制到新 Collection，不删除旧 Collection
go run ./cmd/milvus-migrate --address 127.0.0.1:19530 --source weknora_embeddings --target weknora_embeddings_multilingual --metric-type COSINE
```
